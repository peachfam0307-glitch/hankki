...
// 저장 쓰기(재시도 포함): 순간적인 저장 실패(일시적 오류·순간 용량 부족)는 몇 번 다시 시도해서
// 넘긴다. 그래야 실제로는 저장이 됐는데 경고가 잠깐 떴다 사라지는 일이 안 생긴다.
// 반환: true=저장 성공, false=여러 번 시도해도 실패(정말 공간 부족 등).
async function storageSet(key, value, tries = 4) {
  for (let i = 0; i < tries; i++) {
    try {
      const res = await window.storage.set(key, value);
      if (res !== false) return true; // true/undefined 등은 성공으로 간주
    } catch (e) { /* 아래에서 잠깐 쉬고 재시도 */ }
...
export default function WrongNoteTracker() {
  ...
  async function handleSolPhotoSelect(e) {
    ...
    const list = [];
    // 사진 읽기(압축) 실패는 저장 실패가 아니므로 저장 경고를 띄우지 않고 해당 사진만 건너뛴다.
    for (const f of picked) {
      try { list.push(await compressFile(f)); } catch (err) {}
    }
    if (list.length > 0) {
  ...
  async function addEntry() {
    ...
    if (form.photos.length > 0) {
      ...
      let ok = await storageSet(`wrongnote:img:${id}`, JSON.stringify(photos));
      // 그래도 실패하면(정말 용량 부족) 사진을 한 번 더 작게 줄여 재시도 — 경고 대신 저장 성공을 노림
      if (!ok) {
        try {
          const smaller = [];
          for (const p of photos) smaller.push(await shrinkDataUrl(p));
          photos = smaller;
          ok = await storageSet(`wrongnote:img:${id}`, JSON.stringify(photos));
        } catch (e) {}
      }
      hasImage = ok;
      if (ok) setImageCache((c) => ({ ...c, [id]: photos }));
  ...
  async function resetAll() {
    ...
    try {
      ...
      await storageSet(STORAGE_KEY, JSON.stringify({ entries: [], subjects, customTags, motto }));
      setEntries([]);
      setImageCache({});
      setSolPhotoCache({});
      setVariants({});
  ...
  async function saveSolution(entryId) {
    ...
    if (solForm.photos.length > 0) {
      let photos = solForm.photos;
      let ok = await storageSet(`wrongnote:sol:${sid}`, JSON.stringify(photos));
      if (!ok) {
        try {
          const smaller = [];
          for (const p of photos) smaller.push(await shrinkDataUrl(p));
          photos = smaller;
          ok = await storageSet(`wrongnote:sol:${sid}`, JSON.stringify(photos));
        } catch (e) {}
      }
      hasPhotos = ok;
      if (ok) setSolPhotoCache((c) => ({ ...c, [sid]: photos }));
      else setSaveError(true);
    }
