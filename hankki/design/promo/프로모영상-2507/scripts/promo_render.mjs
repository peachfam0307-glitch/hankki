...
let trim = 1.4
try {
  const bd = execFileSync(ffmpeg, ['-i', `${REC}/${webm}`, '-vf', 'blackdetect=d=0.1:pix_th=0.10', '-an', '-f', 'null', '-'], { encoding: 'utf8' })
  const m = String(bd).match(/black_start:0[.\d]*\s+black_end:([\d.]+)/)
  if (m) trim = Math.max(0, parseFloat(m[1]) - 0.03)
} catch (e) { const m = String(e.stderr || '').match(/black_start:0[.\d]*\s+black_end:([\d.]+)/); if (m) trim = Math.max(0, parseFloat(m[1]) - 0.03) }
