...
// 🙈 **손가락을 잠깐 감춘다** — 「읽어야 하는 화면」에선 곰이 글을 가린다.
//   📮 창업자 2026-08-16 = *"일기클릭하고 **커서가 일기글을 안보이게 막고있어** 그것만 고치면 좋을 것 같아"*
//   ⛔ 맞는 지적이고 프레임으로 확인했다 — 46~47초에 곰이 종이 한가운데 손글씨 위에 앉아 있었다.
//   ⭐ 곰은 «누르는 것을 보여주는» 장치다. 다 누르고 «읽는» 동안엔 있을 이유가 없다.
//      (`뒤로()` 도 같은 이유로 이미 감추고 있었다 — 그 한 줄을 꺼내 쓴다)
const 손감추기 = () =>
  page.evaluate(() => { const c = document.getElementById('hkcur'); if (c) c.classList.remove('on') })
...
// 🙈 **여기서 곰을 감춘다** — 일기는 «읽는» 화면이고, 종이 한가운데 앉으면 손글씨를 가린다(창업자 지적)
await 손감추기()
await 쉼(3000)

// 🎨 ⑪-2 **일꾸(일기 꾸미기)** — 창업자 2026-08-16
//   📮 *"일기화면 옆에 꾸미기는 없네?"* → *"**일꾸도 띄우자. 화면이 휑해보이니까.**"*
//   ⭐ 맞다 — 일기 «한 장»은 종이만 있어서 가로에서 오른쪽이 빈다.
//      꾸미기를 열면 **종이 왼쪽 · 서랍 오른쪽**으로 화면이 꽉 찬다(레꾸와 같은 문법).
//   ⭐⭐ 그리고 이게 우리 필살기다 — 표지 꾸미기(④)는 «레시피»를, 일꾸는 «일기»를 꾸민다.
//      둘 다 보여줘야 「꾸미는 앱」이라는 게 전해진다.
//   ⛔ **「마테」 탭인 채로 두면 서랍이 여전히 휑하다** — 찍어 보고 알았고 창업자도 같이 짚었다:
//      📮 *"꾸미기도 **데코 많은 부분으로 넣자. 지금 마테는 너무 몇개 없어**"*
//      🔢 마테는 띠 몇 개뿐이라 오른쪽이 텅 빈다. **데코가 우리 서랍에서 제일 많다.**
장면('⑪-2 일꾸 — 일기 꾸미기')
await 누르기(page.getByRole('button', { name: '꾸미기 열기' }).first(), '일기 꾸미기', 1400)
await 누르기(page.getByText('데코', { exact: true }).first(), '데코 탭', 1100)
await 손감추기()
await 쉼(3400)

// ⑪ 장보기 — ⑨에서 담은 재료가 「살 것 목록」으로 들어와 있다
장면('⑫ 장보기')
// ⚠️ 이제 «두 겹»이다 — 일꾸(층) → 일기 한 장(화면) → 일기 탭.
//    ⛔ 한 겹만 하면 일기 한 장에 선 채로 「장보기」 탭을 찾다가 못 찾는다(하단바가 없다).
//    ⭐ 일꾸는 «뒤로가기로 닫으면 묻지 않는다»(`DecorEditor.jsx:516`) — 「취소」를 누르면 시트가 뜬다.
await 뒤로(2)                      // 일꾸 → 일기 한 장 → 일기 탭
await 탭누르기('장보기', 1300)
await 쉼(3200)
