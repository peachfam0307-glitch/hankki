...
for (const [날, 편들] of [...날짜별].sort((a, b) => a[0] < b[0] ? -1 : 1)) {
  ...
  for (const r of 편들) {
    ...
    // 🗂 [절대원칙 ④ · 창업자 2026-09-14] «갈래·폴더»도 같이 본다.
    //   📮 창업자 = *"검수할때 한식 양식도 같이 넣자"* ＋ *"이름에 따라서 혹시 다른탭에 들어갈수있으니까"*
    //   ⭐⭐ **레시피 탭에 뜨는 건 갈래(category)가 아니라 «폴더(folder)»다.**
    //      🔢 실측(2026-09-14) = 둘이 다른 편이 27편 — 그래서 화면은 「한식 42」인데 실제 한식은 69편이다.
    //   ⛔ 여기서 어느 쪽이 옳은지 «판정하지 않는다» — 창업자가 편별로 본다(창업자 판정 = 지금은 두고 검수판에만).
    //      그래서 둘이 다르면 화살표로 «둘 다» 보여 준다.
    const 갈래 = r.category || '(없음)'
    const 폴더 = r.folder || ''
    const 자리 = !폴더 || 폴더 === 갈래 ? 갈래 : `${갈래}→${폴더}`
    줄들.push(`   ${검수.padEnd(10)} ${아이콘.padEnd(16)} ${('[' + 자리 + ']').padEnd(14)} ${r.title}`)
    if (재료.length) 줄들.push(`      🛒 링크 없는 재료 ${재료.length}개 — ${재료.join(' · ')}`)
    // ❓❓ [2026-09-14 신설] 그 편에 «창업자에게 물어볼 것»이 적혀 있으면 검수할 때 반드시 띄운다
    //   📮 창업자 = *"나중에검수할때꼭말해줘"*
    //   ⛔⛔ 기억으로 두면 잊는다 — 오늘 하루에도 「물어본다」고 해놓고 흐름에 밀려 몇 번 넘어갔다.
    //   ⭐ 목록을 «손으로» 관리하지 않는다(규칙 22 — 손으로 적은 목록은 반드시 낡는다).
    //      레시피 «안»에 적어 두면 그 편을 검수할 때 저절로 따라온다.
    //   ⛔ 배포를 막지는 않는다 — 물어볼 것이지 틀린 것이 아니다.
    if (r.물어볼것) 줄들.push(`      ❓ 창업자에게 물어볼 것 — ${r.물어볼것}`)
  }
}
