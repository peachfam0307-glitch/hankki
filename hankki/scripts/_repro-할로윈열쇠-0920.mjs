...
//      ⛔⛔ 브라우저 판(`_probe-할로윈열쇠-0920.mjs`)은 **지웠다** — 세 번 다 헛돌았고 «위험»했다.
//         ⑴ 포트 4577 을 앞 판이 쥐어 EADDRINUSE 로 아예 안 돌았다(결과 0줄)
//         ⑵ 포트를 옮겨 돌렸더니 이번엔 **서랍을 못 열었다** — 「서랍 X」인데 「보이나 0」이 같이 찍혔다.
//         🚨 그 0 을 「열쇠가 잘 막았다」로 읽으면 **완전히 틀린 초록불**이 된다. 실제로 그렇게 읽을 뻔했다.
...
// ✅ [창업자 2026-09-21 22:2x 최종] 넷 = 냄비 hw01·hw04 ＋ 2차 시트의 접시 hw09·볼 hw10 (여섯→넷 · 「1,2번할게」)
잰다(컷.length === 4 && ['pf_hw01', 'pf_hw04', 'pf_hw09', 'pf_hw10'].every((k) => 컷.includes(k)), `③ 컷이 넷이다(hw01·hw04·hw09·hw10)`, `지금 ${컷.join(',')}`)
...
const s = 가짜저장()
// ⑧-b [2026-09-21] `열쇠까지: '2026-10-16'` — 그날부터는 열쇠 없이 열린다(아이폰은 구운 판이 박혀 그날 손댈 수 없어서 날짜로 연다)
const 열쇠까지 = (줄.match(/열쇠까지:\s*'(\d{4}-\d{2}-\d{2})'/) || [])[1]
const { isReleased } = await import('../src/season.js')
잰다(열쇠까지 === '2026-10-16', '⑧-a 묶음에 열쇠까지: 2026-10-16 이 붙어 있다', 열쇠까지)
잰다(/x\.열쇠까지\s*&&\s*isReleased\(x\.열쇠까지\)/.test(de), '⑧-b DecorEditor 열쇠켬 이 열쇠까지 를 읽는다')
잰다(isReleased(열쇠까지, new Date(2026, 9, 15)) === false && isReleased(열쇠까지, new Date(2026, 9, 16)) === true, '⑧-c 10/15 엔 닫혀 있고 10/16 엔 열린다')
