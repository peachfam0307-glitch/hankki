// 🍽 홈 「자주 해먹는」 — 프레임을 얹은 레시피의 그림이 «접시 안»에 드나 (창업자 확정 2026-09-01 = 갈래 「라」 · 2026-09-21 그릇배율로 «둘 다 70%»)
// ⭐ [2026-09-21] ①의 숫자가 0.56 → 0.70 으로 바뀌었다 — 창업자 「홈에서 내사진 들어간 레시피 그릇이 넘 작아」. 그릇을 아이콘 크기(70%)로 키우면서 그림도 같은 배율로 따라가니 «접시 안에 담긴다»(①-b)는 그대로다.
//
...
console.log('\n🍽 홈 「자주 해먹는」 음식 그림 — 프레임 얹은 칸도 70%·그림은 접시 안인가 (390×844)\n')
넣(프레임칸.length > 0 && 맨판칸.length > 0, '프레임 칸과 맨판 칸이 한 화면에 둘 다 있다',
  `프레임 ${프레임칸.length}칸 · 맨판 ${맨판칸.length}칸`)
// ① 프레임을 얹은 칸 — [2026-09-21 창업자] 그릇도 그림도 «같이» 70% 로 (v13.98 그릇배율). 9/1 「라」의 알맹이(그림이 레꾸에서 덮은 그대로 접시 안에 든다)는 ①-b 가 지킨다.
//    📮 창업자 2026-09-21 = *"홈에서 비교해봐 내사진 들어간 레시피 그릇이 넘 작아"* — 9/1 엔 그림만 56% 로 줄여 접시에 맞췄는데(그릇은 58% 그대로), 이제 그릇을 아이콘 크기로 키우고 그림은 그 배율로 같이 간다.
넣(프레임칸.length > 0 && 프레임칸.every((v) => Math.abs(v.비 - 0.70) < 0.02),
  '① 프레임 얹은 칸 = 그림÷판 0.70 (그릇이 아이콘 크기로 커진 만큼 그림도 같이 · 2026-09-21)', 프레임칸.map((v) => `${v.이름} ${v.비}`).join(' · '))
