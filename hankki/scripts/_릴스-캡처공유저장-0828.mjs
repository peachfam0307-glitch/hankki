...
const 장면들 = [
  ...
  //    📐📐 ⛔⛔ **줌으로 채우면 안 된다** — 1.30 으로 넣었더니 «글자가 잘렸다»
  //       (「**캡**처는」·「**링**크는」·「**메**모는」의 첫 글자와 「담으세요**!**」).
  //       이 시안은 좌우 여백 없이 꽉 찬 판이라 조금만 키워도 글을 먹는다. 절대원칙 21 이 잡았다.
  //    ✅ 줌은 거의 안 준다(1.00→1.03) ＋ **위쪽에 붙인다**(초점 y 860 = 위 여백 100px).
  //       아래에 남는 여백은 어차피 릴스 UI(좋아요·댓글·캡션)가 덮는 자리라 손해가 없다.
  { id: 's0', 그림: `${시안}/20-한끼홈쇼핑실험실-캡처는갤러리에메모는어디갔지.png`, 초: 2.4, 자막: '', 줌: [1.0, 1.03], 초점: [540, 860], 멈춤: 0.34 },
  ...
  { id: 's5', 그림들: 'abcdef'.split('').map((c) => `${앱화면}/28${c}-목록훅.png`), 초: 2.6, 자막: '레시피가 담겼어요', 줌: [1.04, 1.0], 초점: [540, 900], 멈춤: 0.30 },
