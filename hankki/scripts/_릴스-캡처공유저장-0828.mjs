...
// 🎨 창업자가 GPT 로 만들어 검수까지 마친 홍보 시안 24장 (2026-08-28 저장소에 들어왔다)
//    ⛔ 우리 생성기로는 «다시 못 뽑는다» — 잃으면 끝이라 저장소에 둔다.
const 시안 = join(ROOT, 'design/promo/홍보시안-2508')
...
const 장면들 = [
  ...
  //    · *"우리 병맛홍보시안있지않아? 그거 첫장에 넣을까."* → 창업자가 24장을 보내줬다
  //    ⭐⭐ 「한끼 홈쇼핑 실험실」을 골랐다 — 셋이 맞아떨어진다:
  //       ⑴ **종이(레시피)가 산더미**라 「쌓여있는」이 그림으로 바로 읽힌다
  //       ⑵ 문구가 「**캡처**는 갤러리에 · 링크는 메신저에 · 메모는 어디 갔지?」 —
  //          우리 릴스가 바로 그 «캡처» 얘기라 첫 장과 둘째 장이 이어진다
  //       ⑶ 「찾지 말고, **일단 담으세요!**」 가 다음 장면(공유하기)으로 넘어가는 다리다
  //    ⛔ 자막을 «안» 얹는다 — 시안에 이미 큰 글자가 다 들어 있어 겹치면 둘 다 죽는다
  //    ⛔ 줌으로 꽉 채우지 않는다 — 3:4 라 좌우를 잘라야 하는데 시안 양옆에 내용이 있다.
  //       위아래 크림 여백이 오히려 「포스터를 보여주는」 결이 된다.
  //    📐📐 ⭐ 이 판은 **941×1672 = 정확히 9:16** — 창업자가 릴스용으로 새로 뽑아 줬다.
  //       그래서 **잘림도 여백도 0**이다. 줌은 «숨쉬는 정도»만(1.00→1.02).
  //    ⛔ 더 키우면 위 「레시피가」·아래 「한끼로 가져오세요」가 먹힌다 —
  //       앞선 3:4 시안을 1.30 으로 채웠다가 첫 글자들이 잘렸다(절대원칙 21 이 잡았다).
  { id: 's0', 그림: `${시안}/25-레시피가산처럼쌓였다면-한끼로가져오세요.png`, 초: 2.4, 자막: '', 줌: [1.0, 1.02], 초점: [540, 960], 멈춤: 0.34 },
  ...
  // ⭐⭐ 「담겼어요」를 «흘려서» 보여준다 — 카드 한 장보다 «쭉 쌓인 것»이 훨씬 세다.
  //    🛠 재료 = `_shot-목록훅-0828.mjs` 가 스크롤 자리를 달리해 찍은 아홉 장
  //    ⛔ 첫 장(훅)에 있던 것을 여기로 옮겼다 — 첫 장은 창업자 시안이 맡고,
  //       여기서는 「공유했더니 이렇게 쌓인다」는 «결과»가 되어 자리가 더 맞다.
  //    🥬🥬 📮 창업자 = *"**레시피가 저장되었어요에 공심채가 없어**"*
  //       ⛔ 여섯 장 판은 목록 «맨 위»만 훑어서 **자막은 「담겼어요」인데 담긴 게 화면에 없었다.**
  //       ✅ 이제 **마지막 장이 공심채에 «착지»한다** — 흐르는 동안 「이만큼 쌓였다」,
  //          멈추는 순간 「그게 여기 있다」. 다음 s6·s7 이 공심채 재료·만드는 법이라 그대로 이어진다.
  //       ⛔ 그래서 **멈춤을 0.30 → 0.44 로 키웠다** — 착지한 판을 «보고 갈» 시간이 없으면
  //          아홉 장이 그냥 스쳐 지나가고 공심채는 또 안 보인 것이 된다(창업자 = *"너무 빠르게 지나가지 않게"*).
  { id: 's5', 그림들: 'abcdefghi'.split('').map((c) => `${앱화면}/28${c}-목록훅.png`), 초: 3.2, 자막: '레시피가 담겼어요', 줌: [1.04, 1.0], 초점: [540, 900], 멈춤: 0.44 },
