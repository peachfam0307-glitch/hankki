...
const 바랄값 = {
  ...
  'my-7': 'fe_444', // 제목으로 잡음
  // ⛔ 2026-08-31 — `my-8`(샐러드) 기대값을 `fy_y05` → **`gr_429`** 로 갱신.
  //    **창업자가 옛 컷 650장을 전수해 `fy_y05` 를 「카와이」로 판정했다**(431장 · *"다 보여줘도 돼 뿌리를 뽑자"*).
  //    ⚠️ 우리는 예전에 이걸 *"범용이라, 중복이라 내린 것이고 «얼굴이 없다»"* 로 적어뒀다(store.jsx v96 주석).
  //       **판정이 바뀌었다 — 창업자가 정한다**(규칙 11). 위 2026-08-29 `fe_129` 와 같은 자리다.
  'my-8': 'gr_429',
  ...
  // ⛔ 2026-08-29 — `my-12`(두루치기) 기대값을 `fe_129` → **`fe_417`** 로 갱신.
  //    **창업자가 175장을 전수해 `fe_129` 를 「카와이」로 판정했다**(*"카와이는 전부 삭제하고"*).
  //    ⭐ 그래서 이제 «그대로 남으면 안 되고» 실사 `fe_417`(돼지고기두루치기)로 갈려야 한다.
  //    📌 게이트가 낡은 게 아니라 **판정이 바뀐 것**이다 — 잣대를 새 판정에 맞춘다.
  'my-9': 'fe_446', 'my-10': 'fe_494', 'my-11': 'fe_42', 'my-12': 'fe_417', 'my-13': 'gr_400',
  'my-14': 'fh_k02', // 새 컷이 없다 = 그대로
  // ⛔ 2026-08-31 — my-13(뚝배기파스타 fe_168)·my-16(새우 해장 파스타 fy_y03) 도 창업자 전수 판정에서 카와이.
  //    fy_y03 은 basics.js·ICON_FORCE_V88 둘 다 gr_446 으로 옮겼다(규칙이 그 제목에 주는 값).
  'my-15': 'fe_436', 'my-16': 'gr_446',            // v11.34 사고 되돌리기 ＋ 카와이 철거
  // ⛔ [2026-09-14] my-18(간장 제육볶음) 은 fe_418 이었는데 **그것도 빨간 고추장 제육**이었다.
  //    📮 창업자 = *"간장인데 고추장들어가고"* → (그림 바꿨는데도) *"간장제육안들어감"*
  //    🌲 그림을 정하는 자리가 셋인데(이름표 규칙 · 레시피의 icon · 여기 강제표) **강제표가 제일 세다.**
  //       나는 앞의 둘만 고쳐서 두 번 헛걸음했다. 이제 셋이 같은 값(gr_257)이다.
  // ⛔ [2026-09-14] my-17(해물오일파스타)의 fe_451 은 «알리오 올리오»였다 — 새우도 조개도 없다.
  //    📮 창업자 = *"해물오일파스타는 새우들어간오일파스타"* — 전에 고친다고 해놓고 이 컷을 넣었던 자리다.
  //    ✅ 창업자가 새우·관자가 든 컷을 뽑아 줬다(n3028).
  'my-17': 'n3028', 'my-18': 'gr_257', 'my-19': 'fe_471', // 이름표 ↔ 그림 어긋남
}

const now = Date.now()
const state = {
  recipes: 옛판.map((r, i) => ({ ...r, status: 'sorted', savedAt: now - i * 60000 })),
  seedV: 90, // ⭐ v11.35 까지 받은 폰. 91 미만이라야 마이그레이션이 돈다
}
...
for (const r of 옛판) {
  ...
  const want = 바랄값[r.id]
  // ⛔ `my-12`(두루치기)는 «지킴»에서 뺐다 — 2026-08-29 창업자 카와이 판정으로 갈려야 한다(위 주석)
  // ⛔ 2026-08-31 — `my-8`(샐러드)·`my-13`(뚝배기파스타)도 «지킴»에서 뺐다.
  //    창업자 전수 판정에서 카와이로 잡혀 이제 «갈려야» 한다(위 기대값 주석). 라벨만 남기면 거꾸로 읽힌다.
  const 지킴 = ['my-5', 'my-6', 'my-11', 'my-14'].includes(r.id)
