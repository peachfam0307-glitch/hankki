...
// 📣📣 **선언 카드 = 창업자가 «직접 뽑아 왔다»** — 「한끼엔 요리모드도 타이머도 있다!」
//    📮 *"한끼에는 요리모드에 타이머도 있다!!! 한장 박고 시작할까"* → *"잠깐만 내가 하가 뽑아올게 선언카드"*
//    → *"**이걸 제일먼저넣고 타이머처리반 네가만든꼬르곰한장짜리 빼고 우리ui로넘어가**"*
// ⭐⭐ **순서가 이 카드 덕에 저절로 이어진다** — 카드 맨 아래에 **「다음 장에서 타이머 처리반 등장 →」**이
//    적혀 있어서 **선언 카드 → 처리반 시안**이 «말로» 연결된다. 내 임시 카드엔 없던 것이다.
// ⛔ 내가 만든 임시 카드(꼬르곰 한 장)는 **뺐다** — 창업자 판정.
// 🔢 이 카드는 **941×1672 = 정확히 9:16** 이라 여백 없이 꽉 찬다(실측).
// ⏱ 담긴 게 많다(제목·폰 화면·말풍선 셋·아래 칩 셋·다음 장 안내) → 1.8초로는 못 읽는다. **2.8초.**
const 선언카드 = join(ROOT, 'design/promo/병맛시리즈-창업자-2026-08-28/난리났습니다-8화/4화-0장-선언카드-요리모드도-타이머도.png')
const 카드초 = 2.8
...
// ── ffmpeg 한 방 ──
const 입력 = [
  '-loop', '1', '-t', String(인트로초), '-i', 시안,          // 0 처리반 시안(인트로)
  '-ss', 자르기, '-i', 원본,                                  // 1 앱 실사
  '-loop', '1', '-t', String(아웃트로초), '-i', 시안,        // 2 처리반 시안(아웃트로)
  '-loop', '1', '-t', String(카드초), '-i', 선언카드,        // 3 선언 카드 ⭐맨 앞에 온다
...
const 필터 = [
  `[0:v]${시안필터}[intro]`,
  `[2:v]${시안필터}[outro]`,
  // 📣 선언 카드는 «사이렌을 안 준다» — 여기부터 번쩍이면 정신없고, 이 장은 «읽는» 자리다.
  //    ⭐ 사이렌은 바로 다음 장(처리반)에서 시작되어 「등장!」이 더 세게 온다.
  ...
  // ⭐ 순서 = **선언 카드 → 처리반 시안 → 앱 → 처리반 시안**(창업자 확정)
  `[card][intro][app${자막들.length}][outro]concat=n=4:v=1:a=0[v]`,
].join(';')
