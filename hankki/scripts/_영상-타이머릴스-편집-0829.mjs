...
//    🔢 인트로 84프레임을 하나씩 견줘 보니 **프레임 간 차이 0** — 그림은 완전히 멎어 있었다.
...
//
// 📐 **레이아웃 = 앱이 화면을 꽉 채우고 말풍선이 «그 위»에 앉는다.**
//    ⛔⛔ 처음엔 위에 자막 띠를 두고 앱을 줄였는데 창업자가 접었다 —
//       📮 *"ui에 자막을 바로 넣어도 되지않아? 말풍선이나 알약이나."*
//    ⭐ 그게 맞다. 띠는 **무엇을 가리키는지 안 알려주고**, 띠만큼 앱이 작아진다.
//       말풍선은 «가리키는 것 바로 위»에 앉아 스스로 설명한다.
//    🔢 그래서 자막마다 `y`(앱 세로 비율)를 갖는다 — 뒤 셋은 **하단 타이머 바**를 가리킨다.
//
...
// 📣 선언 카드 — 📮창업자 *"한끼에는 요리모드에 타이머도 있다!!! 한장 박고 시작할까"*
const 카드초 = 1.8
...
// ── ffmpeg 한 방 ──
const 선언카드 = join(OUT, '자막/선언카드.png')
if (!existsSync(선언카드)) { console.error('✗ 선언카드.png 가 없다 — 자막판을 먼저 돌린다'); process.exit(1) }

const 입력 = [
  '-loop', '1', '-t', String(인트로초), '-i', 시안,          // 0 인트로
  '-ss', 자르기, '-i', 원본,                                  // 1 본편
  '-loop', '1', '-t', String(아웃트로초), '-i', 시안,        // 2 아웃트로
  '-loop', '1', '-t', String(카드초), '-i', 선언카드,        // 3 선언 카드
  ...자막들.flatMap((s) => ['-i', s.파일]),                   // 4.. 말풍선
]
...
const 필터 = [
  `[0:v]${시안필터}[intro]`,
  `[2:v]${시안필터}[outro]`,
  // 📣 선언 카드는 «사이렌을 안 준다» — 여기까지 번쩍이면 정신없고, 이 장은 «읽는» 자리다
  `[3:v]scale=${W}:${H},format=yuv420p,fps=${FPS},setsar=1[card]`,
  // ⭐ 앱은 «높이»를 맞춘다 — 폭을 맞추면 하단 타이머 바가 잘리는데 그게 이 릴스의 심장이다
  `[1:v]scale=-2:${H},pad=${W}:${H}:(ow-iw)/2:0:${배경색},format=yuv420p,fps=${FPS},setsar=1[app0]`,
  // 🫧 말풍선은 «앱 세로 비율»로 자리를 잡는다 — 앱이 화면을 꽉 채우므로 릴스 세로와 같다
  ...자막들.map((s, i) =>
    `[app${i}][${i + 4}:v]overlay=(W-w)/2:${Math.round(s.y * H)}:enable='between(t,${s.초},${s.끝})'[app${i + 1}]`),
  `[intro][card][app${자막들.length}][outro]concat=n=4:v=1:a=0[v]`,
].join(';')
