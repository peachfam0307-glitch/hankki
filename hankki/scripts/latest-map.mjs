...
export function byTopic() {
  ...
  //
  // ⛔⛔ **2026-08-10 — 그 고침이 이 컨테이너에선 «안 먹고 있었다».**
  //    ⑴ `mtime` 은 **체크아웃 시각**이라 파일이 전부 같은 초다(여기선 늘 동점) →
  //    ⑵ 그래서 파일명 역순이 결정하는데, 한글이 숫자보다 뒤라
  //       `…-08-09-온보딩열장`(그날 «첫» 판)이 `…-08-09-3-폴드패드`(그날 «마지막» 판)를 이겼다.
  //    📌 세션 브리핑이 **석 달 내내 틀린 「최신」을 들이밀 뻔했다** — 장치가 거짓말하면 규칙보다 나쁘다.
  //
  // ✅ **우리 이름 관례를 코드가 읽게 한다** — 번호 없음 = 그날 «첫» 판 · `-2`·`-3`·`-4` = 그 뒤 판.
  //    (실제로 그렇게 써 왔다: `…-08-05-표지미리캡처` < `…-08-05-2-…` < `…-08-05-3-…` < `…-08-05-4-…`)
  //    ⚠️ 날짜 «바로 뒤»에 붙은 숫자만 판(版)으로 본다 — 제목 속 숫자(`AAB13`·`낱개24`)에 걸리면 안 된다.
  for (const [, v] of t) {
    v.sort((a, b) => b.date.localeCompare(a.date) || (seq(b) - seq(a)) || (mtime(b) - mtime(a)) || b.path.localeCompare(a.path))
  }
  return t
}
// 같은 날 몇 번째 판인가 — `…-2026-08-09-3-폴드패드…` → 3 · 번호가 없으면 1(그날 첫 판)
function seq(d) {
  const m = d.path.match(/\d{4}-\d{2}-\d{2}-(\d{1,2})(?=[-.])/)
  return m ? Number(m[1]) : 1
}
// 파일 수정 시각(초). 못 읽으면 0 — 그럼 파일명으로 갈린다.
// ⚠️ 이 컨테이너에선 체크아웃 시각이라 대개 동점이다 — 위 `seq` 가 진짜 판정자다.
function mtime(d) {
