...
//   ✅ 그래서 옛 기록은 `docs/버전기록-전체.md` 로 옮겼다. ⛔한 글자도 안 지웠다 — 옮겼을 뿐이다.
...
//      급할 때 게이트를 꺼버리고, **꺼진 게이트는 없는 게이트다.** 5줄부터는 미리 알려만 준다.
{
  ...
  const 자 = cm.length
  let 보관 = true
  try { readFileSync(join(ROOT, 'docs/버전기록-전체.md'), 'utf8') } catch { 보관 = false }
  if (!보관) no('docs/버전기록-전체.md 가 없다 — 옛 기록을 옮겨 둔 곳이다. 지웠으면 되살릴 것')
  else if (옛 > 10) no(`CLAUDE.md 에 옛 기록이 ${옛}줄 쌓였다(${자}자) — docs/버전기록-전체.md «맨 위»로 옮길 것`)
  else if (옛 > 5) ok(`옛 기록 ${옛}줄 — 아직 괜찮지만 슬슬 docs/버전기록-전체.md 로 옮길 것 (${자}자)`)
