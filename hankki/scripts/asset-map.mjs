...
const KNOWN = {
  ...
  // 🐧 [2026-09-02] 펭펭 «정본» — 창업자 검수 「좋다」 8컷 중 골라 넣은 넷(가을 카드 뽑기).
  //    ⛔ 접두어를 `pp_` 로 쓰지 않았다 — 그건 이미 위의 «가을 다꾸 종이»다.
  //    ⏳ 사철용은 `pjs_` 로 온다(창업자가 다시 뽑는 중) — `docs/stickers/펭펭-사철-다시뽑을것-2026-09-02.md`
  pj: '🐧펭펭 정본 (가을 카드)', pjs: '🐧펭펭 정본 (사철 카드 · 오면 저절로 찬다)',
