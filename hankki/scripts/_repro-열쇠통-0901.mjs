...
console.log('  ── ⑥-c 기본 인식 세기 ──')
{
  ...
  잰다(앞글자(kv, 't:') === 1, '⑥c-2 ⭐기본 인식 수가 1 올라간다', '기본 ' + 앞글자(kv, 't:'))
  잰다(앞글자(kv, 'tr:') === 1, '⑥c-3 갈래별로도 센다')
  await 기본('고름'); await 기본('고름'); await 기본('실패')
  잰다(앞글자(kv, 't:') === 4, '⑥c-4 넷을 보내면 4', '기본 ' + 앞글자(kv, 't:'))
  잰다(kv._m.get('tr:' + Object.keys({}).length + '') === undefined, '⑥c-5 (자리 채움)')
  const 갈래별 = [...kv._m].filter(([k]) => k.startsWith('tr:')).map(([k, v]) => k.split(':').pop() + '=' + v).sort()
  ...
  잰다(앞글자(kv, 'tr:') === 5 && (kv._m.get('tr:' + [...kv._m.keys()].find((k) => k.endsWith(':실패')).split(':')[1] + ':실패') === '2'
    || [...kv._m].some(([k, v]) => k.endsWith(':실패') && v === '2')),
  ...
  잰다(b.status === 200 && 앞글자(kv2, 'tf:') === 1 && 앞글자(kv2, 't:') === 1,
    '⑥c-13 ⭐창업자가 쓴 것은 «따로» 표시된다(유저 몫이 오염되지 않게)', '창업자 ' + 앞글자(kv2, 'tf:'))
}
