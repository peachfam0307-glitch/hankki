...
// 📅📅 [2026-09-18] **월별 키와 일별 키를 갈라 센다.**
//   ⛔⛔ 그 전엔 `앞글자(kv,'t:')` 가 `t:2026-09`(월)와 `t:2026-09-18`(일)을 «둘 다» 세서
//      기본 인식을 한 번 보냈는데 2 가 나왔다. **판이 틀린 것이지 코드가 틀린 게 아니다**
//      (워커가 2026-09-18 부터 일별로도 세기 시작했다 — 창업자 *"기본인식 제대로 재는 판 만들어"*).
//   ⭐ 가르는 잣대 = **날짜 길이**. 월 = `2026-09`(7자) · 일 = `2026-09-18`(10자).
//   📌 이렇게 두면 **둘 다 제대로 오르는지**를 각각 잰다 — 한쪽만 올라도 잡힌다.
const 날짜부분 = (k) => k.split(':')[1] || ''
const 달칸 = (kv, 앞) => {
  let 합 = 0
  for (const [k, v] of kv._m) if (k.startsWith(앞) && 날짜부분(k).length === 7) 합 += parseInt(v, 10) || 0
  return 합
}
const 날칸 = (kv, 앞) => {
  let 합 = 0
  for (const [k, v] of kv._m) if (k.startsWith(앞) && 날짜부분(k).length === 10) 합 += parseInt(v, 10) || 0
  return 합
}
...
// ── ⑥-b 🪜🪜 «순서»를 끝까지 — 웰컴 → 그 달 5개 → 막힘 ──────────
//   📮 창업자 2026-09-01 = *"웰컴 다 씀 → 그 달 5개 → 그것도 다 씀 → 기본 인식. 이걸 확인할 수 있는 게 열쇠통이야?"*
//   ⛔⛔ **반만 재고 있었다** — 위 ⑥ 은 「웰컴을 다 쓴 사람」을 «심어놓고» 시작한다.
//      ⭐ 그래서 **웰컴에서 월 한도로 «넘어가는 순간»은 아무도 안 보고 있었다.**
//      📌 규칙 18 ⓘ — 통과했다고 그 자리를 잰 것은 아니다. 창업자가 물어서 열어보고 알았다.
//   ⭐ 여기서는 «심지 않는다». 진짜로 10장을 쓰고, 그 뒤 5장을 더 쓰고, 16번째에 막히나를 본다.
console.log('  ── ⑥-b 순서 끝까지 ──')
{
  const kv = mkKv()
  // ① 웰컴 10장 — 비로그인 상한 그대로
  for (let i = 0; i < 9; i++) await 담기(kv, { uid: 'w1' })
  const 아홉 = await 담기(kv, { uid: 'w1' })
  잰다(아홉.status === 200, '⑥b-1 웰컴 10장째까지는 «AI로» 읽힌다', '10장째 ' + 아홉.status)
  잰다(아홉.body.left.welcome === 0, '⑥b-2 열장을 쓰면 웰컴이 0 이 된다', '남은 ' + 아홉.body.left.welcome)
  잰다(앞글자(kv, 'm:') === 10, '⑥b-3 열 장 다 Vision 을 불렀다(전역 10)', '전역 ' + 앞글자(kv, 'm:'))
  // ② ⭐⭐ **웰컴을 «그 달에» 다 쓰면 그 달은 거기서 끝이다 — 5장이 «더» 붙지 않는다.**
  //   📮 창업자 확정 2026-08-13 = *"웰컴20장 다쓰면 **무료5장은 소진한거니까 기본인식으로**"*
  //      → 웰컴과 월 5장은 **한 통(pool)** 이다. 웰컴이 그 달 5장을 «대신한다».
  //   ⛔⛔ **내가 이 판을 처음 쓸 때 「5장이 더 된다」로 잘못 우겼고 서버가 아니라고 답했다**(2026-09-01).
  //      ⭐ 그래서 창업자에게도 틀린 순서를 말했다. **게이트가 내 말을 바로잡은 자리다.**
  //      📌 확정 기록은 CLAUDE.md 에 있었다 — 「없다」가 아니라 **내가 안 읽은 것**이었다.
  const 열한째 = await 담기(kv, { uid: 'w1' })
  잰다(열한째.status === 429 && 열한째.body.error === 'user_quota',
    '⑥b-4 ⭐⭐웰컴을 다 쓰면 «그 달은 바로» 기본 인식이다(5장이 더 안 붙는다)', '11장째 ' + 열한째.status)
  잰다(앞글자(kv, 'm:') === 10, '⑥b-5 ⭐⭐막힌 뒤로는 Vision 을 «안» 부른다 — 구글에 돈이 0 원 나간다', '전역 ' + 앞글자(kv, 'm:'))
  잰다(앞글자(kv, 'q:') >= 1, '⑥b-6 막힘이 세어진다(유료를 언제 켤지 정하는 숫자)', '막힘 ' + 앞글자(kv, 'q:'))
  // ③ ⭐ 「다음 달엔 5장이 열린다」는 위 ⑥ 이 잰다 — 웰컴 0 · 이번 달 0 에서 다섯 장이 통과한다.
  //    (달을 바꿔 잴 수는 없다 — 워커가 날짜를 스스로 만든다. 그 상태를 심어 재는 게 같은 값이다)
  // ④ 로그인한 사람은 그 순서가 «30 부터» 시작한다
  const kv2 = mkKv()
  for (let i = 0; i < 29; i++) await 담기(kv2, { uid: 'w2', sub: 'WSUB' })
  const 서른째 = await 담기(kv2, { uid: 'w2', sub: 'WSUB' })
  잰다(서른째.status === 200 && 서른째.body.left.welcome === 0,
    '⑥b-7 로그인한 사람은 «30장»까지 AI 로 읽힌다', '30장째 ' + 서른째.status + ' · 남은 ' + 서른째.body.left.welcome)
  const 서른하나 = await 담기(kv2, { uid: 'w2', sub: 'WSUB' })
  잰다(서른하나.status === 429, '⑥b-8 31장째부터 기본 인식이다', '31장째 ' + 서른하나.status)
}

// ── ⑥-c 📊 「기본 인식으로 읽었다」 세기 — 유료 켤 때 값·장수를 정하는 잣대 ──
//   📮 창업자 2026-09-01 = *"무료이용률도 알아야 가격이나 장수를 수정하니까"* · *"촘촘하게 설계해줘.. 꼭"*
//   ⭐ 다각도로 찌른다 — 세나 · 갈래가 갈리나 · 안 건드려야 할 것을 안 건드리나 · 이상한 값이 와도 안 깨지나
console.log('  ── ⑥-c 기본 인식 세기 ──')
{
  const kv = mkKv()
  // ⛔ 다른 칸들과 «같은 방식»으로 만든다 — 진짜 Request 에 진짜 헤더·몸통.
  //    첫 판이 진짜 Request 에 «가짜 headers.get» 을 섞어 넣어 몸통이 통째로 비었다(2026-09-01).
  const 기본틀 = (갈래, kvx, founder) => worker.fetch(new Request('https://hankki-ocr.example/', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json', 'x-hankki-token': APP_TOKEN, Origin: ORIGIN,
      'CF-Connecting-IP': 'ip' + (++ip번호), ...(founder ? { 'x-hankki-founder': 'FOUNDERKEY' } : {}),
    },
    body: JSON.stringify({ 기본: 갈래 }),
  }), ENV(kvx))
  const 기본 = async (갈래) => {
    const res = await 기본틀(갈래, kv, false)
    return { status: res.status, body: await res.json() }
  }
  const a = await 기본('막힘')
  잰다(a.status === 200 && a.body.ok === true, '⑥c-1 신호를 받는다')
  // 📅 [2026-09-18] 월·일을 «갈라» 잰다 — 둘 다 올라야 한다(한쪽만 올라도 여기서 잡힌다)
  잰다(달칸(kv, 't:') === 1 && 날칸(kv, 't:') === 1, '⑥c-2 ⭐기본 인식 수가 월·일 각각 1 올라간다',
    '달 ' + 달칸(kv, 't:') + ' · 날 ' + 날칸(kv, 't:'))
  잰다(달칸(kv, 'tr:') === 1 && 날칸(kv, 'tr:') === 1, '⑥c-3 갈래별로도 센다(월·일 각각)')
  await 기본('고름'); await 기본('고름'); await 기본('실패')
  잰다(달칸(kv, 't:') === 4 && 날칸(kv, 't:') === 4, '⑥c-4 넷을 보내면 월·일 각각 4',
    '달 ' + 달칸(kv, 't:') + ' · 날 ' + 날칸(kv, 't:'))
  잰다(kv._m.get('tr:' + Object.keys({}).length + '') === undefined, '⑥c-5 (자리 채움)')
  // 📅 월 키만 본다 — 일 키에도 같은 갈래가 한 벌 더 쌓여 있다(위 ⑥c-2·4 가 그걸 잰다)
  const 갈래별 = [...kv._m].filter(([k]) => k.startsWith('tr:') && 날짜부분(k).length === 7)
    .map(([k, v]) => k.split(':').pop() + '=' + v).sort()
  잰다(갈래별.join(',') === '고름=2,막힘=1,실패=1', '⑥c-6 ⭐갈래가 «갈려서» 쌓인다(처방이 갈리니까)', 갈래별.join(' '))
  // 🚨 제일 중요한 것 — 이 신호가 «돈 쓰는 통»을 건드리면 안 된다
  잰다(앞글자(kv, 'm:') === 0 && 앞글자(kv, 'd:') === 0, '⑥c-7 ⭐⭐전역 통(Vision)을 «안» 건드린다 — 돈이 0원')
  잰다(앞글자(kv, 'q:') === 0, '⑥c-8 ⛔「막힘」 카운터와 «따로» 센다(갈래 이름이 같아도 다른 통이다)')
  잰다(앞글자(kv, 'wu:') === 0, '⑥c-9 ⛔열쇠를 «안» 깎는다')
  잰다([...kv._m.keys()].every((k) => !/^earn:|^bo:/.test(k)), '⑥c-10 ⛔행동 열쇠를 «안» 준다')
  // 🕳 이상한 값이 와도 안 깨진다
  const bad = await 기본('아무거나')
  잰다(bad.status === 200, '⑥c-11 모르는 갈래가 와도 200 이다(신호는 곁일이라 유저를 막으면 안 된다)')
  // 📅 월 칸만 센다(일 칸에 같은 것이 한 벌 더 있다) · 갈래 이름은 넷을 넘지 않아야 한다
  const 갈래이름 = new Set([...kv._m.keys()].filter((k) => k.startsWith('tr:')).map((k) => k.split(':').pop()))
  잰다(달칸(kv, 'tr:') === 5 && 날칸(kv, 'tr:') === 5
    && [...갈래이름].every((n) => ['막힘', '고름', '실패', '없음'].includes(n))
    && [...kv._m].some(([k, v]) => k.endsWith(':실패') && 날짜부분(k).length === 7 && v === '2'),
    '⑥c-12 ⭐모르는 갈래는 «실패»로 떨어진다(모르는 통을 새로 만들지 않는다)',
    '달 ' + 달칸(kv, 'tr:') + ' · 날 ' + 날칸(kv, 'tr:') + ' · 이름 ' + [...갈래이름].join(','))
  // 👤 창업자 몫은 갈라 센다 — 안 그러면 「유저가 얼마나 쓰나」가 오염된다
  const kv2 = mkKv()
  await 기본('고름')                         // (kv 에 쌓임 — 아래는 새 통)
  const b = { status: (await 기본틀('고름', kv2, true)).status }
  // 📅 월·일 각각 1 이라야 한다 — 창업자 표시도 두 칸에 나란히 쌓인다
  잰다(b.status === 200 && 달칸(kv2, 'tf:') === 1 && 날칸(kv2, 'tf:') === 1
    && 달칸(kv2, 't:') === 1 && 날칸(kv2, 't:') === 1,
    '⑥c-13 ⭐창업자가 쓴 것은 «따로» 표시된다(유저 몫이 오염되지 않게)',
    '창업자 달 ' + 달칸(kv2, 'tf:') + ' · 날 ' + 날칸(kv2, 'tf:'))
}
...
// ── ⑩-b 🎁 로그인 선물 안내가 «두 화면 모두»에 있나 ─────────────────
//   📮 창업자 2026-09-01 = *"로그인화면에서 10개 주고 로그인하면 20개준다는 것도 안내붙였어?"*
//   ⛔⛔ 그 전엔 «첫 화면(CloudGate)»에만 있었다. 그런데 첫 화면은 «새로 깐 사람»만 본다
//      → 이미 쓰던 사람은 설정에서 열어도 선물 얘기를 한 글자도 못 봤다.
//      📌 「붙였다」와 «유저가 가는 길에 붙였다»는 다른 말이다 — 오늘 냉장고와 똑같은 모양이다.
console.log('  ── ⑩-b 로그인 선물 안내 ──')
{
  // ⛔⛔ 「줄 «머리»가 // 인가」로 거르면 «여러 줄 주석의 가운뎃줄»이 코드로 남는다.
  //    첫 판이 그래서 ⑩b-4 를 틀리게 죽였다(주석 안의 「30개」를 코드로 셌다 · 2026-09-01).
  //    ✅ 덩어리 주석({/* … */} · /* … */)을 «통째로» 걷어낸 뒤 한 줄 주석을 뺀다.
  const 코드 = (p) => fs.readFileSync(path.join(여기, p), 'utf8')
    .replace(/\{\/\*[\s\S]*?\*\/\}/g, '').replace(/\/\*[\s\S]*?\*\//g, '')
    .split('\n').filter((l) => !/^\s*\/\//.test(l)).join('\n')
  잰다(/<KeyGift\s*\/>/.test(코드('../src/components/CloudGate.jsx')), '⑩b-1 첫 화면(CloudGate)에 있다')
  잰다(/<KeyGift\s*\/>/.test(코드('../src/components/CloudSheet.jsx')), '⑩b-2 ⭐설정 시트(CloudSheet)에도 있다')
  const gift = fs.readFileSync(path.join(여기, '../src/components/KeyGift.jsx'), 'utf8')
  잰다(/무료열쇠상한\(\)/.test(gift), '⑩b-3 두 상한을 «서버가 준 값»에서 읽는다')
  잰다(/\b(10|20|30)\b/.test(코드('../src/components/KeyGift.jsx')) === false,
    '⑩b-4 ⛔숫자를 «글자로» 박지 않았다(상한을 바꿔도 문구가 안 낡는다)')
  잰다(/비로그인|\{더\}/.test(gift), '⑩b-5 ⭐「지금 몇 개 · 얼마나 더」도 적는다(창업자가 콕 집었다)')
}
