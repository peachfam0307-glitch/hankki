...
// 📊 8주 막대 누르면 그 주만 보이나 (창업자 2026-09-17 «누르면 자세히 보여??»)
const 막대 = p.locator('.fc-weeks button:not([disabled])')
console.log('누를 수 있는 막대 =', await 막대.count())
await 막대.first().click(); await p.waitForTimeout(600)
console.log('머리글 =', (await p.locator('.fc-k').first().textContent()).trim())
await p.screenshot({ path: join(OUT, '식비화면-4-막대누름.png'), fullPage: true })
