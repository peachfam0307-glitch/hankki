...
// ⭐⭐ [2026-09-10] 잣대를 «일부러» 넓혔다 — 유저 눈 스위치가 앞에 붙었다.
//    지금 모양 = 무제한: 유저눈인가() ? false : (!!v.무제한 || tidyFounder())
//    ⛔ 지키려는 것은 그대로다 — ①서버 값을 본다 ②폰 표식이 받친다. 유저 눈은 그 «앞»에 선다.
칸('getOcrLeft 가 무제한을 내준다', /!!v\.무제한 \|\| tidyFounder\(\)/.test(ocr))
칸('서버 답이 «아직 없을 때»도 무제한을 내준다(첫 화면)', /무제한: 유저눈인가\(\) \? false : tidyFounder\(\)/.test(ocr))
// ⭐ [2026-09-10 신설] 서버가 준 「무제한」을 «담는가» — 이걸 안 담으면 위 v.무제한 이 늘 비어
//    「서버가 말해준다」(창업자 확정 ⓐ)가 죽은 코드가 된다. 실제로 그랬다(적대적 검토가 잡았다).
칸('saveOcrLeft 가 서버의 무제한을 담는다', /무제한: left\.무제한 === true/.test(ocr))
