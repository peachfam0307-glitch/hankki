...
if (!있나) { 나쁨('상세 화면에서 「요리모드 시작」 버튼을 못 찾았다 — 아래 칸은 아무것도 못 잰다'); }
  ...
  if (잰값.없음) 나쁨(`${잰값.없음} 을 못 찾았다`)
    ...
    // ⭐⭐ [2026-09-10] 그라디언트를 «없앴다» — 잣대가 두 갈래를 다 읽어야 한다.
    //    옛 판: linear-gradient(to top, var(--bg) 92%, transparent) → 위 8% 가 비쳤다(창업자 재제보).
    //    지금:  단색 var(--bg) → 비치는 띠 0px. ⛔단색을 「못 읽었다」로 죽이면 «고친 쪽»이 실패한다.
    const 그라디언트 = /gradient/.test(잰값.배경)
    const 불투명 = 그라디언트 ? Number((잰값.배경.match(/(\d+)%/) || [])[1] || 0) : 100
    const 비치는띠 = Math.round(잰값.바높이 * (100 - 불투명) / 100)
    console.log(`  · 배경 = ${그라디언트 ? `그라디언트(불투명 지점 ${불투명}%)` : '단색(불투명)'} → **비치는 띠 ${비치는띠}px**`)
    if (겹친적 > 0) console.log(`  · (참고) 완성 칸이 바 뒤로 최대 ${겹친적}px 지나간다 — sticky 라 정상이다`)
    if (그라디언트 && !불투명) 나쁨('배경 그라디언트에서 불투명 지점을 못 읽었다 — 잣대가 헛돌았다')
