...
async function 장면(이름, 준비, 하기, { 씨뿌리기 = false } = {}) {
  ...
  const ctx = await b.newContext({
    ...
    //   ⛔⛔ **320 은 안 된다** — 📮 창업자 2026-09-18 = *"화면 좁히니까 글자 열이랑 다 깨져"*
    //      내가 「작은 폰 폭이라 안 깨진다」고 «해 보지도 않고» 적었다. 실제로는 깨졌다.
    //   ✅ 360 = 갤럭시 기본 폭이다 — 앱이 매일 실제로 그려지는 폭이라 «안 깨지는 게 확인된» 값.
    //      게다가 360 x 640 은 정확히 9:16 이다(0.5625). 390 보다 8% 크게 나온다.
    //   📌 더 키우려면 여기가 아니라 «글씨 자체»를 봐야 한다 — 폭을 더 줄이면 또 깨진다.
    //
    // 🚨🚨🚨 「화면이 작다」의 «진짜» 뿌리 — 📮 창업자 = *"왜 화면이 꽉차지 않고 4등분되어있어?"*
    //   ⛔⛔ **Playwright 은 페이지를 «키워서» 담지 않는다.** recordVideo.size 를 1080x1920 으로 줘도
    //      페이지는 «CSS 픽셀 그대로» 왼쪽 위 구석에 그려지고 나머지는 회색으로 남는다.
    //      (deviceScaleFactor 를 올려도 소용없다 — 영상은 CSS 픽셀로 담긴다)
    //      🔢 실측 = 틀 1080x1920 에 페이지 360x640 → **1/9 크기로 구석에 박혔다.** 눈으로 확인했다.
    //   🌲 그걸 모르고 나는 그 회색까지 통째로 늘려 왔다. 그래서 «첫 판부터 계속» 작았던 것이다.
    //      창업자가 세 번 말할 때까지 폭만 좁히며 엉뚱한 곳을 고쳤다(320 은 그래서 깨지기만 했다).
