...
// ⛔⛔ [2026-09-10] 여기 「basic-kongguksu」가 «박혀» 있었다 — 맨 앞이 늘 콩국수라고 여긴 것이다.
//    그날 목록 순서를 「최신순」으로 고치자 맨 앞이 애호박 국밥이 되며 이 판이 죽었다(앱은 멀쩡했다).
//    📌 재현판이 «남의 순서»에 기대면, 그 순서를 고치는 순간 거짓 빨간불이 뜬다.
//    ✅ ②에서 «실제로 연» 레시피의 쪽지에서 열쇠를 읽는다 — 무엇이 맨 앞이든 맞는다.
const 새열쇠 = (() => {
  const 쪽지 = (넣은뒤.find((x) => x.id !== 'old-1') || {}).쪽지 || ''
  return 쪽지.replace(/^idb:\/\//, '')
})()
