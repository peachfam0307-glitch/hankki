...
const 갈래 = [
  ...
  // ⛔⛔ ㉡「세로 레일」도 이제 «뺀다» — 창업자가 「ㄴ가자」로 골라 **진짜 앱에 들어갔다**.
  //    그런데 이 판은 레일을 «얹는» 판이라, 레일이 이미 있는 앱에 또 얹으니 스티커가 0칸이 됐다.
  //    📌 **판정이 끝난 갈래를 판에 남겨두면 「지금」과 겹쳐 거짓 그림이 된다.**
  // { id: '다-레일정리', 이름: '㉡ 세로 레일' },
  // 🆕 창업자 = 「아직도 여백이 너무 많아보여.」
  //    머리(76→42)를 깎고도 남는 여백이 어디인지 **재 보니 「칸 안」이었다** —
  //    한 칸이 52px 인데 그림이 31px 이라 **칸의 절반이 빈다.**
  //    ⛔ 그림 크기는 «종류별»로 창업자가 맞춰 온 값이라 내가 정하지 않는다(규칙 11) → 얹어서 보여준다.
  { id: '바-꽉채움', 이름: '㉣ 칸을 꽉 채움' },
  ...
  // ⛔ ㉢ 도 같은 이유로 뺀다(레일을 «안» 쓰는 갈래였다 — 창업자가 레일을 골랐다)
...
for (const W of 폭들) {
  ...
  for (const g of 갈래) {
    ...
    const m = await p.evaluate((어느) => {
      ...
      if (어느 === '바-꽉채움') {
        // ㉣ 칸(52px)은 그대로 두고 «그림만» 키운다 — 칸을 줄이면 손가락이 좁아지고 줄 수도 안 는다.
