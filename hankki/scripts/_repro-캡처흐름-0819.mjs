...
try {
  ...
  if (chooser) {
    ...
    try {
      ...
      // ✂️✂️ **[창업자 확정 2026-09-13] 사슬의 둘째 고리가 «바뀌었다» — 자르기를 «건너뛴다».**
      //   📮 창업자 = *"굳이 귀찮게 자를필요 없으니까"* · *"안내만 안뜨게 숨길수는 없어?"* → ⓑ 확정
      //   ⭐ 그래서 이제 물어야 할 것은 「자르기가 뜨나」가 아니라 **「자르기 «없이» 바로 읽기로 가나」**다.
      //      ⛔ 옛 잣대를 그대로 두면 «고쳐진 앱»을 고장이라 우긴다(게이트가 반대로 말한다).
      //   ⛔ 자르기 자체는 살아 있다 — 「재료 칸만 채우기」·「만드는 법 칸만 채우기」 길에선 그대로 뜬다.
      // ✂️✂️ **[창업자 확정 2026-09-15] 그 결정을 되돌렸다 — «언제나» 자른다.**
      //   📮 창업자 = *"자르기안내도 없어 어제 우리 문구까지 다 정했잖아"*
      //   ⛔⛔ 9/13 에 자르기를 건너뛰게 하는 바람에 9/14 에 만든 안내 두 갈래가 한 번도 못 떴다.
      //      글자는 있는데 그 화면이 안 열렸다 — 그래서 아무 검사도 못 잡았다.
      //   ⭐ 그러니 이제 물을 것은 **「자르기가 뜨고, 거기서 읽기로 이어지나」**다.
      //      ⛔ 「뜨나」만 재면 안 된다 — 뜨기만 하고 뒤가 막히면 그게 더 나쁘다(아래 ②-c 가 잇는다).
      const 자르기 = /자르기|잘라|남겨주세요/.test(글)
      if (자르기) ok('자르기 화면이 떴다 (창업자 확정 2026-09-15 · 안내가 여기 산다)')
      else no('⛔ 자르기 화면이 안 떴다 — 9/14 에 정한 안내 두 갈래가 또 숨는다')
      // ▶️ 자른 뒤로 이어 준다 — 사람이 「이 부분만 읽기」를 누르는 그 자리다.
      //    ⛔ 안 누르면 읽기가 시작 안 돼서 아래 칸들이 «앱 탓»으로 죽는다(판이 거짓말을 한다).
      try {
        await page.getByRole('button', { name: /이 부분만 읽기/ }).first().click({ timeout: 4000 })
        ok('「이 부분만 읽기」를 눌러 읽기로 이어졌다')
      } catch { no('⛔ 자르기 화면에서 「이 부분만 읽기」 단추를 못 찾았다 — 여기서 막히면 못 읽는다') }
  ...
  console.log('②-c 「읽는 중」 안내 — 진짜 로딩을 일으켜 잡는다')
  {
    ...
    // ✂️ [2026-09-13] 자르기를 건너뛰면서 「이 부분만 읽기」 단추가 «없어졌다» —
    //   사진을 고르는 순간 곧바로 읽기가 시작된다. 그래서 누를 것이 없다.
    //   ⛔ 옛 판은 그 단추를 못 찾아 「단추가 없다」로 죽었다 — 고쳐진 앱을 고장이라 우긴 것이다.
    //   ⭐ 물어야 할 것은 그대로다 = **기다리는 동안 화면이 조용하지 않은가.**
    {
      ...
      if (떴나) {
        ...
        if (r.안내y !== null && r.안내y < r.화면) ok(`읽는 동안 진행 표시가 보인다 (${r.안내y}px)`)
        else no('읽는 동안 진행 표시가 화면 밖이다')
      ...
      } else no('사진을 골랐는데 「읽는 중」 안내가 안 뜬다 — 기다리는 동안 화면이 조용하다')
    }
