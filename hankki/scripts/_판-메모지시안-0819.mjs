...
// ⛔ 아티팩트는 <!doctype>·<html>·<head>·<body> 를 «쓰지 않는다» — 발행할 때 감싸 준다.
//    ⭐ 그래서 <title> 과 <style> 로 시작한다.
...
<div class="note">⛔ 종이 «원래 비율»을 지켰어 — 늘리면 스프링·테이프가 뭉개지거든(첫 판이 그랬어).<br>
⭐ 그래서 <b>가로가 넓은 종이일수록 세로를 적게 먹어</b> — 요리 모드에서 재료 목록 위에 얹히는 거라 그게 유리해.</div>
</div>`
