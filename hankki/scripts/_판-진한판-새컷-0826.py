...
# ⭐ --옛컷 = 8/24 이후 시트에서 «안» 온 것 = 앱에 남아 있는 옛 세대
#    📮 창업자 2026-08-26 = "지금 앱에 올라간건 400컷이넘는다며, 지금 네가 보여준던 330정도잖아
#       나머지 100컷은 어떻게 된거야?" → 그 104컷이 이것이다.
옛모드 = '--옛컷' in sys.argv
컷 = []
for k in pk:
...
제목 = '앱에 남은 «옛 컷»' if 옛모드 else '8/24 이후 새 컷'
말머리 = '진한판-옛컷' if 옛모드 else '진한판-새컷'
print(f'{제목} 중 픽커에 실린 것 = {len(컷)}개')
...
for 쪽 in range(쪽수):
    ...
    d.text((여백, 20), f'한끼 · {제목} 전수검수   {쪽+1} / {쪽수}   ({len(컷)}컷)', font=Fh, fill=FG)
    ...
    f = f'{out}/{말머리}-{쪽+1:02d}.png'
    im.save(f); 만든.append(f)
    print(' ', f, f'{os.path.getsize(f)//1024}KB')
json.dump([{'갈래':a,'이름':b,'키':c} for a,b,c,_ in 컷],
          open(f'{앱}/scripts/_{말머리}-목록-0826.json','w',encoding='utf-8'), ensure_ascii=False, indent=1)
print(f'\n✅ {len(만든)}장')
