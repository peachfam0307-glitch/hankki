...
마무리로 통 들깨를 뿌려 완성합니다.`)
  ...
  // 🍚 [2026-08-29 갱신] 기대값 「완성해요」 → **「완성이에요」**
  //   ⭐ 「완성」은 이름씨라 「완성이에요」가 맞다. 그 전엔 앱 안에서 말이 «갈려» 있었다 —
  //      「완성합니다」→「완성해요」  ／  「…뿌리면 완성」→「완성이에요」 (같은 뜻인데 두 모양)
  //   ⛔ 이 줄을 「완성해요」로 되돌리지 말 것 — 되돌리면 그 갈림이 되살아난다.
  chk(has(r.steps, '완성이에요'), '「완성합니다」 → 「완성이에요」', r.steps.join(' / '))
