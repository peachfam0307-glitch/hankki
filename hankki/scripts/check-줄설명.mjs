...
// ═══════════════════════════════════════════════════════════════
// ② 🔢 **[2026-09-21 창업자가 또 잡았다] 「세 가지」라고 써놓고 두 편만 넣은 줄**
//
// 📮 창업자 = *"올라간거 두개가 다른데?"* → *"검수다했다면서 왜이래..."*
//
// ⛔⛔ **위 ① 이 못 잡았다.** ① 은 「덮밥·무침」 같은 «요리 이름 낱말»만 본다.
//    9/21 버섯 줄은 「끓이고 볶고 **부치는** 세 가지로」였는데 —
//    「볶고」·「부치는」은 «동사꼴»이라 요리말 목록(볶음·전)에 안 걸렸고,
//    **「세 가지」라는 개수말은 아무도 안 보고 있었다.**
//    🔢 실측 = 그 자리에서 **7개 주**가 같은 병이었다(고구마·대하·김장날 고기·항정살·묵은지·연말 파스타·버섯).
//    🌲 뿌리 = 2026-09-06 에 「홈은 2편만」으로 바꾸며 ids 를 셋→둘로 줄였는데 **why 는 안 따라왔다.**
//
// ⭐ 잣대 = 설명의 「한/두/세/네/다섯 가지」와 **ids 개수가 같아야 한다.** 숫자는 안 틀린다.
const 개수말 = { 한: 1, 두: 2, 세: 3, 네: 4, 다섯: 5, 여섯: 6 }
// ⛔ ③ 「동사꼴(부치는→전)」도 만들어 봤다가 «뺐다» — 헛잡는다.
//    🔢 실측 = 토마토샐러드(무치는 것) · 고구마크룽지(굽는 것) · 두루치기(볶는 것) 셋이 걸렸다.
//    🌲 우리말 요리 이름은 «조리법을 안 담는다» — 샐러드·크룽지·두루치기가 그렇다.
//    📌 시끄러운 게이트는 죽은 게이트다. 숫자(②)만 남긴다 — **숫자는 안 틀린다.**
const 개수틀림 = []
for (const m of 본문.matchAll(/why:\s*'([^']*)'[\s\S]{0,500}?ids:\s*\[([^\]]*)\]/g)) {
  const why = m[1]
  const ids = (m[2].match(/'([^']+)'/g) || []).map((s) => s.replace(/'/g, ''))
  if (!ids.length) continue
  const g = why.match(/(한|두|세|네|다섯|여섯)\s*가지/)
  if (g && 개수말[g[1]] !== ids.length) 개수틀림.push({ why, 샌말: g[1], 실제: ids.length })
}
if (개수틀림.length) {
  console.error('\n🔢 **줄 설명 게이트 — 글이 «개수»를 거짓말한다**\n')
  for (const x of 개수틀림) {
    console.error(`   ⛔ 글  = ${x.why}`)
    console.error(`      ⤷ 글은 「${x.샌말} 가지」인데 편은 **${x.실제}개**다\n`)
  }
  console.error('   📮 창업자 2026-09-21 = "올라간거 두개가 다른데?" · "검수다했다면서 왜이래..."')
  console.error('   ⛔ ids 를 줄이면 why 의 «개수말»도 같이 줄인다 — 유저는 글을 먼저 읽는다.\n')
  process.exit(1)
}
console.log('✅ 줄 설명 — 글과 실제 편이 맞는다 (이름·개수)')
