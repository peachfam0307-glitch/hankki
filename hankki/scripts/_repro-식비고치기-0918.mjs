...
//      ③적는 칸 안내가 「두부 1910」을 보여주나 ④값 안 적은 줄 글자가 「값」이 아니라 「＋ 금액」인가
...
await p.locator('input[placeholder*="두부"], input[placeholder*="살 재료"]').first().fill('대파')
await p.keyboard.press('Enter'); await p.waitForTimeout(500)
...
본다('고치기 시트가 «그 줄 값»을 물고 열린다', ((await p.locator('.fc-in').first().textContent()) || '').includes('1,910'))
