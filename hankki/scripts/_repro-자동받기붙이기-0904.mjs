...
//      합친 판은 양쪽을 다 가져서 덮어도 잃을 게 없다(절대원칙 34).
{
  ...
  // ⭐⭐ 여기가 이 칸의 심장 — 받은 «뒤»에 클라우드로 되올라갔나
  잰다(!!창고.get('users/구글번호1/recipes/u-pd'),
    '⑮-b ⬆️ ⭐ 패드에만 있던 편이 «클라우드로 올라갔다» (＝폰이 받을 수 있다)',
    창고.get('users/구글번호1/recipes/u-pd') ? '올라갔다' : '⛔안 올라갔다 — 한 방향만 흐른다')
  잰다(창고.get('users/구글번호1')?.기기 === '패드',
    '⑮-c 🏷 도장이 이 기기로 넘어왔다 (다음엔 상대가 받는다)',
    창고.get('users/구글번호1')?.기기 || '⛔안 바뀌었다')
  // ⛔ 받은 편이 되올리기에 «지워지지» 않았나 — 옛 백업을 올리면 이게 사라진다
  잰다(!!창고.get('users/구글번호1/recipes/u-ph'),
    '⑮-d ⛔ 방금 받은 편을 되올리기가 «안» 지웠다 (합친 판을 올린다)',
    창고.get('users/구글번호1/recipes/u-ph') ? '그대로 있다' : '⛔지워졌다')

  const 기록5 = await L.읽기()
  잰다((기록5[0]?.올린것 || 0) > 0, '⑮-e 📜 「주고받았다」가 기록에 남는다',
