...
// ⭐ [2026-09-08] 「제휴」 → 「쿠팡 파트너스」로 갈았다. 창업자 확정 문구가
//    *"쿠팡 파트너스 활동으로 일정액의 수수료를 받아요"* 라 「제휴」라는 낱말이 화면에서 사라졌다.
//    ⛔ 잣대를 «느슨하게» 한 게 아니라 **같은 사실을 더 정확히 재게** 한 것이다 —
//       「제휴」는 어느 제휴인지 안 밝히지만 「쿠팡 파트너스」는 누구에게 받는지까지 밝힌다.
const NOTICE = ['수수료', '쿠팡 파트너스']

// ⛔⛔ **조건부·불확정 표현 금지** — 공정위 「추천·보증 등에 관한 표시·광고 심사지침」
//    (2023-12-01 시행 개정판)이 *"소정의 수수료를 지급받을 수 있음"* 류를 **«부적절 예시»로 명시**했다.
//    📌 2026-09-08 까지 우리 화면이 「제휴 수수료를 **받아도** 값은 그대로예요」였다 —
//       제휴가 없던 때는 맞는 말이었지만, 받기 시작한 뒤에는 「가정」으로 읽혀 사실을 가린다.
//    ⭐ 사람이 기억할 일로 두지 않는다 — 조건부 표현이 다시 들어오면 여기서 배포가 막힌다.
const 조건부 = [/받아도/, /받을\s*수\s*있/, /받게\s*될/, /받을지도/, /지급받을\s*수/]
...
// ⛔⛔ **주석을 «통째로» 걷어낸다** — 줄머리만 보던 옛 방식은 JSX 주석(`{/* … */}`)의 가운뎃줄을
//    못 걸러서, **설명 주석에 적힌 「쿠팡 파트너스」가 검사를 통과시켜 버린다**(＝화면엔 고지가 없는데 초록불).
//    2026-09-08 에 이 파일 위 §되살릴 문구·아래 설명 주석이 실제로 그 낱말을 다 갖고 있었다.
const shopUi = stripComments(read('src/screens/ShopScreen.jsx'))
const hasNotice = NOTICE.every((w) => shopUi.includes(w))
const 걸린조건부 = 조건부.filter((re) => re.test(shopUi))

console.log(`  🔗 제휴 링크 ${hits.length}건 — ${hits.slice(0, 5).join(' · ')}`)
if (hasNotice && !걸린조건부.length) {
  console.log('  ok  제휴 고지 문구가 화면에 있다 (확정형)\n')
  process.exit(0)
}

if (hasNotice && 걸린조건부.length) {
  console.log(`\n  ✗ ⛔⛔ **고지가 「조건부 표현」이다 — 배포 차단** (${걸린조건부.map((r) => r.source).join(' · ')})`)
  console.log('     공정위 추천·보증 심사지침(2023-12-01 시행)이 「지급받을 수 있음」 류를 «부적절 예시»로 명시했다.')
  console.log('     👉 「받아도」·「받을 수 있어요」 → **「받아요」** 처럼 확정형으로 고칠 것.\n')
  process.exit(1)
}
