...
for (const 제목 of 유튜브편) {
  ...
  // ⛔ scrollIntoViewIfNeeded 가 제목만 걸치게 해서 카드 본문이 또 잘렸다(두 번째 실패).
  //    ✅ 그래서 «카드 자체»를 찾아 그 요소만 찍는다 — 화면 어디에 있든 확실하다.
  ...
  console.log(`  ⑤ ${제목} → ${줄}`)
  const 영상칸 = p.locator('a, button').filter({ hasText: '영상으로 보기' }).last()
  await 영상칸.screenshot({ path: `${OUT}/판0923/⑤-영상문구-${제목}.png` }).catch(async () => { await 찍다(`⑤-영상문구-${제목}`, 844) })
  await 뒤로()
}
