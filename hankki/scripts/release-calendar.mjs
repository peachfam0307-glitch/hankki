...
if (mode === '--month') {
  ...
  // ⛔⛔ [2026-09-02 고침] 이 줄이 *"날짜를 받는 일반판은 아직 «없다»"* 로 **낡아 있었다.**
  //    `_판-검수.mjs` 가 날짜를 인자로 받는다(머리주석 = 「쓰기: node scripts/_판-검수.mjs 2026-08-24」).
  //    ⭐ 낡은 안내는 «없는 일»을 만든다 — 다음 세션이 있는 도구를 못 보고 또 만든다(규칙 12ⓑ·17).
  //    📌 그래서 이름을 손으로 적지 말고 «있는 것»을 그대로 부르게 한다.
  console.log(`\n   👉 판 재료 = 위 날짜들 ${미검수.map((g) => g.date).join(' ')}`)
  console.log(`      👉 판 뽑기 = node scripts/_판-검수.mjs ${미검수.map((g) => g.date).join(' ')}`)
  process.exit(0)
}
