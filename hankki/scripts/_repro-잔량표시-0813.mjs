...
// 🔑 [2026-09-01] 열쇠 알약이 «부품»으로 빠졌다(KeyBadge.jsx) — 설정 화면도 같은 걸 쓰려고.
//   ⛔ 그래서 이 판이 «옛 파일만» 보면 문구를 못 찾고 죽는다 — 실제로 2026-09-01 에 2칸이 그렇게 죽었다.
//      ⭐ 게이트가 «맞게» 걸린 것이다(v11.30 에 게이트 넷이 이름을 못 찾아 죽은 것과 같은 자리).
...
// ⛔ [2026-09-01] 옛 잣대는 `aria-label={`무료 …` 처럼 «붙어 있는 모양»을 봤다.
...
chk('🔒⭐ 초록 박스가 「다 쓰면 기본 인식으로」를 말한다',
  imp.includes('를 다 쓰면 기본 인식으로'), 'true')
chk('🔒⭐ 그리고 「무료로 계속」까지 말한다 — ⛔「못 쓴다」로 읽히면 그 자리에서 나간다',
  imp.includes('무료로 계속'), 'true')
...
// ⛔ [2026-09-01] JSX 안에선 `5{KEY_UNIT}` 였는데 부품에선 템플릿 문자열이라 `5${KEY_UNIT}` 가 된다.
//    ✅ 둘 다 받는다 — 지키려는 것은 «달러 기호»가 아니라 「매달 몇 개인지 밝히나」다.
...
// ⛔ [2026-09-01] 두 파일을 이어 붙였으니 «자리»는 가져오기 화면 «안»에서만 재야 한다.
//    부품 파일이 뒤에 붙어서 `imp-key` 가 늘 뒤로 밀린다 — 그러면 이 칸이 영영 빨간불이 된다.
//    ✅ 이제 그 자리에 서는 것은 부품(`<KeyBadge />`)이다.
chk(
  '🔒⭐ 잔량이 «상단바»에 있다(제목보다 앞 · 본문 아니다)',
  impOnly.indexOf('<KeyBadge />') < impOnly.indexOf('가져오는 방법을 선택'),
  'true',
)
