...
const badge = (l) =>
  l.total > 0
    ...
    : '이번 달 무료 AI 스캔을 다 썼어요 · 기본 인식으로 계속 읽어 드려요 · 다음 달에 무료 5장 채워져요'
...
chk('🔒 화면이 「~남았어요」체를 쓴다(창업자 지시 · 「남음」 금지)', /장<\/span> 남았어요/.test(imp), 'true')
chk('🔒 화면에 「다 써도 기본 인식으로 계속 읽어 드려요」가 있다', imp.includes('다 써도 기본 인식으로 계속 읽어 드려요'), 'true')
chk('🔒 화면이 「다음 달에 무료 5장 채워져요」로 «장수를» 밝힌다', imp.includes('다음 달에 <b') && imp.includes('무료 5장'), 'true')
chk('🔒⭐ 웰컴 알약이 «줄바꿈»으로 두 줄이다(창업자 지시)', /20장이에요<br \/>/.test(imp), 'true')
