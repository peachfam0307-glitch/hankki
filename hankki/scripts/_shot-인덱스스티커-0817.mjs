...
// 🏷🏷 [2026-08-18 창업자] **인덱스는 «표시»다 — 누르는 자리가 아니다.**
//    📮 *"있자나.. **인덱스는 안눌려도 되지않아?**"* → *"**표시용도니까.** 인덱스해놓은 것 끼리 모아보려면
//       **대표인덱스버튼**을 만들거나.. 다른 방법으로 **그거 하나 누르면 같은 인덱스 붙인게 보이면** 되니까."*
//    ⭐⭐ 전제가 «바뀌었다». 그래서 아래 셋이 통째로 사라진다 —
//       ⑴ **손가락 칸 34px 제약** (누를 게 아니면 안 지켜도 된다 → 그림을 더 키울 수 있다)
//       ⑵ **「안 눌린 상태를 어떻게 그리나」** (안 걸린 카드엔 **아무것도 안 그린다**. 빈 책갈피가 필요 없다)
//       ⑶ 어제 걱정한 **오터치**
//    ⛔ 그러니 A~D 는 «옛 전제»로 만든 판이다. **표시용 전제로 다시 만든다.**
...
const 놓기 = [
  ...
  // ⭐ 표시용이라 «손가락 칸»에 안 갇힌다 → 더 큰 것도 후보가 된다. 이건 어제는 못 만들던 판이다.
  { 이름: 'E-크게', 설명: 'E · 크게 ＋ 반쯤 걸침 (40px · 표시용이라 가능)', 흰원: false, px: 40, 밖: 16, 기울기: 0 },
  { 이름: 'F-크게기울임', 설명: 'F · 크게 ＋ 반쯤 걸침 ＋ 기울임 (40px · −10°)', 흰원: false, px: 40, 밖: 16, 기울기: -10 },
]
...
// 🎬 찍을 판 = (놓는 방식 × 그림 묶음)
const 찍기 = [
  ['C-걸침', '살아남은것'], ['D-걸침기울임', '살아남은것'],
  ['E-크게', '살아남은것'], ['F-크게기울임', '살아남은것'],
  ['E-크게', '창업자픽'],
]
...
// ⭐⭐ [2026-08-18] **인덱스를 «섞어서» 심는다.** 어제 판은 6개 «전부» 붙어 있어서
//    「표시」로 보이질 않았다 — 진짜 목록은 **몇 개만** 붙는다. 그래야 눈에 띄는지가 보인다.
...
const state = {
  recipes: [
    ...
    R('dddd', '된장찌개', 'fe_133', false), R('eeeee', '김치찌개', 'fe_128', true), R('ffffff', '어묵탕', 'fh_k18', false),
    R('ggggggg', '두부조림', 'fe_66', true), R('hhhhhhhh', '무생채', 'fe_95', false), R('iiiiiiiii', '계란말이', 'fe_04', false),
  ],
