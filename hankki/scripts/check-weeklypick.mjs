...
//      **못 지킬 기준은 기준이 아니다**(③에서 이미 겪었다).
//
//   🔧🔧 **[2026-09-08 고침] 「몰 2곳 이상」을 «막는 조건»에서 «알리는 값»으로 내렸다.**
//      📮 창업자 = *"검사를 고쳐"* (그 앞 = *"아니 왜 너무비슷하면 안된다는거야?"*)
//      ⛔ 그날 사고 = 제품 «하나»(치밀 골드퀸 현미밥)를 쿠팡→오아시스로 바꿨더니
//         `pickRotate` 가 몰 축으로도 순서를 펴는 탓에 **26주 배치가 통째로 재계산**됐고,
//         2026-12-22 주가 「갈래4 · 몰1」이 되어 **배포가 막혔다**(run 34188585366).
//         👉 골드퀸을 아예 빼도 그 주는 그대로 실패했다 — **방아쇠였을 뿐 원인이 아니다.**
//
//      🔢🔢 **실측이 답을 줬다 — 이건 「못 지킬 기준」이었다**
//         · 쿠팡 비중 = 제품 127개 중 **67개(53%)** (컬리 20 · 한살림 14 · 자연드림 11 · 직접 7 · 네이버 5 · 오아시스 3)
//         · 네 개가 «전부 쿠팡»일 확률 ≈ 0.53⁴ = **7.7%** → **26주 기대값 2주**
//         · 그런데 잣대는 **0주**를 요구했다 = 제품을 하나만 손대도 언제든 또 막힌다(오늘이 그랬다)
//         · 같은 26주를 축별로 재보면 → **갈래 3 미만 = 0주 ✅ · 몰 2 미만 = 1주**
//
//      ⭐⭐ **가른 잣대 = 「우리가 정할 수 있나」**
//         · **갈래**(무슨 물건인가) = 우리가 «고른다» → 지킬 수 있다 → **막는다**
//         · **몰**(어디서 파나) = 그 회사가 정한다. 우리 손 밖이다 → **알리기만 한다**
//         📌 창업자가 8/28 에 말한 「섞어서」는 **물건이 섞이라는 뜻**이었다.
//            몰 조건은 내가 덧붙인 것이고, 그게 오늘 발목을 잡았다.
//
//      ⛔ **숫자를 넓히는 땜빵이 아니다**(절대원칙 34) — 「몰 2 → 1」로 «완화»한 게 아니라
//         **막을 것과 알릴 것을 갈랐다.** 실패의 «모양»이 바뀐다:
//         「통제 못 하는 것으로 배포를 막는다」 → 「통제 못 하는 건 «보여주고», 통제할 수 있는 것만 막는다」
//      ⚠️ 그래서 **몰 뭉침은 계속 «세어서 찍는다»** — 눈에서 사라지지 않는다.
//         숫자가 크게 늘면(예: 26주 중 5주 넘게) 그건 큐레이션이 한쪽으로 쏠렸다는 신호다.
...
// ⛔ 막는 것 = 갈래 뭉침 하나뿐 (＝창업자가 말한 「섞어서」)
const 갈래뭉침 = 주갈래.filter((w) => w.개 >= 3 && w.갈래 < 3)
if (갈래뭉침.length) no(`한 갈래로 뭉친 주가 ${갈래뭉침.length}개 — ${갈래뭉침.slice(0, 3).map((w) => `${w.ymd}(갈래${w.갈래})`).join(' ')}`)
else ok(`26주 내내 물건이 «섞여» 나온다 (갈래 최소 ${Math.min(...주갈래.map((w) => w.갈래))}가지)`)

// 📢 알리기만 하는 것 = 몰 뭉침 (우리 손 밖이라 안 막는다 · ⛔없애지 말 것 — 쏠림 신호다)
const 몰뭉침 = 주갈래.filter((w) => w.개 >= 3 && w.몰 < 2)
if (몰뭉침.length) {
  console.log(`  📢 한 몰로만 뭉친 주 ${몰뭉침.length}개 — ${몰뭉침.slice(0, 3).map((w) => `${w.ymd}(${w.몰}곳)`).join(' ')}`)
  console.log('     (쿠팡이 제품의 절반이라 26주에 2주쯤은 저절로 그렇게 된다 — 막지 않는다)')
}
