...
//   ⭐ 둘 다 «창업자 본인 시나리오»다. 여기서 안 재면 창업자 폰에서 데이터가 날아간다.
{
  ...
  // ⛔⛔ [2026-09-04] 여기가 «비동기인데 `잰다`» 였다 — 바로 위 주석이 경고하는 그 함정에
  //    이 판에서 딱 한 칸, 그것도 «제일 위험한 칸»이 빠져 있었다(빈 폰이 클라우드를 덮는 것).
  //    ⭐ 로컬에선 30/30 초록불이었는데 CI 가 죽어서 드러났다 — 던진 Promise 를 아무도 안 잡아
  //       ✅ 를 찍고 지나갔다가, 한참 뒤 «uncaught» 로 터진 것이다. ＝ 조용한 거짓 초록불.
  //    📌 초록불이 «무엇을 보고 있는지»를 봐야 한다(규칙 18 ⓘ).
  await 잰다비동기('⑱ 🚨 클라우드 것을 «안 가져왔으면» 안 올린다 (기변 — 빈 폰이 클라우드를 덮는 것을 막는다)', async () => {
