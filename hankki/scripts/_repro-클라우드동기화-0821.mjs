...
//      클라우드엔 사진이 안 올라가므로, 그냥 덮으면 폰에 있던 사진이 빈 채로 덮인다.
{
  ...
  잰다('㉕ 📷 폰에만 있던 «사진»이 안 사라진다 ⭐이 판의 심장 (자동으로 덮어도)', () => {
    ...
    assert.ok(받.살린사진 >= 2, '살린 사진 수가 이상하다: ' + 받.살린사진)
  })
  ...
  await 잰다비동기('㉗ 🚨 이 기기에도 «안 올린 변경»이 있으면 자동으로 «안» 받는다 (사람이 고른다)', async () => {
    await C.올리기({ ...백업(), diary: [], recipes: [꾸민것('폰이 또 고침', false), { id: 'r2', title: '된장국' }] }, { 기기: '폰A' })
    const 패드에서고친것 = { ...백업(), diary: [], recipes: [꾸민것('패드에서도 고쳤다', true), { id: 'r2', title: '된장국' }] }
    const r = await C.저절로받기(async () => 패드에서고친것)
    assert.equal(r.했나, false, '🚨 양쪽이 바뀌었는데 자동으로 덮었다')
    assert.equal(r.왜, '양쪽이바뀜', '왜 = ' + r.왜)
  })
