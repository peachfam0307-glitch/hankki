...
if (await 기록탭.count()) {
  ...
  // 🔢 2026-08-13 갱신 — v10.47 밤에 **꼬르곰·펭펭 재판 32컷을 일꾸 「기록」 탭 맨 위**에 넣었다
  //    → 8그룹 99컷 ＋ 2그룹 32컷 = **10그룹 131컷.** 옛 기대값(8·99)이 그대로라 이 판이 ⛔ 나고 있었다.
  //    📌 자산이 늘면 이 숫자도 같이 올린다 — «기록 탭이 살아 있나»를 보는 칸이지 개수를 얼리는 칸이 아니다.
  봄(일꾸.그룹.length === 10, `기록 탭에 10그룹 (지금 ${일꾸.그룹.length})`)
  봄(일꾸.총칸 === 131, `기록 탭에 131컷 (지금 ${일꾸.총칸})`)
