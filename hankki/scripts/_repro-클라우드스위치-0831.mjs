...
async function 설정에클라우드있나 (pg) {
  ...
  // 🔐 **[2026-09-08] 카드 «이름»이 로그인 상태로 갈린다** — 비로그인 = 「로그인하러 가기」 · 로그인 = 「클라우드 저장」
  //   ⛔ 옛 검사는 「클라우드 저장」 글자만 찾아서, 이름을 가른 v12.74 에서 배포를 막았다(게이트가 «맞게» 걸린 것).
  //   ⭐ 이 검사가 재는 건 «이름»이 아니라 **「설정에 그 입구가 있나」** 다 → 둘 중 하나면 통과다.
  //      📌 이름을 또 바꾸면 여기도 같이 고친다(같은 잣대는 같은 말이라야 한다).
  return { 도착, 있나: /클라우드 저장|로그인하러 가기/.test(글) }
}
...
}
{
  ...
  칸(s.도착 && s.있나, '③ 설정에 클라우드 입구가 있다(이름은 로그인 상태로 갈린다)')
  await ctx.close()
}
