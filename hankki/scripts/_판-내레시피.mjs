...
// ──────────────────────────────────────────────────────────────────
// 📄 재고표 «문서»도 같이 쓴다 — ⭐**손으로 관리하면 반드시 낡는다**
//
// ⛔⛔ [2026-08-19 신설] 창업자 = *"낡은문서 다 고치고"*
//    재고표 `.md` 가 **「앱에 넣음 40편」으로 낡아 있었다**(진짜는 55편).
//    2026-08-18 에 이 도구로 판(HTML)만 만들고 **문서는 손으로 적어서** 그 뒤 16편이 반영이 안 됐다.
//    📌 **판과 문서가 갈리면 문서가 진다** — 다음 사람은 문서를 읽는다.
// ✅ 이제 이 도구를 돌리면 **문서도 같이 최신이 된다.** 손으로 고칠 일이 없다.
// ⛔ 레시피 «내용»은 안 쓴다 — 저장소가 공개(public)라 **제목·줄수·갈래만**.
// ⛔ 날짜는 «한 곳»에서만 만든다 — 절대원칙 27 (`check-kst` 게이트가 막는다)
const { todayKST } = await import('../src/today.js')
const 오늘 = todayKST()
const 표줄 = (x) => {
  const 꼬리 = x.들어감
    ? (x.들어감.앱이름 !== x.t ? `앱에선 「${x.들어감.앱이름}」` : '그 이름 그대로') +
      (x.들어감.from ? ` · ${x.들어감.from} 열림` : '')
    : (x.새것 ? '🆕 8/10 뒤에 쓴 것' : '')
  return `| ${x.열쇠} | ${재료(x.r)} | ${순서(x.r)} | ${꼬리} |`
}
const 절만들기 = (제목, 설명, 고름) => {
  const 것 = 편들.filter(고름)
  return `\n## ${제목} — ${것.length}편\n\n> ${설명}\n\n| 이름 | 재료 | 순서 | |\n|---|---:|---:|---|\n${것.map(표줄).join('\n')}\n`
}

const md = `# 📮 창업자 레시피 재고표 — ${편들.length}편 (${있는것[있는것.length - 1].replace('.json', '')} 백업 기준)
...
| | 편 | 무슨 뜻 |
|---|---:|---|
| ✅ **앱에 넣음** | **${센다((x) => x.들어감)}** | \`basics.js\` 에 \`origin: '창업자'\` 이름표가 붙어 있다 |
| 🅰 **바로 넣을 수 있음** | **${센다((x) => !x.들어감 && x.k === 'A')}** | 만드는 법 3걸음 이상 — 내가 지어낼 게 없다 |
| 🅱 **순서가 1~2걸음** | **${센다((x) => !x.들어감 && x.k === 'B')}** | 내가 풀어 쓰고 창업자가 검수 |
| 🅲 **재료만 있음** | **${센다((x) => !x.들어감 && x.k === 'C')}** | ⛔**만드는 법은 창업자가 쓴다** — 내가 지어내면 창업자 요리가 아니다 |

⚠️ 백업 안에 **같은 제목이 여럿**인 것은 뒤에 (1)(2)를 붙여 갈랐다.
🆕 = 그 앞 백업엔 없고 이번 백업에 새로 생긴 것
${절만들기('✅ 앱에 넣음', '이름을 바꿔 넣은 편은 바뀐 이름과 열리는 날짜를 같이 적었다.', (x) => x.들어감)}
${절만들기('🅰 바로 넣을 수 있음', '만드는 법이 3걸음 넘게 있다. 그대로 옮기면 된다.', (x) => !x.들어감 && x.k === 'A')}
${절만들기('🅱 순서가 1~2걸음', '내가 풀어 써야 한다 — 내가 쓴 걸음은 창업자 검수 대상이다.', (x) => !x.들어감 && x.k === 'B')}
${절만들기('🅲 재료만 있음', '⛔ 만드는 법을 내가 지어내지 않는다. 창업자가 쓴 뒤에 넣는다.', (x) => !x.들어감 && x.k === 'C')}`

const 문서 = new URL('../docs/내레시피-재고표-2026-08-18.md', import.meta.url)
writeFileSync(문서, md)
console.log(`📄 재고표 문서도 갱신 — docs/내레시피-재고표-2026-08-18.md`)
