...
// §4.5 💰💰 식비 깔때기 (2026-09-19 · 창업자 *"우리 식비탭도 재는거 넣어야하지않아?"*)
//
// ⛔ 왜 따로 절을 내나 — §4 는 이벤트를 «줄줄이» 늘어놓을 뿐이라 «열었는데 안 적은 사람»이 안 보인다.
//    식비는 어제(9/19) 나간 새 기능이라 우리가 알아야 할 게 「몇 명이 썼나」가 아니라 «어디서 멈추나»다.
// ⛔ 값이 하나도 없으면 절을 «안» 만든다 — 빈 표를 남기면 다음 사람이 「0이다」로 읽는다.
