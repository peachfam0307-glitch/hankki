...
let 심은날 = {}
try {
  const 출력 = execSync(`node hankki/scripts/계측-잣대.mjs --부터 ${달}-01 --까지 ${마지막} --전부`,
    { cwd: git뿌리, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] })
  for (const l of 출력.split('\n')) {
    const m = l.match(/^\s{3}(\S+)\s+심은 날 (\d{4}-\d{2}-\d{2}) (\d{2}:\d{2})/)
    if (m) 심은날[m[1]] = `${m[2]} ${m[3]}`
  }
} catch { 죽는다('계측-잣대.mjs --전부 를 못 돌렸다 — 심은 날을 모르면 0 과 「안 쟀다」를 못 가른다') }
...
const 있었나 = (이름, 날, 값있나) => {
  ...
  if (심 > 날) {
    if (값있나) { 의심.push(`\`${이름}\` — ${날} 에 값이 있는데 잣대는 심은 날을 ${s} 로 읽었다`); return 'doubt' }
    return false
  }
...
P(`> ⬜ = **그때 그 눈이 없었다** (⛔0 이 아니다) · 🌱 = 그날 새로 심은 눈 · ⚠️ = 값은 있는데 심은 날이 더 늦다(도구를 의심한다)`)
P()
const 의심자리 = L.length
P()

// §1 계측 연표
P(`## §1 🌱 계측 연표 — «언제 무슨 눈이 생겼나»`)
P()
...
P()
P(`🔢 **모두 ${Object.keys(심은날).length}가지** · 제일 오래 잰 것도 **${Math.round((Date.parse(마지막) - Date.parse(Object.keys(묶음).sort()[0].slice(0, 10))) / 86400000)}일**뿐이다.`)
P()

// §2 누적 — 사람
P(`## §2 🔢 날마다 (누적 · 아래로 쌓인다)`)
P()
P(`| 날짜 | 활성 | 새 사용자 | 재방문 | 로그인 누적 | 평균 참여 | 조회 | 이벤트 | 캡처 |`)
P(`|---|---:|---:|---:|---:|---:|---:|---:|---|`)
for (const d of 날들) {
  const v = 장부.날[d], o = v.개요 || {}
  const 로 = v.로그인누적 ? `**${v.로그인누적.진짜}** (${v.로그인누적.원값}−${v.로그인누적.뺀수})` : '모름'
  P(`| ${d.slice(5)} | ${N(o.활성)} | ${N(o.새)} | ${N(o.재방문)} | ${로} | ${초(o.참여초)} | ${N(o.조회)} | ${N(o.이벤트)} | ${v.미완사유 ? '⚠️미완' : '✅'} |`)
}
P()
...
P(`## §4 🖐 흐름 이벤트 «한 사람 수» — ⬜는 그때 눈이 없던 것`)
P()
...
for (const d of 날들) {
  ...
  if (새눈.length) P(`🌱 **이날 새로 생긴 눈 ${새눈.length}개** = ${새눈.sort().map((x) => `\`${x}\``).join(' · ')} — ⛔이날 값은 «하루치가 아니다».`)
...
if (의심.length) {
  const 줄 = ['> ⚠️⚠️ **잣대의 「심은 날」을 의심할 곳 ' + [...new Set(의심)].length + '곳** — 값이 있는데 심은 날이 더 늦다.',
    '> 　 코드를 다시 쓰면 `git log -S` 가 «다시 쓴 날»을 잡는다. **실물이 이긴다 — 값을 지우지 않는다.**']
  for (const m of [...new Set(의심)].slice(0, 8)) 줄.push('> 　 · ' + m)
  L.splice(의심자리, 0, ...줄)
}
const 글 = L.join('\n') + '\n'
if (a.includes('--쓰기')) {
