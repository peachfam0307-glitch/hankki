...
// ⭐⭐ [창업자 2026-09-22 *"앞으로 보고에 앱 처음 켠 사람 먼저 놔줘"*] **앱 안 숫자를 «왼쪽»에 놓는다.**
//    ⛔ 왜 = 9/20·9/22 두 번 다 같은 오해가 났다 — *"로그인을 10명이 했는데 새 사용자가 78은 뻥튀기 아니야?"*
//       78 은 「한끼 받기」 안내 페이지만 보고 간 62명이 섞인 값이고, 앱을 처음 켠 사람은 20명이었다.
//       내 보고가 78 을 «맨 앞»에 세워서 그 오해를 만들었다. 자리를 바꿔 뿌리를 없앤다.
//    ⛔ 안내 페이지 숫자를 지우지는 않는다 — 광고가 일하는지 재는 자리다. 오른쪽으로 민다.
P(`| 날짜 | ⭐앱을<br>처음 켬 | ⭐앱 홈을<br>연 사람 | ⭐로그인·가입<br>한 사람 | 로그인 누적 | 평균 참여 | 활성<br>(안내 페이지 포함) | 새 사용자<br>(안내 페이지 포함) | 조회 | 캡처 |`)
P(`|---|---:|---:|---:|---:|---:|---:|---:|---:|---|`)
for (const d of 날들) {
  const v = 장부.날[d], o = v.개요 || {}
  const 로 = v.로그인누적 ? `**${v.로그인누적.진짜}** (${v.로그인누적.원값}−${v.로그인누적.뺀수})` : '모름'
  P(`| ${d.slice(5)} | **${N(앱첫(v))}** | **${N(앱홈(v))}** | **${N(로그인한사람(v))}** | ${로} | ${초(o.참여초)} | ${N(o.활성)} | ${N(o.새)} | ${N(o.조회)} | ${v.미완사유 ? '⚠️미완' : '✅'} |`)
}
P()
P(`> ⛔⛔ **「활성·새 사용자」와 「로그인」을 나란히 놓고 보지 말 것** — 앞의 둘에는 «앱에 안 들어온 사람»(안내 페이지만 보고 간 사람)이 섞인다.`)
P(`> 　 그 페이지엔 로그인 화면이 아예 없다. **견줄 자리는 「앱을 처음 켬」 대 「로그인·가입」이다.**`)
P()
