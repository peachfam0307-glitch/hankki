...
// ⛔⛔ [2026-09-04] 그 전엔 «같은 줄»에 스위치가 있나만 봤다. 순서를 「받기 → 얹기 → 올리기」로
//    바꾸면서 스위치가 «감싸는 if» 로 올라갔고, 그러자 이 칸이 «고쳐진 코드»를 빨간불로 읽었다.
//    ✅ 그래서 «줄 모양»이 아니라 **«스위치 안쪽에서 부르나»**를 잰다 — 모양이 바뀌어도 안 낡는다.
const 게이트자리 = 코드줄.findIndex((줄) => /if \(자동받기켤까\(\)\)/.test(줄))
const 받기자리 = 코드줄.findIndex((줄) => /await 저절로받기\(/.test(줄))
잰다(
  게이트자리 >= 0 && 받기자리 > 게이트자리 && 받기줄.length === 1,
  '⑤-b 자동 받기는 «그 스위치를 지나야만» 돈다',
  게이트자리 < 0 ? '⛔스위치로 감싸지 않았다' : '스위치 ' + 게이트자리 + '줄 → 받기 ' + 받기자리 + '줄'
)
