...
if (앨범삭제.못함) {
  ...
  // ⏳⏳ **여기는 「고장」이 아니라 «아직 안 정한 것»이다** — 그래서 ⛔ 로 세지 않는다.
  //   ✅ 제보 ⑩의 진짜 버그(기록을 지워도 횟수가 «안 줄던» 것)는 위 칸에서 고쳐졌다.
  //   ❓ 남은 것 = **문턱**. `HomeScreen:157` 이 `cooked > 0` 이라 **한 번만 해먹어도** 「자주 해먹는 요리」에 뜬다.
  //      3번 해먹고 2번을 지워 1번이 남았는데 이름은 여전히 「자주」다.
  //      → **몇 번부터 «자주»로 칠지는 창업자가 정한다**(규칙 11 · 미감·말맛 판정).
  //   ⛔ 문턱을 내 맘대로 2로 올리면 **이미 1번씩 해먹은 사람들 목록이 통째로 빈다.**
  console.log(`⏳ ⑩-2 「자주 해먹는 요리」 문턱 — 처음 ${자주처음} → 하나 지운 뒤(cooked 1) ${자주뒤}` +
    `\n     └ 창업자 판정 대기: 몇 번부터 «자주»인가? (지금은 1번부터)`)
}
