...
// ②-b ⭐ **반 화면쯤에서 뜨나** — 창업자 폰 제보 2026-08-10 *"아 한참내려야 보이네"*
//    문턱이 `clientHeight × 0.9`(폰 802px) 이라 카드 두 줄을 지나야 겨우 떴다.
//    ⛔ 이 칸이 없으면 **문턱이 다시 높아져도 아무도 모른다** — ③(1400px)은 어느 문턱에서든 통과한다.
await 굴리기(470)
칸(await 있나(), '②-b 반 화면쯤 내려오면 뜬다(창업자 제보 자리)', `470px`)
