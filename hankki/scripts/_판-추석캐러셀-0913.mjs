...
장.push({ 이름: '3-전', html: `
  ...
  <div style="position:absolute;left:70px;right:70px;top:390px;display:grid;grid-template-columns:1fr 1fr;gap:28px">
    ${전.map((g) => `
  ...
  ${곰펭('cs_b11', 250, 'right:50px;top:120px')}` })
...
장.push({ 이름: '4-나물', html: `
  ...
  ${나물.map((g, i) => `
    <div style="position:absolute;left:80px;right:80px;top:${390 + i * 450}px;height:400px;display:flex;align-items:center;gap:40px;background:#fff;border:3px solid #efe2cf;border-radius:44px;padding:0 50px">
      ${컷(g.그림, 290)}
  ...
  ${곰펭('cs_b08', 250, 'right:50px;top:120px')}` })
