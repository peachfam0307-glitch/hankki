...
// 📅 주 시작 요일 — 주별 회색 줄 → 달력에서 토요일(9/19 · 오늘 기준 가장 가까운 토요일) 고르기
await p.screenshot({ path: join(밖, '주시작-1-기본.png') })
await p.locator('.fc-mstart').click(); await p.waitForTimeout(400)
const 토요일 = (() => { const d = new Date(todayKST() + 'T00:00:00Z'); d.setUTCDate(d.getUTCDate() - ((d.getUTCDay() + 1) % 7)); return d.toISOString().slice(0, 10) })()
await p.locator('.fc-mstart-in input[type=date]').fill(토요일); await p.waitForTimeout(300)
await p.screenshot({ path: join(밖, '주시작-2-고르기.png') })
await p.locator('.fc-ask-btns button.danger').click(); await p.waitForTimeout(700)
await p.screenshot({ path: join(밖, '주시작-3-토요일부터.png') })
console.log('주 저장 =', await p.evaluate(() => JSON.parse(localStorage.getItem('hankki:v1')).foodBudget))
