...
// ⛔⛔ [2026-09-13 고침] 「참치액」을 여기서 «통째로» 막으면 안 된다 — 창업자가 참치액 제품 링크를
//    직접 가져왔다(모스스토리). 이 검사가 보려던 건 「참치액에 «참치» 링크가 붙는 것」이지
//    「참치액에 참치액 링크가 붙는 것」이 아니다. 후자는 우리가 바라던 바다.
//    ⭐ 그래서 잣대를 «좁혔다» — 그 재료의 «제 링크»면 통과, 다른 재료 것이면 실패.
//    ⛔ 느슨하게 푼 게 아니다: 노두유·간장게장은 제 링크가 «없어서» 그대로 다 막힌다.
const 제링크 = { 참치액: 'mosstory' }
for (const [이름, v] of [['노두유', 답.노두유], ['간장게장', 답.간장게장], ['참치액', 답.참치액]]) {
  if (!v) continue
  const 제것 = 제링크[이름] && String(v).includes(제링크[이름])
  if (제것) continue
  탈.push(`⛔⛔ 「${이름}」 에 링크가 붙었다 — 낱말 «한가운데»에서 걸린 것이다 → ${String(v).slice(0, 60)}\n   📌 2026-08-03: 「노두유(중국 진간장)」가 「두유」로 걸렸다. 안 붙는 것보다 «엉뚱한 게 붙는 것»이 나쁘다.`)
}
