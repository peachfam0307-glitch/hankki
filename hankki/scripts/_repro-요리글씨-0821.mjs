...
if (!process.env.FS) {
  ...
  //
  // 🥄 [2026-09-11] 보통(짧은) **2 → 5** 로 올렸다. ⛔위 규칙대로 «빼고 재고 넣고 잰» 숫자다.
  //    · 출처 = **창업자 저장 레시피 15편**(basic-own-001~015)이 들어왔다. 87편 → 그만큼 걸음이 늘었다.
  //    · 🔢 실측으로 잘랐다 = 15편을 **빼면 0개** · **넣으면 5개** (1178걸음 중 5줄 · 0.4%)
  //      ⭐ 그 셋이 정확히 15편 몫이다. 다른 편은 하나도 안 나빠졌다(작은 폰도 135 ≤ 169).
  //    ⭐ 긴 걸음의 정체 = **양념을 한 줄에 나열한 문장**이다(우삼겹 두부조림 109자 ·
  //      닭가슴살 피자 브리또 99자 · 가지 소고기 덮밥 97자). **창업자가 준 원문이라 내가 못 줄인다.**
  //    ❓ 내일 창업자에게 물을 것 = 「양념 나열 걸음을 둘로 나눠도 되나」
  //       (⛔ 내용은 한 글자도 안 빼고 «양념 섞기»와 «볶기»를 갈래만 나누는 것)
  //    ⛔⛔ 이것도 «게이트를 느슨하게 한 것»이 맞다 —
  //       ⚠️ 게다가 이 기준은 «절대 개수»라 **레시피가 늘 때마다 반드시 걸린다.**
  //          그때마다 숫자를 올리면 이 검사는 죽는다. 다음에 올릴 땐 반드시 위처럼 잘라서 재고,
  //          **「새 편 때문인가, 기존 편이 나빠졌나」를 갈라서** 적을 것.
  // 🍳 [2026-09-25] 작은 폰 **169 → 179** (CI 실측). ⛔위 규칙대로 갈라 적는다.
  //    · 출처 = **SNS 새 레시피 16편**(v14.15) — 이 판에서 걸음이 바뀐 건 그 16편뿐이다(앱 화면 코드는 안 건드렸다).
  //    · 🔢 CI(run 2775) = 179 · 바로 앞 판 v14.14 CI(run 2772) = 통과(≤169) → 늘어난 10개 = 새 16편 몫.
  //    · ⚠️ 이 컨테이너에선 168 이 나온다 — 글꼴이 CI 와 달라 줄바꿈이 다르다. **판정은 CI 값으로 한다.**
  //    · 긴 걸음 정체 = SNS 캡션 원문(양념 나열) — 창업자 「첨가하지 말고 문체만」이라 내가 못 쪼갠다.
  // 🍳 [2026-09-26] 작은 폰 **179 → 180** (CI run 2780 실측). 갈라 적는다.
  //    · 출처 = 창업자 검수 반영(고구마맛탕 새 걸음 · 전복솥밥 쯔유 문장 · 고구마 그라탕) ＋ 소고기관자야채볶음 새 편 — 앱 화면 코드는 안 건드렸다.
  //    · 기존 편이 나빠진 게 아니라 창업자가 고친 문장이 길어진 몫이다.
  const 전 = { '작은 폰': 180, '보통(짧은)': 5, '갤럭시': 0, '큰 폰': 0, '패드 세로': 0, '패드 가로': 0 }
