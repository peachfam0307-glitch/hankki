...
// 05 요리모드 — 📮 *"요리모드+타이머열린 것 둘다"* → 곁 폰에 걸음, 주인공 폰에 타이머 시트
const 장05 = () => `<style>${공통}
.back{top:600px;width:360px;height:700px}
.steps{top:1380px}</style>
${머리('불 앞에서도<br>편하게', '큰 글씨 걸음 · 화면도 안 꺼져요')}
<div class="back"><img src="${앱('25-요리모드-걸음')}"></div>
<div class="front"><img src="${앱('25b-요리모드-타이머')}"></div>
...
const 장들 = {
  ...
  'v8-05-요리모드': 장05,
  'v8-06-일꾸': () => 장({ 머리: '오늘의 한 끼가<br>일기가 돼요', 부제: '속지 고르고 · 사진 한 장 · 한 줄 · 스티커까지', 파일: '26b-일꾸', 곰: 'gp_pengym',
