...
// ⛔ 창업자 = *"재로칸이 만드는법이 섞여왔다면.. 이 안내도 정신이 없어"* — 통째로 뺐다.
//    «잘못 읽혔을 때»의 안내인데 그 일이 나기도 전에 읽게 했다. 각 칸의 📷 단추가 그때 있다.
chk('⑫ ⛔「섞여 들어왔다면」 안내가 사라졌다', !t편집.includes('섞여 들어왔다면') && !t편집.includes('재료·순서가 섞이면'))
