...
// ══ ⑧ 📢 「AI 가 더 다듬는 중」이라고 «먼저» 말하나 (창업자 지적 2026-09-01) ═══════════
// 📮 창업자 = *"ai로 읽으니까 시간이 좀 걸리거든 … 유저들이 이게뭐야 할 것 같아.
//    너무 오래걸리니까 **이상하게 읽힌거 혹은 안읽힌거 잘못된 걸로 오해**할수도있어"*
//    ＋ *"**인스타로 유입 될 것 같거든 첫인상이니까**"*
// ⭐⭐ 심장 = **문이 «둘 다» 말하나.** 인스타 공유받기(App.jsx)가 첫인상이라 여기가 더 중요하다.
//    ⛔ 한 문만 고치면 「제일 많이 쓰는 길」만 조용한 채로 남는다(v11.30 「열쇠 이름」과 같은 사고).
// ⛔ 「어느 자리에 있나」가 아니라 **「앞 토스트를 안 덮나」**까지 본다 —
//    따로 띄우면 열쇠 잔량 안내를 즉시 덮어 «정보를 잃는다».
{
  const 문구 = (await import('../src/tidy.js?t=' + Date.now())).AI다듬는중
  chk('⑧-a 문구가 «한 곳»에 있다 (tidy.js)', typeof 문구 === 'string' && 문구.includes('다듬는 중'), 문구)
  chk('⑧-b 걸리는 시간을 말한다 (막연히 기다리게 하지 않는다)', /\d/.test(문구), 문구)

  // ⛔⛔ **주석을 빼고 «코드»만 본다** — 안 그러면 「주석에 적어둔 이름」을 세어
  //    고침을 통째로 지워도 초록불이 된다(규칙 18 ⓘ · 2026-08-24 열쇠이름 판이 밟은 그 함정).
  //    🧪 실제로 이 판을 만들 때 그랬다 — 고침을 지웠는데 ⑧-c 가 «통과»했다.
  const 읽기 = (p) => readFileSync(ROOT + p, 'utf8')
    .split('\n').filter((줄) => !/^\s*(\/\/|\*|\/\*)/.test(줄)).join('\n')
  for (const [이름, 길] of [['인스타 공유받기(첫인상)', 'src/App.jsx'], ['편집기 캡처', 'src/screens/EditorScreen.jsx']]) {
    const s = 읽기(길)
    chk(`⑧-c ${이름} — 「다듬는 중」을 붙인다`, s.includes('AI다듬는중'), 길)
    // ⛔ «따로» 띄우면 안 된다 — 바로 앞 토스트를 덮는다. 꼬리로 «더해» 써야 한다.
    chk(`⑧-d ${이름} — 따로 안 띄우고 «같은 줄»에 붙인다`,
      !/showToast\(\s*AI다듬는중/.test(s) && /\+\s*AI다듬는중/.test(s), 길)
  }
}
