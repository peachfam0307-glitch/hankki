...
//   ⛔⛔ **[2026-09-02] 잣대를 «소스 글자»에서 «진짜 돌려서»로 옮겼다.**
//      전엔 `/politeSteps\(\s*ai\.steps\s*\)/` 로 **소스 모양**을 봤다. 그런데 그날
//      `mergeTidy` 가 AI 걸음에서 「계량 기준」 줄을 걸러내게 되면서
//      `politeSteps(ai.steps.filter(…))` 가 되자 **정규식이 못 찾아 빨간불**이 났다 —
//      말투는 «멀쩡히 다듬어지고 있었는데» 글자 모양만 달라진 것이다.
//      📌 규칙 18 ⓘ — 검사가 «무엇을 보는지». 소스 모양을 보면 리팩터링마다 거짓 빨간불이 난다.
//   ✅ 그래서 **`mergeTidy` 를 실제로 불러** AI 가 준 한다체가 해요체로 나오는지 본다.
//      ⭐ 이러면 안에서 어떻게 부르든 «결과»만 맞으면 통과한다.
const { mergeTidy: 합치기 } = await import('../src/tidy.js')
const 합친것 = 합치기(
  { title: '테스트', ingredients: ['소금 1T'], steps: ['원래 걸음이에요'], memo: '' },
  { title: '', ingredients: [], steps: ['재료를 넣고 볶는다', '불을 끄줘요'], memo: '' },
)
chk('⑥-c AI 걸음도 «해요체 다듬기»를 거친다',
  합친것.steps.length === 2 &&
  !/볶는다$/.test(합친것.steps[0]) && /요$/.test(합친것.steps[0]) &&
  합친것.steps[1] === '불을 꺼줘요',
  `(AI 만 문체 표준 밖에 있으면 화면에서 말투가 갈린다 — 실물: ${JSON.stringify(합친것.steps)})`)
...
// ══ ⑨ 📢 「AI 가 더 다듬는 중」이라고 «먼저» 말하나 (창업자 2026-09-01) ═══════════════
//
// 📮 창업자 = *"인스타로 유입 될 것 같거든 **첫인상**이니까"*
//    · *"**새유저들 기다리다가 앱 삭제하면 어떻게해**"*
//    · *"너무 오래걸리니까 이상하게 읽힌거 혹은 안읽힌거 **잘못된 걸로 오해**할수도있어"*
//
// ⭐⭐ **심장 = 문이 «둘 다» 말하나.** 인스타 공유받기(App.jsx)가 첫인상이라 더 중요하다.
//    ⛔ 한 문만 고치면 「제일 많이 쓰는 길」만 조용한 채로 남는다 —
//       2026-08-29 에 정확히 그 사고가 있었다(AI 를 편집 캡처에만 붙여 공유받기는 워커를 0번 불렀다).
//
// ⛔⛔ **「어느 자리에 있나」로 끝내지 않는다** — «앞 토스트를 안 덮나»까지 본다.
//    따로 띄우면 열쇠 잔량 안내를 즉시 덮어 «정보를 잃는다». 꼬리로 «더해» 써야 한다.
{
  const 문구 = (await import('../src/tidy.js?t=' + Date.now())).AI다듬는중
  chk('⑨-a 문구가 «한 곳»에 있다 (tidy.js)', typeof 문구 === 'string' && 문구.includes('다듬는 중'), 문구)
  chk('⑨-b 얼마나 걸리는지 말한다 (막연히 기다리게 하지 않는다)', /\d/.test(문구), 문구)

  // ⛔⛔ **주석을 걷어내고 «코드»만 본다.** 안 그러면 주석에 적어둔 이름을 세어
  //    고침을 통째로 지워도 초록불이 된다(규칙 18 ⓘ — 이 파일 ⑦번 칸이 이미 밟은 함정).
  const 코드만보기 = (p) => readFileSync(ROOT + p, 'utf8')
    .replace(/\/\/[^\n]*/g, '').replace(/\/\*[\s\S]*?\*\//g, '')
  for (const [이름, 길] of [
    ['인스타 공유받기(⭐첫인상)', 'src/App.jsx'],
    ['편집기 캡처', 'src/screens/EditorScreen.jsx'],
  ]) {
    const s = 코드만보기(길)
    chk(`⑨-c ${이름} — 「다듬는 중」을 쓴다`, s.includes('AI다듬는중'), 길)
    chk(`⑨-d ${이름} — «따로» 안 띄우고 같은 줄에 «더한다»`,
      !/showToast\(\s*AI다듬는중/.test(s) && /\+\s*\(?[^\n;]*AI다듬는중/.test(s),
      '(따로 띄우면 바로 앞 열쇠 잔량 안내를 덮는다)')
  }

  // 🆓🆓 [2026-09-14 창업자 실물 캡처 20:37] **무료로 읽었으면 그 꼬리를 «안» 붙인다.**
  //   ⛔ 그 전엔 꼬리가 조건 «밖»에 있어 어떤 경우에도 무조건 붙었다 —
  //      무료는 AI 를 아예 안 부르는데 「AI가 더 다듬는 중이에요(20~60초)」라고 도는 척을 했다.
  //   📮 창업자 = "무료할때도 위에 AI가 못읽는다 파랑토스트? 그런거 떠" · "무료읽기에만 안떠야해"
  //   ⭐ 「안 붙는다」만 재면 꼬리를 통째로 지워도 초록불이 된다 → 위 ⑨-d 가 «붙는다»를 같이 잰다.
  {
    const s = 코드만보기('src/screens/EditorScreen.jsx')
    chk('⑨-e 무료로 읽었으면 「AI가 더 다듬는 중」을 안 붙인다',
      /freeTail\s*\?\s*''\s*:\s*AI다듬는중/.test(s),
      '(무료는 AI 를 안 부른다 — 도는 척하면 그게 거짓말이다)')
  }
}
