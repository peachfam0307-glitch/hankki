...
// ✂️✂️ [2026-09-15] **이름대로 «자르기까지» 누른다.**
//   ⛔⛔ 9/13 에 자르기를 건너뛰게 고치면서 이 함수의 «속»을 비웠다 — 이름은 「넣고자르기」인데
//      사진만 넣고 기다렸다. 9/15 에 창업자가 자르기를 되살리자 이 판이 «앱 탓»으로 죽었다.
//      ＝ 판이 거짓말을 한 것이다. 자르기 화면에서 멈춰 있었을 뿐 앱은 멀쩡했다.
//   ⭐ 그래서 사람이 하는 그대로 「이 부분만 읽기」를 누른다. 그래야 읽기가 실제로 시작된다.
async function 사진넣고자르기(p) {
  await p.locator('input[type=file]').first().setInputFiles({ name: 'r.png', mimeType: 'image/png', buffer: 사진 })
  // ⛔ 못 찾으면 «조용히 넘어가지 않는다» — 자르기가 사라졌다는 뜻이고, 그건 9/14 안내가 또 숨었다는 뜻이다.
  await p.getByRole('button', { name: /이 부분만 읽기/ }).first().click({ timeout: 8000 })
...
// ② 열쇠가 남았으면 단추가 «둘»
{
  ...
  재기('열쇠 있으면 단추 둘 (AI · 그냥)',
    // 📮 [2026-09-18 창업자] 단추 글자가 「열쇠는 안 쓰지만 인식률이 떨어져요」로 바뀌었다.
    //   ⛔ 옛 판은 '열쇠 안 써요' 를 찾아서 여기서 죽었다 — **판이 낡은 것이지 코드가 틀린 게 아니다.**
    //   ⭐ 이제 «둘 다» 본다 — ⑴열쇠를 안 쓴다는 말 ⑵인식률이 떨어진다는 «손해»가 단추 안에 있나.
    //      손해를 단추 «안»에 넣은 게 이번 고침의 핵심이라, 그게 빠지면 여기서 잡혀야 한다.
    살아있는.length === 2 && 살아있는[0].includes('AI')
      && 살아있는[1].includes('안 쓰지만') && 살아있는[1].includes('인식률'), JSON.stringify(살아있는))
  const 결과 = await p.locator('.pad.fade').innerText()
  // 📮 [2026-09-18 창업자] *"덜 읽혀요는 ai말투야. 인식률이 떨어진다고 그냥 직관적으로 적자"*
  //   ＋ *"초록박스는 그냥읽기 아래에 둘래??"* → 사진 갈래만 단추 «아래»로 내렸다.
  재기('「인식률이 떨어져요」를 집어 준다 (창업자 지시)',
    결과.includes('기본 인식은 인식률이 떨어져요') && 결과.includes('보면서 고칠 수'),
    결과.includes('인식률이 떨어져요') ? 'ok' : '없다')
  await ctx.close()
}
