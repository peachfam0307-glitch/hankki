...
const 장들 = {
  ...
  //    ⛔ 씨앗 레시피는 음식 아이콘만 붙어 **「꾸몄다」가 안 보인다** — 이 장이 파는 게 정확히 그것이다.
  //    ⭐ 창업자 백업에서 «스티커나 배경지가 실제로 얹힌 편»만 골라 찍는다(`BACKUP=` · 그 판 ⑧칸).
  '02-요리책': { 파일: '20b-요리책-창업자.png', 머리: '레시피가 쌓이면<br>나만의 요리책', 부제: '표지도 내 마음대로 꾸며요 · 스티커 붙이고 배경 깔고' },
  ...
  // 🎴🍂 [2026-09-02] 📮 창업자 = *"레꾸자랑도 «가을꺼»로하자 예쁜걸로"*
  //    ⛔⛔ 그 전엔 `10-랜덤카드.png` 를 썼는데 **2026-08-22 파일**이라 여름 카드였다.
  //       이 판이 그 화면을 «한 번도 안 찍고» 있었고, 그래서 앱이 바뀌어도 스토어만 낡아 갔다(2026-07-31 함정).
  //    ⭐ 이제 `40-자랑-<스킨>.png` 를 찍는다(그 판 ⑨칸 · `CARDS=` 로 갈래를 한꺼번에).
  //       🍂 가을 컷은 09-01 에 열려 **어느 스킨을 골라도 캐릭터가 가을 옷**을 입는다.
  '07-자랑': { 파일: `40-자랑-${process.env.CARD || 'arch'}.png`, 머리: '오늘의 한 끼를<br>카드 한 장으로', // ⛔ 42% 면 아래 「이 카드를 내 레시피 표지로」가 반쯤 잘린다(창업자 지적) → 위로 당긴다
