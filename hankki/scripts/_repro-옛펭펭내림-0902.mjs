...
//   ⛔⛔⛔ **[같은 날 밤 · 세 번째] 씬 풀에도 있었다** — 창업자 = *"옛펭펭아직있어"*(`scene_cook_kitchen`).
//      🔢 낮 = 솔로 9컷 · 저녁 = 콤비 12컷 · 밤 = **씬 6컷.** 세 번 다 뿌리가 하나다 —
//         **이 잣대를 「이름」으로 세웠다.** 바로 위에 내 손으로 «그림으로 세웠어야 했다»고 적어놓고 또 그랬다.
//      ⛔ 게다가 **풀이 아예 달랐다** — 씬은 `scenepool/` 이고 이 잣대는 그 폴더를 **본 적이 없다.**
//      ✅ 여섯을 넣는다(눈으로 갈랐다 = 흰 트렌치 ＋ 벨트 없음).
//      ⚠️ 그래도 이건 **여전히 「이름」 잣대**라 새 풀·새 컷이 오면 또 샌다 —
//         그래서 아래 ⑵ 「풀 인구조사」를 같이 둔다. **그게 실패의 «모양»을 바꾸는 쪽이다**(절대원칙 34).
const 옛펭펭 = /^(peng_|pn_|duo_|scene_(camp|cook_kitchen|market|movie|sandcastle|soup)\.)/
