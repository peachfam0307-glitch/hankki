...
for (const 격자 of ['big', 'small']) {
  for (const px of 크기들) {
    ...
    // ⭐ `SIZES=0` 이면 **아무것도 안 갈아끼우고 «지금 코드 그대로»** 찍는다(규칙 30).
    //    ⛔ 이게 없으면 확정된 값을 확인하려 해도 판이 덮어써서 영영 못 본다.
    if (px) await page.addStyleTag({ content: `.fav-dot .idx-clip { height: ${px}px !important; }` })
