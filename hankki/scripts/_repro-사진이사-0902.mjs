...
//
// 🖼 **여기(재는 게이트)와 「_shot-사진이사-0902.mjs」(보여주는 판)는 «다른 그림»을 쓴다 — 일부러 그렇다.**
//    여기서 900×1200 을 «직접 그리는» 이유 = 「화면에 뜬 그림이 «내가 심은 그것»인가」를
//    naturalWidth === 900 으로 콕 집기 위해서다(음식 아이콘도 data: 라 그것만으론 못 가린다).
//    ⛔ 진짜 사진으로 바꾸면 크기가 제각각이라 그 잣대가 흐려진다. 판 쪽과 맞추려 하지 말 것.
