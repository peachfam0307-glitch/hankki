...
// ⛔ [2026-09-05] 여름 씬 컷이 «풀에서 통째로» 내려갔다 — v12.56(다른 세션 · 창업자 *"셋 다 빼자. 그리고 배포해"*).
//    창업자 원문 ⑵ *"여름씬은 이제 빼도 될 것 같아"* 가 «가을에만 안 나오게»에서 «아예 없음»으로 끝난 것이다.
//    그래서 이 칸은 「여름 날짜로 돌려도 여름 씬이 «0장»」으로 뒤집는다 — 되살아나면 여기서 죽는다.
//    ⚠️ 그날 CI(#2143)가 옛 칸으로 죽어 v12.56 배포가 막혔다 — 앱이 아니라 «판»이 낡았던 것.
적기(!SUMMER.some((s) => 여름씬.has(s)), `여름 날짜로 돌려도 여름 씬은 «없다» — 풀에서 내렸다 (${[...여름씬].filter((s) => SUMMER.includes(s)).join(', ') || '0장'})`)
// ⛔ 「씬이 통째로 없어서 0」이면 안 된다 — 여름에도 사철 씬은 나와야 한다(규칙 18 ⓘ)
적기(여름씬.size >= 4, `여름에도 사철 씬은 그대로 나온다 (${여름씬.size}장)`)
