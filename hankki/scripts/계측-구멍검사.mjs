...
const 안불려도되는 = new Set(['통계꺼짐', '통계끄기설정', '통계시작', '다시왔나', '수집주소만들기', '점검보내기인가', '보낼까', '앱으로켰나', '보낼경로'])  // 2026-09-16 넷 = stats.js 안에서만 쓰는 판정(보낼까·점검보내기인가·앱으로켰나) ＋ 순수 함수(보낼경로 · 재현판 _repro-내부기기 가 잰다)  // 내부·설정용 · 수집주소만들기 = 🍎 직접 전송 주소를 «만들기만» 하는 순수 함수(stats.js 안에서 직접보내기가 부르고 재현판 _repro-아이폰통계직접 이 잰다 · 2026-09-15)
