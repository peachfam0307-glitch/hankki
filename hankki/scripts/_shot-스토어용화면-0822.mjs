...
// ④ 꾸미기 — ⛔「배경」 탭은 스티커가 «한 장도» 안 보인다(단추 셋뿐).
//    스토어에서 「레꾸」를 보여주려면 **스티커가 깔린 서랍**이라야 한다 → 데코 탭을 연다.
await 굴리기(p, -2000)
...
if (await 꾸미기.count()) {
  ...
  for (const 글자 of ['나중에 볼게요', '닫기']) {
    const b = p.getByRole('button', { name: 글자 }).first()
    if (await b.count()) { await b.click(); await p.waitForTimeout(900); break }
  }
  ...
  // ⛔ 탭만 눌러선 «스티커가 안 보인다» — 서랍 맨 위엔 단추 셋(선물·사진·되돌리기)이 있고
  //    스티커 격자는 그 «아래»라 자막 띠에 통째로 덮인다(2026-08-22 실측 · 규칙 21).
  //    → 서랍 «안»을 굴려 스티커를 위로 올린다.
  await p.mouse.move(195, 790)
