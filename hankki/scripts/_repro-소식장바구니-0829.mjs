...
//   ⛔ [2026-09-05 고침] 「첫 장바구니 줄 == 마지막 줄」로 재면 **7일 안에 장바구니가 «두 묶음» 열린 날**
//      (9/05 = 8/29 것 ＋ 9/05 것)에 거짓 빨간불이 난다 — 앱은 둘 다 맨 아래에 잘 뒀는데 판이 틀렸다.
//      ⭐ 잣대 = 「첫 장바구니 줄 «아래»는 전부 장바구니」. 그러면 묶음이 몇 개든 «맨 아래 덩어리»인지 본다.
const 아래전부장바구니 = 장바구니줄 >= 0 && 열림.slice(장바구니줄).every((r) => r.includes('살림템'))
if (장바구니줄 >= 0 && !아래전부장바구니)
  나쁨(`장바구니가 맨 아래가 아니다 — ${장바구니줄 + 1}/${열림.length} (창업자 *"대신 아래 나중에"*)`)
else if (장바구니줄 >= 0) 좋음(`장바구니가 «맨 아래»에 있다 (${열림.length - 장바구니줄}묶음이 다 아래에 붙어 있다)`)
