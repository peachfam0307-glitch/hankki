...
//    ⭐ 9/5: 장바구니 줄이 «둘 이상»일 수 있다(D-0 에 3종이 새로 열림) — 첫 줄이 아니라 «장바구니 아닌 줄이 뒤에 있나»를 본다
const 뒤에딴줄 = 열림.findIndex((r, i) => i > 장바구니줄 && !r.includes('살림템'))
if (장바구니줄 >= 0 && 뒤에딴줄 >= 0)
  나쁨(`장바구니가 맨 아래가 아니다 — ${장바구니줄 + 1}/${열림.length} 뒤에 「${열림[뒤에딴줄].slice(0, 20)}」 (창업자 *"대신 아래 나중에"*)`)
else if (장바구니줄 >= 0) 좋음(`장바구니가 «맨 아래»에 있다 (${열림.length - 장바구니줄}줄)`)
