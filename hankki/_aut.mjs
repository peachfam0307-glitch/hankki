...
// 확인할 것: ①7월엔 네 세트가 하나도 안 보인다 ②9/1·10/1·12/1에 저절로 뜬다
//            ③뜰 땐 맨 위(제철) ④서랍 이미지 깨짐 0 ⑤pageerror 0
//
...
for (const r of rows) {
  console.log(`\n── ${r.label} ──`)
  console.log(`  친구들: ${r['친구들'].join(', ') || '(없음)'}   맨위3=${JSON.stringify(r['친구들_첫3'])}  깨짐 ${r['친구들_깨짐']}`)
  console.log(`  데코  : ${r['데코'].join(', ') || '(없음)'}   맨위3=${JSON.stringify(r['데코_첫3'])}  깨짐 ${r['데코_깨짐']}`)
  console.log(`  pageerror ${r.err}`)
}

// 판정
let bad = 0
const want = { '오늘(7/30)': [], '9/5': ['가을', '추석'], '10/5': ['가을', '추석', '핼러윈'], '12/5': ['가을', '추석', '핼러윈', '크리스마스'] }
for (const r of rows) {
