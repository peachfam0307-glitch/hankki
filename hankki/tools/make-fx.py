...
#
# ⭐⭐ **네 번째이자 제일 중요한 지적 — 「위로 올려라」** (창업자 2026-08-03
#    *"위치를 위로 더 올려야해 **애들 붙이면 아래쪽에 연기넣으면 안보여**"*)
#    📌 **배경 효과는 스티커 «뒤»에 깔린다.** 유저는 캐릭터를 아래쪽에 붙이니까
#      아래에 깐 연기는 그대로 가려진다 — 만들어도 아무도 못 본다.
#    ⛔ 「안개는 땅에 깔리는 것」이라는 «현실»을 따르다가 «화면에서 안 보이는» 걸 만들 뻔했다.
#      우리가 만드는 건 자연이 아니라 **꾸미기 판**이다. 보이는 자리에 둬야 한다.
#    → 아래로 갈수록 진해지는 「띠」를 버리고 **가운데 위쪽이 제일 진한 「봉우리」**로 바꿨다.
#      맨 위(달·별)와 맨 아래(캐릭터 발치)는 저절로 0 이 된다.
FOG_LAYERS = [(.26, 1, 5), (.29, 1, 4), (.17, 2, 3)]   # (진하기, 한 판에 도는 바퀴, 물결 개수)
FOG_COL = (226, 220, 236)
FOG_PEAK, FOG_WIDE = .40, .26      # 봉우리 자리(0=위, 1=아래) · 퍼짐


def fog(bg, F=60, seed=7):
    """안개 — 가운데 위쪽에 걸려 옆으로 «아주 천천히» 흐르고 모양이 변한다. 이음매 없음."""
    SZ = bg.size[0]
    rng = np.random.default_rng(seed)
    t = np.arange(SZ) / SZ * 2 * np.pi
    X, Y = np.meshgrid(t, t)
    # 봉우리 하나짜리 띠 — 위아래 끝에서 저절로 사라진다(경계선이 안 생긴다)
    yy = np.linspace(0, 1, SZ)[:, None]
    band = np.exp(-((yy - FOG_PEAK) / FOG_WIDE) ** 2) * np.clip((yy - .10) / .15, 0, 1)
