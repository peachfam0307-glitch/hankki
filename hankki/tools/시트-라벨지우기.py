...
MAXINK = 0.50      # 이보다 꽉 찬 띠는 그림
# ⛔ [2026-08-24 · 두 번째] 0.42 로도 «세 개»가 넘었다 — s03(0.464) · s06(0.442) · s07(0.443).
#    전부 긴 이름(「대파듬뿍 소불고기전골」 같은 것)이라 띠가 빽빽하다.
# 🔢 창업자 시트 16장 전수 실측 —
#    · 높이 14~90 인 띠 = **113개, 전부 라벨** (높이 21~43 · 잉크 0.01~0.464)
#    · 그림 띠(칸의 첫 띠) 높이 = **155~357** → **90 아래가 하나도 없다**
#    ⭐ 즉 **가르는 것은 이미 «높이»(MAXH 90)** 이고 MAXINK 는 «두 번째 그물»이다.
#    ✅ 0.50 = 라벨 최대 0.464 위로 여유를 두되 그물은 남긴다.
#    ⛔ 더 올리지 말 것 — 얇고 진한 «그림 띠»(고명 한 줄 같은 것)를 놓치게 된다.
...
# ⛔⛔ [2026-08-24] **자기검사가 «헛경보»를 냈다 — 창업자 시트 5장이 통째로 멈췄다.**
#   📮 창업자가 설명을 괄호로 붙여 뽑으면서 라벨이 **두세 줄**이 됐다
#      (실측 s05 = 한 칸에 라벨 띠가 2~3개 · 높이 24~32px · 잉크 0.19~0.32).
#   ⭐ 그러면 첫 줄을 지운 «그 자리»가 **둘째 줄의 「지키는 구역」(keep) 안»에 들어간다.**
#      → 도구가 **자기가 지운 라벨을 「그림을 먹었다」로 세고** 멈췄다. 그림은 한 픽셀도 안 먹었다.
#   ✅ 그래서 「내가 일부러 지운 자리」를 따로 적어 두고 검사에서 «뺀다».
#      ⛔ 검사를 «약하게» 만든 게 아니다 — 지운 적 없는 자리는 여전히 한 픽셀도 못 먹는다.
지운자리 = np.zeros((H, W), bool)
...
for r in range(rowsN):
    for c in range(colsN):
        ...
        for i in labels:
            ...
            지운자리[y0c + top:y0c + e + 1 + dn, x0c:x1c] = True   # ⭐ 아래 자기검사에서 뺀다(2026-08-24)
            killed += 1
            note = ''
            if top < s:
...
for (ys, ye, xs_, xe_) in keep:
    쓴곳 = 지운자리[ys:ye + 1, xs_:xe_]          # 내가 «일부러» 지운 라벨 자리 — 검사에서 뺀다
    b = (art[ys:ye + 1, xs_:xe_] & ~쓴곳).sum()
    n = (after[ys:ye + 1, xs_:xe_] & ~쓴곳).sum()
    if n < b:
        lost += b - n
