...
const PORT = 4215, HOST = '127.0.0.1', BASE = `http://${HOST}:${PORT}/`
...
try {
  ...
  for (const [tab,file] of [['데코','sm-deco.png'],['친구들','sm-buddies.png']]) {
    await page.getByText(tab,{exact:true}).first().click(); await page.waitForTimeout(900)
    const secs = await page.evaluate(()=>[...document.querySelectorAll('.decor-sec-label')].map(e=>e.textContent))
    console.log(`${tab} 탭 섹션 순서:`, secs.join(' / '))
