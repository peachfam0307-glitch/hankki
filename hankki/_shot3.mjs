...
const PORT = 4197, HOST = '127.0.0.1', BASE = `http://${HOST}:${PORT}/`
...
try {
  ...
  console.log('미정리 0 → 칩 숨김?', !/정리 안 한 레시피/.test(await page.evaluate(() => document.body.innerText)))
  ...
  await page.locator('input[inputmode="url"]').first().fill('https://youtu.be/dQw4w9WgXcQ'); await page.waitForTimeout(200)
  await page.screenshot({ path: `${OUT}/n-2-yt-link.png` })
  await page.getByText('바로가기로 저장').first().click(); await page.waitForTimeout(1400)
  ...
  console.log('저장 후 홈 칩 뜸?', /정리 안 한 레시피 1개/.test(home1))
  await page.screenshot({ path: `${OUT}/n-3-home-chip.png` })
  await page.getByText('정리 안 한 레시피').first().click(); await page.waitForTimeout(900)
  console.log('칩 탭 → 목록 열림?', (await page.evaluate(() => document.body.innerText)).includes('Inbox'))
  await page.goto(BASE, { waitUntil: 'domcontentloaded', timeout: 30000 }); await page.waitForTimeout(1400)
  await page.getByRole('button', { name: '가져오기' }).first().click(); await page.waitForTimeout(700)
