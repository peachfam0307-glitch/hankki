...
//   ③ 🖼 **사진은 안 올린다**(창업자 확정 「글자부터」). 폰 안과 백업 파일엔 그대로 남는다.
...
async function 묶어쓰기(F, db, 할일들, 진행) {
  let 한것 = 0
  for (let i = 0; i < 할일들.length; i += 묶음) {
    const 조각 = 할일들.slice(i, i + 묶음)
 ...
 */
export async function 올리기(백업, { 진행, 통째로 } = {}) {
  ...
  const 담기 = (갈래, id, 원소) => {
    ...
    할일.push({ 자리: F.doc(db, 'users', 사람.번호, 갈래, 이름으로(id)), 값: { j: 글 } })
  }
...
export const _내부 = { 사진털기, 문서로, 이름으로, 지문, 메타칸, 한덩어리, 묶음 }
