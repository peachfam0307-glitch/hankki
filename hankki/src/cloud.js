...
export async function 로그인(공급자 = 'google.com') {
  ...
  표식쓰기(true)
  번호쓰기(사람.번호)
  // 📊 [2026-09-14] 🔑 로그인 «성공». ⛔누구인지·어느 계정인지 한 자도 안 보낸다.
  //   ⭐ 왜 재나 = 로그인은 「7일 삭제를 막는 «유일한» 보험」인데(📄로그인-필수로-2026-09-07),
  //      Firebase 는 «성공한 사람»만 센다 → 「프로필까지 온 사람 중 몇이 실제로 됐나」를 못 봤다.
  //   ⛔⛔ 여기뿐이다 — `cloud.js:591` 은 「계정 지우기 전 재인증」이라 로그인이 아니다(붙이면 숫자가 거짓이 된다).
  try { 로그인됨() } catch { /* 통계가 죽어도 로그인은 된다 */ }
