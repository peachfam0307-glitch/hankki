...
import { 로그인됨, 새계정인가 } from './stats.js'
...
export async function 로그인(공급자 = 'google.com') {
  ...
  표식쓰기(true)
  번호쓰기(사람.번호)
  // 🔑 [2026-09-14] 「로그인 몇 명·무엇으로」 — 성공한 «여기» 한 곳에서만 센다(지켜보기 콜백 ⛔ · 계정삭제 전 재인증도 ⛔). 통계가 죽어도 로그인은 그대로.
  try { 로그인됨(공급자, 새계정인가(user && user.metadata)) } catch { /* 통계가 죽어도 로그인은 된다 */ }
