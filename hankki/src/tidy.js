...
// 📢📢 **「AI 가 더 다듬는 중」 꼬리** — 두 문(편집기 캡처 · 인스타 공유받기)이 «같은 말»을 쓴다.
...
//      🔢 실물(2026-09-01 항정살조림) = 걸음에 홍보문구·재료·**댓글**·「모두 보기」가 그대로 들어갔다.
//   ⭐ 오래 걸리는 걸 없앨 순 없다 — **「기다리는 중」이라고 말해주는 것**이 답이다.
//   ⛔ «따로» 띄우지 않는다 — 바로 앞 토스트(열쇠 잔량·초안 안내)를 즉시 덮어 못 보게 된다.
//      그래서 **같은 줄에 꼬리로 붙인다.** 길이는 부르는 쪽이 20초를 준다.
//   ⛔ 이름이 갈리면 한쪽만 낡는다 — 문구는 «여기 한 곳»에서만 고친다(v11.30 「열쇠 이름」과 같은 결).
