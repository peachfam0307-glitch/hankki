...
const MYRECIPES_COACH_STEPS = [
  ...
  { sel: '[data-coach="log"]', label: '한끼 일기', desc: '요리하고 "만들었어요!" 한 번이면 사진·한 줄·팁이 쌓여요 · 다음엔 "그때 그 맛" 그대로 재현!' },
]
        ...
        <div className="pad fade log-2col">
          ...
          <div className="t-sub" style={{ fontSize: 12.5, lineHeight: 1.55, marginBottom: 10 }}>
            {/* ✍️ [2026-08-12] 창업자 *"한끼일기에 나만의 별점~~블라블라 하는거 설명 바꾸면 좋겠어."*
                ...
                ⛔⛔ [2026-08-20 창업자 *"별점부분도 삭제하고"*] 「별점」을 뺐다 —
                   같은 날 별점 입구를 «안 되살린다»고 확정(ⓐ)했는데 **이 안내만 아직 별점을 말하고 있었다.**
                   유저가 별점을 찾다 못 찾는 자리다. 📌 기능을 접으면 그 기능을 «설명하는 글»도 같이 접는다
                   (v11.02 에서 북마크 → 책갈피로 이름을 옮길 때 배운 그것). */}
            <b style={{ color: 'var(--text)' }}>오늘 뭘 해먹었는지</b>가 한 장씩 쌓여요. 사진 한 장, 한 줄이면 충분해요 — 나중에 넘겨보면 <b style={{ color: 'var(--text)' }}>그날의 내가</b> 보여요.
          </div>

          {entries.length === 0 ? (
            <div className="empty" style={{ marginTop: 10 }}>{'아직 기록이 없어요.\n요리하고 "만들었어요!"만 눌러도 사진·한 줄이 한 장씩 쌓여요.\n다음에 "그때 그 맛"을 그대로 재현하는 나만의 요리 일기예요'}</div>
          ) : (
            <>
