          ...
          <div style={{
            ...
            {ocrLeft.total > 0 ? (
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                {/* 🔑🔑 [창업자 확정 2026-08-29] 「계속 된다」로 끝내지 «않는다» — «어떻게 하면 되는지»까지.
                    📮 창업자 = *"그걸 정확히 안내해야해. 열쇠가 소진되면, 갤러리-사진 가져오기에서 담고 수정하라고"*
                       · *"사진을 보면서 수정할 수가 있다. **기본인식이라 인식률의 차이가 잇으니까 이부분을 집어줘야해
                         그래야 무료유저도 안떠나**"*
                    ⭐⭐ 옛 문구는 「기본 인식으로 계속 읽어 드려요」에서 끝나서 **「되긴 된다」만 말했다.**
                       유저는 결과가 엉성한 걸 보고 «앱이 못한다»고 읽는다 — 고칠 길이 있는 걸 모른 채.
                    ⭐ 그래서 셋을 한 줄씩 = ⑴덜 정확하다(솔직히) ⑵**사진으로 담아라**(붙여넣기론 볼 사진이 없다)
                       ⑶사진 보며 고칠 수 있다 — 그게 「캡쳐 보면서 쓰기」(EditorScreen)로 이어진다.
                    ⛔ 「덜 정확해요」를 빼지 말 것 — 감추면 유저가 «고장»으로 읽는다. */}
                <div style={{ fontSize: 15.3, color: 'var(--text-sub)', marginTop: 2, lineHeight: 1.5 }}>
                  기본 인식으로 계속 읽어요 · 조금 덜 정확해요<br />
                  <b style={{ fontWeight: 800, color: '#8a6a3a' }}>사진으로 담으면</b> 사진 보며 고칠 수 있어요<br />
                  다음 달에 <b style={{ fontWeight: 800, color: '#8a6a3a' }}>무료 5{KEY_UNIT}</b> 채워져요
                </div>
              </div>
            )}
