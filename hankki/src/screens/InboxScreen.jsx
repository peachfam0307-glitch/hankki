...
function 남은까닭(r) {
  ...
  // 🆓🆓 [창업자 2026-09-14 «무료읽기에만 안떠야해»] 무료로 읽은 편엔 AI 얘기를 «한 줄도» 안 한다.
  //   ⛔ AI 를 부른 적이 없으니 「안 됐어요」는 거짓말이고, 「한 번 더」 누를 단추도 안 보인다(218줄).
  //      말은 남고 단추는 없는 게 제일 나쁜 모양이다.
  //   ⭐ 열쇠로 읽은 쪽은 그대로 뜬다 — 거긴 진짜로 실패한 것이다.
  if (!r?.freeRead && r?.tidying) return 'AI가 다듬는 중이에요 · 20~60초 걸려요'
  if (!r?.freeRead && (r?.tidyFail === 1 || r?.tidyFail === 2)) return 'AI 다듬기가 안 됐어요 · 아래 단추로 한 번 더'
