    ...
    <div className="screen fade" style={{ paddingBottom: 0 }}>
      ...
      <div className="detail-bar">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 5, height: 36, marginLeft: 10, padding: '0 15px', background: 'var(--brown)', color: '#fffdf8', fontSize: 15.5, fontWeight: 800, borderRadius: 999, border: 'none' }}
          >
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '12px 20px 0' }}>
        <button
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: 5, height: 34, background: 'var(--cream)', color: 'var(--brown)', fontSize: 14.5, fontWeight: 800, padding: '0 13px 0 9px', borderRadius: 999, border: 'none' }}
        >
        ...
        <button
          ...
          data-coach="decor"
          aria-label="레시피 꾸미기"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 5, height: 34, background: 'var(--brown)', color: '#fff', fontSize: 14.5, fontWeight: 800, padding: '0 13px', borderRadius: 999, border: 'none' }}
        >
      ...
      <div className="pad" style={{ paddingTop: 18, paddingBottom: 120 }}>
        {r.status === 'unsorted' && (
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--brown)' }}>아직 정리 전이에요</div>
              <div className="t-sub" style={{ fontSize: 14.5 }}>제목·재료·태그를 정리하고 레시피로 저장하기</div>
            </div>
        ...
        {r.sample && (
          <div style={{ fontSize: 15, marginTop: 6, lineHeight: 1.55, color: 'var(--brown)', letterSpacing: '-.2px' }}>
            ...
            <div className="sec-head" style={{ marginTop: 14, marginBottom: 6 }}>
              <div className="sec-title-row" style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <button className="press" onClick={() => setGuide(true)} aria-label="계량·손질 가이드"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '3px 8px', borderRadius: 999, background: 'var(--cream)', color: 'var(--brown)', fontSize: 14, fontWeight: 700, whiteSpace: 'nowrap', flex: '0 0 auto' }}>
        ...
        {pantryPicks.length > 0 && (
          <div data-coach="pantry" className="card" style={{ marginTop: 20, padding: 14, background: 'var(--cream)', border: '1.5px solid var(--cream-deep)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 16.5, fontWeight: 800, color: 'var(--brown)', marginBottom: 8 }}>
            ...
            {shownPicks.map((p) => (
              <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: '1px solid rgba(0,0,0,.05)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {mallBadge(p) && (
                    <span style={{ marginLeft: 6, fontSize: 14, fontWeight: 700, whiteSpace: 'nowrap', borderRadius: 5, padding: '1px 6px', ...(mallBadge(p).includes('조합원') ? { color: '#fff', background: '#c2703f' } : { color: 'var(--brown)', background: 'var(--cream-deep)' }) }}>{mallBadge(p)}</span>
                  )}
                </div>
                {/* ⛔ 한살림은 사러가기를 안 그린다 (창업자 2026-08-17 *"링크안달면되고"*) */}
                {isHansalim(p)
                  ? <span style={{ flex: '0 0 auto', fontSize: 14, fontWeight: 700, color: 'var(--text-sub)' }}>매장에서</span>
                  : <button className="press" onClick={() => openUrl(productLink(p))} style={{ flex: '0 0 auto', padding: '6px 13px', borderRadius: 10, background: 'var(--cream-deep)', color: 'var(--brown)', fontWeight: 800, fontSize: 14.5 }}>사러가기</button>}
              </div>
            ))}
            ...
            {pantryPicks.length > PICK_FOLD && (
              <button className="press" onClick={() => setPicksOpen((v) => !v)} aria-label={picksOpen ? '장바구니 재료 접기' : '장바구니 재료 더 보기'} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, width: '100%', marginTop: 2, padding: '9px 0', borderTop: '1px solid rgba(0,0,0,.05)', color: 'var(--brown)', fontWeight: 800, fontSize: 14.5 }}>
                {picksOpen ? '접기' : `${pantryPicks.length - PICK_FOLD}개 더보기`}
                <Icon name={picksOpen ? 'chevron-up' : 'chevron-down'} size={13} color="var(--brown)" />
              </button>
            )}
            <button className="press" onClick={addAllPicks} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, width: '100%', marginTop: 11, padding: '11px 0', borderRadius: 12, background: 'var(--brown)', color: '#fff', fontWeight: 800, fontSize: 15 }}>
            ...
            <div style={{ fontSize: 14, color: 'var(--text-sub)', textAlign: 'center', marginTop: 7, lineHeight: 1.5 }}>평소에 제가 쓰는 재료들이에요 · 레시피에도 계속 추가돼요</div>
          </div>
        )}
        ...
        {r.memo && (
          <>
            ...
            <div className="card" style={{ padding: 14, fontSize: 15, lineHeight: 1.6, color: 'var(--text)', background: 'var(--cream)', border: 'none', whiteSpace: 'pre-line' }}>
              {r.memo}
            </div>
          </>
        )}
        ...
        {r.sourceUrl && (
          <>
            ...
            <a href={r.sourceUrl} target="_blank" rel="noreferrer" className="card press" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 14, textDecoration: 'none', color: 'var(--text)' }}>
              ...
              <span style={{ flex: 1, fontSize: 15.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.sourceUrl}</span>
        ...
        <Portal>
          <div className="sheet-mask" onClick={() => setShareSheet(false)}>
            <div className="sheet" onClick={(e) => e.stopPropagation()}>
              <div style={{ fontSize: 17.5, fontWeight: 800, textAlign: 'center', color: 'var(--text)' }}>친구랑 공유하기</div>
              <div style={{ fontSize: 14.5, color: 'var(--text-sub)', textAlign: 'center', margin: '4px 0 16px' }}>예쁜 카드로 카톡·인스타에 톡 보내요</div>
              <button className="press" onClick={() => { setShareSheet(false); setDrawOpen(true) }}
                ...
                <span><span style={{ fontSize: 16.5, fontWeight: 800, color: 'var(--text)' }}>랜덤 카드 뽑기</span><br /><span style={{ fontSize: 14.5, color: 'var(--text-sub)' }}>꼬르곰·펭펭이 매번 다르게 · 안 꾸며도 예쁘게</span></span>
              </button>
              <button className="press" onClick={isDecorated ? doShareCover : () => { setShareSheet(false); setDecorOpen(true) }}
                ...
                <span><span style={{ fontSize: 16.5, fontWeight: 800, color: 'var(--text)' }}>내가 꾸민 표지로</span><br /><span style={{ fontSize: 14.5, color: 'var(--text-sub)' }}>{isDecorated ? '배경·스티커·효과 그대로 캡처' : '먼저 예쁘게 꾸며볼까요 →'}</span></span>
              </button>
            </div>
          </div>
        </Portal>
