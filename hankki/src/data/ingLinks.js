...
export const ING_LINKS = {
  ...
  // 🌿 [창업자 확정 2026-09-21] 맞는 표기는 «들깻가루» — 국립국어원 = 「고춧가루·후춧가루와 같은 꼴이라
  //    사전에 없어도 들깻가루로 적는 것이 맞다」. ⛔ 우리는 6편이 「들깨가루」로 쓰고 있어 거꾸로였다.
  //    ⛔ 처음엔 옛 표기 「들깨가루」도 열쇠로 남겼는데 check-inglinks 가 「한 줄도 안 붙는다」고 잡았다.
  //       ⭐ 내 근거가 «틀렸다» — 장바구니는 담을 때 url 을 «줄에 저장»한다(store.jsx:1158 `url: ingLink(…)`).
  //          이미 담긴 줄은 이름으로 다시 찾지 않으니 옛 열쇠가 없어도 안 끊긴다. 그래서 뺐다.
  //       📌 아이콘 표(ingIcons)에는 옛 표기를 «남겼다» — 그쪽은 이름으로 «그때그때» 찾는다.
  '들깻가루': 'https://link.coupang.com/a/hcIaobH7Ce', // 햇님마을 들깨가루 100g · 들깨(국산) 100% · 창업자 캡처 2026-09-21
...
export const ingLink = (name) => {
  ...
  // 💰💰 **[2026-09-15 · 창업자 판정] 「링크가 «있는» 첫 제품」을 고른다 — 맨 앞이 아니라.**
  //   📮 창업자 = *"한살림은 뒤로 다 밀고, 쿠팡을 앞에 둬야지."*
  //            ＋ *"큐레이션에 한살림추천템은 그대로 두고 쿠팡을 추가로 더 올려야 할 것 같아."*
  //
  //   ⛔⛔ 뿌리 = 옛 판은 **맨 앞 제품 하나**만 봤다. 그런데 `productLink()` 는
  //      한살림·아이쿱을 **빈 문자열**로 돌려준다(조합원 전용 · 창업자 2026-08-17 *"링크안달면되고"*).
  //      → **한살림이 그 낱말을 잡으면 뒤에 있는 쿠팡 제품이 «영영» 못 나왔다.**
  //      🔢 실측(2026-09-15) = `matches` 낱말 99개 중 **55개**가 막혀 있었다. 그중 **45개**가 이 꼴 —
  //         어묵·우유·요거트·만두·김치 넷·잼 둘·참치 셋·목이버섯… **뒤에 쿠팡/컬리 제품이 이미 있었다.**
  //      💰 막힌 동안 그 재료를 담아도 **수수료 0원**이었다(파트너스 링크가 아니라 일반 검색으로 갔다).
  //      📌 어묵이 그 증거 — 「어묵」은 막히고 **「사각어묵」은 붙었다.** 같은 제품인데 부르는 말만 달랐다.
  //
  //   ⭐⭐ **왜 `picksForIngredients` «안»을 안 고쳤나 — 그 함수를 셋이 쓴다.**
  //      ① 여기(재료 링크)  ② `RecipeDetailScreen`(상세 화면 제품 추천)  ③ `curation.js`(이번 주 픽)
  //      ⛔ 안을 고치면 **화면 추천 차례와 이번 주 픽까지 흔들린다** — 창업자가 정한 차례다.
  //      ✅ 그래서 **고르는 쪽(여기)에서만** 거른다. 화면은 한 글자도 안 바뀐다.
  //         ＝ 창업자 말 그대로 「한살림 추천템은 그대로 두고」 링크만 살아난다.
  //
  //   ⛔ 순서를 «뒤집지» 않는다 — 앞에서부터 보되 **링크가 있는 첫 놈**을 집을 뿐이다.
  //      그래야 창업자가 매긴 차례(좋은 것부터)가 링크 있는 것들 사이에서 그대로 지켜진다.
  const 후보 = picksForIngredients([n], '')
  for (const p of 후보) {
