// 🥕🥕 냉장고·장보기 «재료» 아이콘 — 창업자 시트 11장(2026-08-12) 낱개 171컷.
//
// ✅✅ 창업자 확정 *"냉장고(장보기)에 넣자. **주부장바구니는 그림체가 달라 따로 뽑아야해.**"*
//   ⛔ 큐레이션(주부의 장바구니)엔 쓰지 않는다 — 거긴 assets/curation/cu_* 제품 일러다.
//
// ⛔⛔ **「요리 제목」 규칙(ICON_RULES)에 얹지 않는다.** 2026-08-12 아침에 정확히 그 사고가 났다 —
//   냉장고에 「애호박」을 담으면 「애호박새우젓볶음」 규칙에 걸려 **완성 접시 사진**이 붙었다.
//   재료와 요리는 «다른 축»이라 표를 따로 둔다. guessIngredientIcon 이 이 표를 «먼저» 본다.
//
// ⛔ 이름·비율은 손으로 적지 않는다 — scripts/_넣기-재료아이콘-0812.mjs 가 앱등록.json 에서 낸다.
const 그림 = import.meta.glob('../assets/stickers/ing/*.png', { eager: true, query: '?url', import: 'default' })
export const ING_SRC = Object.fromEntries(
  Object.entries(그림).map(([p, url]) => [키뽑기(p), url]),
)
// 경로에서 파일 이름만 — ⛔ 확장자를 떼는 방식을 바꾸면 키가 통째로 어긋난다
function 키뽑기(p) {
  const f = p.split('/').pop()
  return f.slice(0, f.lastIndexOf('.'))
}

// [재료 이름, 아이콘 키] — ⭐긴 이름 먼저(「모짜렐라치즈」 > 「치즈」)
export const ING_RULES = [
  ['모짜렐라치즈', 'ig_s7_02'],
  ['샌드위치치즈', 'ig_s7_03'],
  ['샐러드드레싱', 'ig_s10_16'],
  ['토마토소스', 'ig_s1_07'],
  ['곤드레나물', 'ig_s2_12'],
  ['샐러드채소', 'ig_s2_16'],
  ['페퍼론치노', 'ig_s5_13'],
  ['발사믹소스', 'ig_s5_14'],
  ['천도복숭아', 'ig_s7_06'],
  ['건조매생이', 'ig_s8_02'],
  ['생오리고기', 'ig_s8_06'],
  ['베이크드빈', 'ig_s9_08'],
  ['올리브오일', 'ig_s10_13'],
  ['그릭요거트', 'ig_s11_09'],
  // 🦑 [2026-09-12 창업자 제공] 냉장고에 «컷 자체가 없던» 넷 — 문어·관자·해물모듬·참깨.
  //    📮 창업자 *"문어 관자는 냉장고재료에 없더라고.. 잘라서 냉장고에 넣어줘."*
  //    ⛔ 「문어」는 전에 요리 규칙의 오징어 그림(squid)을 빌려 쓰고 있었다 — 이제 제 그림이 있다.
  //    ⭐ 「참깨」는 기존 「통깨」(ig_s4_01 = 나무 스쿱)와 «다른 컷»이다. 둘 다 둔다.
  ['해물모듬', 'ig_s12_03'],
  ['브로콜리', 'ig_s2_06'],
  ['파프리카', 'ig_s3_16'],
  ['들깻가루', 'ig_s4_06'],
  ['들깨가루', 'ig_s4_06'],   // ⛇옛 표기 — 이미 깔린 폰의 장바구니 줄이 안 끊기게 남긴다
  ['냉면육수', 'ig_s4_07'],
  ['치킨스톡', 'ig_s4_08'],
  ['가루육수', 'ig_s4_10'],
  ['스리라차', 'ig_s4_13'],
  ['사골육수', 'ig_s4_15'],
  ['초피액젓', 'ig_s5_02'],
  ['올리고당', 'ig_s5_07'],
  ['초고추장', 'ig_s5_15'],
  ['돼지고기', 'ig_s6_08'],
  ['체다치즈', 'ig_s7_01'],
  ['훈제오리', 'ig_s8_05'],
  ['치아바타', 'ig_s9_07'],
  ['블루베리', 'ig_s10_02'],
  ['파인애플', 'ig_s10_04'],
  ['아보카도', 'ig_s10_06'],
  ['떡볶이떡', 'ig_s10_11'],
  ['파스타면', 'ig_s11_03'],
  ['부침가루', 'ig_s11_06'],
  ['튀김가루', 'ig_s11_07'],
  ['라면사리', 'ig_s11_12'],
  ['그래놀라', 'ig_s11_13'],
  ['도토리묵', 'ig_s11_16'],
  ['시금치', 'ig_s1_03'],
  ['애호박', 'ig_s1_06'],
  ['공심채', 'ig_s1_09'],
  ['단호박', 'ig_s1_10'],
  ['콩나물', 'ig_s1_11'],
  ['미나리', 'ig_s1_13'],
  ['옥수수', 'ig_s2_01'],
  ['양상추', 'ig_s2_03'],
  ['양배추', 'ig_s2_04'],
  ['청고추', 'ig_s2_05'],
  ['홍고추', 'ig_s2_07'],
  ['청경채', 'ig_s2_09'],
  ['고사리', 'ig_s2_10'],
  ['시래기', 'ig_s2_11'],
  ['콜라비', 'ig_s2_14'],
  ['샐러리', 'ig_s2_15'],
  ['루꼴라', 'ig_s3_11'],
  ['굴소스', 'ig_s4_03'],
  ['매실청', 'ig_s4_04'],
  ['참기름', 'ig_s4_09'],
  ['들기름', 'ig_s4_11'],
  ['다시마', 'ig_s4_16'],
  ['진간장', 'ig_s5_03'],
  ['국간장', 'ig_s5_06'],
  ['고추장', 'ig_s5_11'],
  ['고등어', 'ig_s6_01'],
  ['임연수', 'ig_s6_03'],
  ['가자미', 'ig_s6_04'],
  ['참치캔', 'ig_s6_05'],
  ['소고기', 'ig_s6_09'],
  ['닭다리', 'ig_s6_10'],
  // 🐔 [2026-08-23 창업자 판정] *"닭고기는 통닭으로"* — 컷은 «이미 있었다»(`영계` = ig_s6_11).
  //    ⭐ 못 그린 게 아니라 «이름을 안 이어둔» 것이었다. 줄 하나로 끝난다.
  ['닭고기', 'ig_s6_11'],
  ['소세지', 'ig_s6_13'],
  ['황태채', 'ig_s6_14'],
  ['토마토', 'ig_s7_04'],
  ['복숭아', 'ig_s7_05'],
  ['바나나', 'ig_s7_08'],
  ['단무지', 'ig_s8_01'],
  ['명란젓', 'ig_s8_03'],
  ['쭈꾸미', 'ig_s8_09'],
  ['고구마', 'ig_s8_13'],
  ['양고기', 'ig_s8_15'],
  ['요거트', 'ig_s8_16'],
  ['밀가루', 'ig_s9_04'],
  ['메밀면', 'ig_s9_05'],
  ['바게트', 'ig_s9_06'],
  ['생크림', 'ig_s9_10'],
  ['두부면', 'ig_s9_11'],
  ['콩국물', 'ig_s9_12'],
  ['오징어', 'ig_s9_16'],
  ['떡국떡', 'ig_s10_14'],
  ['묵은지', 'ig_s11_02'],
  ['우동면', 'ig_s11_11'],
  ['시리얼', 'ig_s11_14'],
  ['누룽지', 'ig_s11_15'],
  ['생강', 'ig_s1_02'],
  ['연근', 'ig_s1_04'],
  ['부추', 'ig_s1_05'],
  ['봄동', 'ig_s1_14'],
  ['쑥갓', 'ig_s1_15'],
  ['우엉', 'ig_s1_16'],
  ['가지', 'ig_s2_02'],
  ['깻잎', 'ig_s2_08'],
  ['대파', 'ig_s3_01'],
  ['숙주', 'ig_s3_02'],
  ['쪽파', 'ig_s3_03'],
  ['배추', 'ig_s3_04'],
  ['당근', 'ig_s3_05'],
  ['마늘', 'ig_s3_06'],
  ['양파', 'ig_s3_07'],
  ['오이', 'ig_s3_08'],
  ['감자', 'ig_s3_09'],
  ['상추', 'ig_s3_10'],
  ['버섯', 'ig_s3_12'],
  ['달래', 'ig_s3_13'],
  ['냉이', 'ig_s3_14'],
  ['두릅', 'ig_s3_15'],
  ['문어', 'ig_s12_01'],
  ['관자', 'ig_s12_02'],
  ['참깨', 'ig_s12_04'],
  ['통깨', 'ig_s4_01'],
  ['후추', 'ig_s4_02'],
  ['배즙', 'ig_s4_05'],
  ['청주', 'ig_s4_12'],
  ['연두', 'ig_s4_14'],
  ['쯔유', 'ig_s5_01'],
  ['소금', 'ig_s5_04'],
  ['설탕', 'ig_s5_05'],
  ['맛술', 'ig_s5_08'],
  ['된장', 'ig_s5_09'],
  ['쌈장', 'ig_s5_16'],
  ['삼치', 'ig_s6_02'],
  ['미역', 'ig_s6_06'],
  ['대하', 'ig_s6_07'],
  ['영계', 'ig_s6_11'],
  ['멸치', 'ig_s6_15'],
  ['맛살', 'ig_s6_16'],
  ['키위', 'ig_s7_07'],
  ['딸기', 'ig_s7_09'],
  ['사과', 'ig_s7_10'],
  ['포도', 'ig_s7_12'],
  ['수박', 'ig_s7_15'],
  ['참외', 'ig_s7_16'],
  ['전복', 'ig_s8_04'],
  ['연어', 'ig_s8_07'],
  ['낙지', 'ig_s8_08'],
  ['순대', 'ig_s8_10'],
  ['잡곡', 'ig_s8_12'],
  ['장어', 'ig_s8_14'],
  ['계란', 'ig_s9_01'],
  ['두부', 'ig_s9_02'],
  ['식빵', 'ig_s9_03'],
  ['우유', 'ig_s9_09'],
  // 🥬 [2026-08-23 창업자 제공] 김치 — 그때까지 «컷 자체가 없어» 못 붙이던 것.
  //    ⛔ 반드시 `['김', …]`(1글자) «위»에 둔다 — 아래에 두면 「김」이 김치를 먼저 채간다.
  //       이 파일 아래 주석이 그 함정을 이미 적어두고 있다(「김」이 «김치»에).
  ['김치', 'ig_add01'],
  ['꽃게', 'ig_s9_13'],
  ['갈치', 'ig_s9_14'],
  ['새우', 'ig_s9_15'],
  ['망고', 'ig_s10_01'],
  ['메론', 'ig_s10_03'],
  ['체리', 'ig_s10_05'],
  ['레몬', 'ig_s10_07'],
  ['두유', 'ig_s10_08'],
  ['버터', 'ig_s10_09'],
  ['잼류', 'ig_s10_10'],
  ['식초', 'ig_s10_12'],
  ['만두', 'ig_s10_15'],
  ['전분', 'ig_s11_04'],
  ['당면', 'ig_s11_05'],
  ['소면', 'ig_s11_10'],
  ['무', 'ig_s1_01'],
  ['햄', 'ig_s6_12'],
  ['배', 'ig_s7_11'],
  ['감', 'ig_s7_13'],
  ['귤', 'ig_s7_14'],
  ['김', 'ig_s8_11'],
  ['꿀', 'ig_s11_01'],
  ['쌀', 'ig_s11_08'],
]

// 가로÷세로 — 실제 PNG 에서 잰 값(⛔짐작 아님)
export const ING_RATIO = {
  ig_s1_01: 0.6463,
  ig_s1_02: 0.9371,
  ig_s1_03: 0.7636,
  ig_s1_04: 0.9076,
  ig_s1_05: 0.695,
  ig_s1_06: 0.6514,
  ig_s1_07: 0.9028,
  ig_s1_09: 0.8082,
  ig_s1_10: 1.1414,
  ig_s1_11: 0.9243,
  ig_s1_13: 0.6038,
  ig_s1_14: 0.937,
  ig_s1_15: 0.7531,
  ig_s1_16: 0.6931,
  ig_s10_01: 1.0691,
  ig_s10_02: 1.2132,
  ig_s10_03: 1.0625,
  ig_s10_04: 0.8227,
  ig_s10_05: 0.917,
  ig_s10_06: 1.0407,
  ig_s10_07: 1.1429,
  ig_s10_08: 0.8078,
  ig_s10_09: 0.9003,
  ig_s10_10: 1.319,
  ig_s10_11: 1.2938,
  ig_s10_12: 0.5392,
  ig_s10_13: 0.6638,
  ig_s10_14: 1.3582,
  ig_s10_15: 1.2387,
  ig_s10_16: 1.0661,
  ig_s11_01: 0.7724,
  ig_s11_02: 1.1815,
  // 🥬 김치 (창업자 2026-08-23 · 실측 420×363)
  // ⛔ 받은 원본은 991×857 · 1,171KB 였다 — 다른 재료 171장은 긴변 중앙값 296px · 91KB 다.
  //    앱은 이 그림을 34~56px 로 그리므로 화면은 똑같은데 유저가 받는 양만 5배였다 → 420px 로 줄였다.
  //    ⭐ 원본은 `hold/검색아이콘-0823` 의 `75206f2d` 에 그대로 있다(더 크게 쓸 일이 생기면 거기서).
  ig_add01: 1.157,
  ig_s11_03: 0.887,
  ig_s11_04: 1.0741,
  ig_s11_05: 1.1202,
  ig_s11_06: 1.1047,
  ig_s11_07: 1.1811,
  ig_s11_08: 1.0269,
  ig_s11_09: 1.1944,
  ig_s11_10: 1.2415,
  ig_s11_11: 1.2291,
  ig_s11_12: 1.1464,
  ig_s11_13: 1.1681,
  ig_s11_14: 1.2287,
  ig_s11_15: 1.3153,
  ig_s11_16: 1.2085,
  ig_s2_01: 0.6429,
  ig_s2_02: 0.716,
  ig_s2_03: 0.9856,
  ig_s2_04: 0.919,
  ig_s2_05: 0.7423,
  ig_s2_06: 0.8963,
  ig_s2_07: 0.7615,
  ig_s2_08: 0.7918,
  ig_s2_09: 0.7803,
  ig_s2_10: 0.6629,
  ig_s2_11: 0.6861,
  ig_s2_12: 0.7057,
  ig_s2_14: 0.6702,
  ig_s2_15: 0.6266,
  ig_s2_16: 0.7775,
  ig_s3_01: 0.6697,
  ig_s3_02: 0.9757,
  ig_s3_03: 0.6994,
  ig_s3_04: 0.7087,
  ig_s3_05: 1.0513,
  ig_s3_06: 1.0625,
  ig_s3_07: 0.8881,
  ig_s3_08: 1.1299,
  ig_s3_09: 1.0541,
  ig_s3_10: 1.0326,
  ig_s3_11: 0.9896,
  ig_s3_12: 1.3755,
  ig_s3_13: 0.7134,
  ig_s3_14: 0.882,
  ig_s3_15: 0.8164,
  ig_s3_16: 1.0565,
  ig_s4_01: 1.1975,
  ig_s4_02: 0.507,
  ig_s4_03: 0.5559,
  ig_s4_04: 0.8803,
  ig_s4_05: 0.9412,
  ig_s4_06: 1.3364,
  ig_s4_07: 0.8352,
  ig_s4_08: 0.9368,
  ig_s4_09: 0.5106,
  ig_s4_10: 1.1533,
  ig_s4_11: 0.443,
  ig_s4_12: 0.4482,
  ig_s4_13: 0.485,
  ig_s4_14: 0.4406,
  ig_s4_15: 1.1033,
  ig_s4_16: 1.2748,
  ig_s5_01: 0.541,
  ig_s5_02: 0.4146,
  ig_s5_03: 0.4551,
  ig_s5_04: 0.9741,
  ig_s5_05: 0.7887,
  ig_s5_06: 0.4367,
  ig_s5_07: 0.5113,
  ig_s5_08: 0.4656,
  ig_s5_09: 1.187,
  ig_s5_11: 1.1621,
  ig_s5_13: 1.3949,
  ig_s5_14: 0.4183,
  ig_s5_15: 0.4883,
  ig_s5_16: 1.2,
  ig_s6_01: 1.9363,
  ig_s6_02: 2.4264,
  ig_s6_03: 1.8198,
  ig_s6_04: 1.3692,
  ig_s6_05: 1.0922,
  ig_s6_06: 1.4231,
  ig_s6_07: 1.0868,
  ig_s6_08: 1.3868,
  ig_s6_09: 1.1751,
  ig_s6_10: 1.0496,
  ig_s6_11: 1.0611,
  ig_s6_12: 1.2348,
  ig_s6_13: 1.5429,
  ig_s6_14: 1.8304,
  ig_s6_15: 1.44,
  ig_s6_16: 1.5025,
  ig_s7_01: 1.0988,
  ig_s7_02: 1.3624,
  ig_s7_03: 1.2304,
  ig_s7_04: 1.1713,
  ig_s7_05: 1.2826,
  ig_s7_06: 1.2435,
  ig_s7_07: 1.1841,
  ig_s7_08: 1.2645,
  ig_s7_09: 1.2466,
  ig_s7_10: 1.0766,
  ig_s7_11: 1.0511,
  ig_s7_12: 0.9963,
  ig_s7_13: 1.3581,
  ig_s7_14: 1.1885,
  ig_s7_15: 1.2748,
  ig_s7_16: 1.3318,
  ig_s8_01: 1.1793,
  ig_s8_02: 1.0542,
  ig_s8_03: 1.1152,
  ig_s8_04: 1.186,
  ig_s8_05: 1.3694,
  ig_s8_06: 1.3348,
  ig_s8_07: 1.3109,
  ig_s8_08: 1.2776,
  ig_s8_09: 1.1083,
  ig_s8_10: 1.2851,
  ig_s8_11: 1.3624,
  ig_s8_12: 1.0288,
  ig_s8_13: 1.551,
  ig_s8_14: 1.5048,
  ig_s8_15: 1.2197,
  ig_s8_16: 1.4292,
  ig_s9_01: 1.2286,
  ig_s9_02: 1.1103,
  ig_s9_03: 1.0539,
  ig_s9_04: 0.9333,
  ig_s9_05: 1.2944,
  ig_s9_06: 1.244,
  ig_s9_07: 1.3728,
  ig_s9_08: 1.1858,
  ig_s9_09: 0.8239,
  ig_s9_10: 1.1293,
  ig_s9_11: 1.1826,
  ig_s9_12: 0.9672,
  ig_s9_13: 1.385,
  ig_s9_14: 1.4163,
  ig_s9_15: 1.1959,
  ig_s9_16: 1.1285,
}

// 재료 이름 → 아이콘 키. 못 찾으면 null(그때 옛 SVG 도형으로 간다).
//   ⭐ 「돼지고기 앞다리살 300g」처럼 «수식어＋분량»이 붙어도 걸리게 «포함»으로 본다.
//
// ⛔⛔ **한 글자 이름은 «포함»으로 보면 안 된다** (CLAUDE.md 핀 — 게·굴·김으로 스파게티·바게트가 해산물이 된 그 함정).
//   여기서도 그대로 터진다: 「김」이 «김치»에 · 「무」가 «단무지»에 · 「배」가 «배추»에 · 「감」이 «감자»에.
//   ⭐ 긴 이름을 먼저 보니 「배추」·「감자」는 저절로 피해지지만, 목록에 «없는» 말(김치·단무지 아닌 것)엔 안 통한다.
//   ✅ 그래서 한 글자는 **분량을 뗀 이름이 정확히 그것일 때만** 건다.
const 분량떼기 = (s) => s.replace(/[0-9]+([.,/~-][0-9]+)?\s*(g|kg|ml|l|개|장|알|쪽|대|줌|팩|봉|캔|컵|큰술|작은술|스푼|모|마리|통|단|포기|주먹)?/gi, '').trim()
export function ingIconOf(name = '') {
  const s = String(name).split(/\s+/).join('')
  const 알맹이 = 분량떼기(s)
  for (const [이름, 키] of ING_RULES) {
    if (이름.length === 1) { if (알맹이 === 이름) return 키; continue }   // ⛔ 한 글자는 «정확히 같을 때»만
    if (s.includes(이름)) return 키
  }
  return null
}
