...
const ENDINGS = [
  ...
  ['볶는다', '볶아요'],
  ['끓인다', '끓여요'],
  // 🍖 [2026-08-15] 수제 떡갈비를 넣다가 게이트가 잡은 둘. **데이터가 아니라 여기를 고친다** —
  //   다른 레시피도 같은 말을 쓸 수 있어서다(「진한맛이 난다」 때와 같은 자리).
  //   ⚠️ 「누르다」는 ㄹ 불규칙이라 «눌러요» 다. 기계로 만들면 「누르어요」가 된다 — 그래서 손으로 적는다.
  ['빚는다', '빚어요'],
  ['누른다', '눌러요'],
  ['치댄다', '치대요'],
  ['소분한다', '소분해요'],
  ...
  ['간다', '갈아요'],
  ['끈다', '꺼요'],
  // 🔥 [2026-08-29 · 창업자가 실물에서 잡았다] AI 가 「끄줘요」로 썼다 — *"꺼줘요를 끄줘요"*
  //   ⭐ 「끄-」는 ㅡ 가 떨어지는 말이라 「끄＋어」 = **꺼**다(끄어요 ⛔ / 꺼요 ✅).
  //   ⛔ ㅡ 탈락을 «규칙»으로 만들지 않는다 — 「크다·쓰다·트다」까지 건드리면 엉뚱한 말이 망가진다.
  //      이 파일 원칙 그대로 **실제로 나온 말만** 한 줄씩 넣는다.
  ['끄줘요', '꺼줘요'],
  ['끄어요', '꺼요'],
  // 🍚 [2026-08-29 · 백업 대조로 잡았다] AI 가 「…뿌리면 완성해요」로 냈다.
  //   ⭐ 「완성」은 «이름씨»라 「완성해요」가 아니라 **「완성이에요」**다.
  //      바로 위 23줄의 ['완성','완성이에요'] 와 같은 결인데, AI 가 «이미 해요체로» 내놓는 바람에
  //      그 규칙이 안 걸렸다(어미가 「완성」이 아니라 「완성해요」였다).
  //   ⛔ 「하다」 전체를 건드리지 않는다 — 「볶아요·섞어요」처럼 멀쩡한 말이 널렸다. 이 낱말만.
  ['완성해요', '완성이에요'],
  // 🗣 [2026-08-29 · 백숙 실물에서 잡았다] **한 레시피 «안»에서 말투가 갈렸다**
  //   실측 = 1·2·3 「불러주세요·닦아주세요·덮어주세요」 ／ 4·5·6 「끓여줘요·끓여줘요·부어줘요」
  //   ⭐ 원문이 「~주세요」면 AI 가 그대로 두는데, 우리 말투는 **「~줘요」**다 —
  //      기본 레시피 964걸음 중 「주세요」는 **단 1건**(실측). 갈리면 남이 쓴 글처럼 읽힌다.
  //   ⛔ 「~요」로 끝나서 기존 해요체 게이트(check-steps)는 이걸 «통과»시킨다(규칙 18 ⓘ).
  //   ⭐ 글자도 짧아진다(4자→3자) — 좁은 폰에서 손해가 아니라 이득이다.
  ['주세요', '줘요'],
  ['둔다', '둬요'],
  ['낸다', '내요'],
  ...
  ['날린다', '날려요'],
  ['편다', '펴요'],       // 얇게 편다 → 펴요
  // ── 2026-08-15 · `hold/주간레시피-11월-0813` 33편을 polish 태워 전수로 찾은 8개 ──
  //   📮 창업자 *"꼬들단무지고쳐"* 로 시작했는데 **같은 사고가 12줄 더 있었다.**
  //   ⛔ 그 브랜치는 배포 게이트를 안 타서 아무도 안 보고 있었다 — **넣는 날 한꺼번에 걸릴 자리**였다.
  //   ⭐ 여기 미리 넣어 두면 **hold 파일은 한 글자도 안 건드려도** 넣을 때 저절로 맞는다.
  //   🔢 걸린 13줄 = 배추겉절이·배추전·가리비구이·만둣국·묵은지볶음2·키쉬·굴국밥2·굴전·매생이바지락국2·꼬들단무지
  ['밭친다', '밭쳐요'],       // 체에 밭친다 → 밭쳐요
  ['두드린다', '두드려요'],   // 칼등으로 두드린다 → 두드려요
  ['벌린다', '벌려요'],       // 데쳐 입을 벌린다 → 벌려요
  ['않는다', '않아요'],       // ⭐젓지 않는다 → 젓지 않아요 (한 줄에 문장이 둘일 때 뒤가 안 바뀌던 그것)
  ['덮는다', '덮어요'],       // 뚜껑을 덮는다 → 덮어요
  ['조린다', '조려요'],       // 약불에 조린다 → 조려요
  ['찌른다', '찔러요'],       // 포크로 찌른다 → 찔러요 (⚠️「르」 불규칙 — 「찌르어요」가 아니다)
  ['떨어뜨린다', '떨어뜨려요'], // 한 방울 떨어뜨린다 → 떨어뜨려요
  ['만다', '말아요'],
  ['한다', '해요'],
