...
const ENDINGS = [
  // ── 2026-08-13 · 주간 레시피 11·12월 18편을 쓰며 나온 말 (#78) ──
  //   ⛔ 여기 없으면 화면에 «한다체 그대로» 나간다 — `check-steps` 가 배포를 막는다.
  //   ⚠️⚠️ 그런데 그 게이트는 **`basicRecipes`(오늘 열린 것)만** 본다 → `from` 이 미래인 주간 레시피는
  //      **한 줄도 검사되지 않는다.** 이번 18편도 내가 따로 돌려서 잡았다. (게이트 자체를 고칠 거리다)
  ['쪼갠다', '쪼개요'],
  ['바꾼다', '바꿔요'],
  ['덮는다', '덮어요'],
  ['두드린다', '두드려요'],
  ...
  ['조린다', '조려요'],
  ['찌른다', '찔러요'],
  // ⭐ 「젓지 않는다」·「건드리지 않는다」처럼 **부정형 종결**은 요리 순서에 자주 나온다.
  //    한 낱말씩 넣으면 끝이 없어서 «않는다» 하나로 받는다(앞말이 무엇이든 어미만 바뀐다).
  ['않는다', '않아요'],
  ['밭친다', '밭쳐요'],
  ['떨어뜨린다', '떨어뜨려요'],
  ['편다', '펴요'],
