...
export function 알림허락받기({ 다시묻기 = false } = {}) {
  ...
  // ✅ [2026-09-21 창업자 녹화 «안켜져»] 브라우저가 이미 «허용»인데 우리 답이 yes 가 아닌 폰(예: 「괜찮아요」 했다가 크롬에서 따로 허용 · 끄기 뒤) —
  //    설정 「켜기」에서 «시트도 권한창도 없이» 곧장 false 가 나와 「알림을 켜지 못했어요」만 떴다. 허용이면 물을 게 없다 → 그 자리에서 yes.
  //    ⛔ 다시묻기 «없이»(담기 자리)는 그대로 안 묻는다 — 성가시게 안 한다는 잣대(물어도되나)가 그대로다.
  if (다시묻기 && 푸시가능() && 브라우저권한() === 'granted') { 알림동의쓰기('yes'); return Promise.resolve(true) }
