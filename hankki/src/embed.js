...
export function embedUrl(url = '') {
  ...
  if (m) {
    ...
    //
    // 🙋🙋 [창업자 확정 2026-09-04] MFK 조회는 «미뤘다» — 2026-10-05 에 다시 본다
    //   📮 창업자 = *"3번으로 하고 **한달쯤 뒤에 다시생각하자.** 영상을 보면서 받아적는 사람이
    //             많을까 싶어. 요즘은 다 영상ai가 읽어주는데."* → *"ㅇㅇ 그러자"*(10/5 확정)
    //   ⛔ 이 약속을 «문서에 적지 않는다» — 그러면 반드시 잊는다(우리가 이미 배운 것:
    //      「대기·예정」이라 적어둔 줄이 넉 달 뒤에도 그대로 「대기」였다 · doc-guard --stale).
    //   ✅ 그래서 «결정이 사는 이 자리»에 적고, `release-calendar.mjs` 가 그날 달력에 꺼내 보여준다.
    //      ⭐ 주석에 둔 이유 = 앱 번들에 한 글자도 안 실린다. 달력은 파일 «글자»를 읽는다.
    //      ⛔ 아래 세 줄의 모양(`recheck:` · `on:` · `what:`)을 바꾸면 달력이 못 읽는다.
    //   recheck: [
    //     { on: '2026-10-05', what: 'MFK 조회를 붙일지 다시 정한다 — 붙이면 ①아동용 여부 ②공식 썸네일 주소가 한 번에 온다(회색 둘이 같이 지워진다). 창업자 몫 = API 키 발급 한 번' },
    //   ]
