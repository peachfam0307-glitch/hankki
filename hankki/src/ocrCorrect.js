...
export function normalizeNumerals(text) {
  ...
  s = s.replace(/(\d)\s+(kg|g|ml|cc|cm|mm|개|알|쪽|봉|모|장|대|톨|컵|스푼|숟가락|줌|꼬집|줄|캔|팩|조각|인분|마리|공기)/gi, '$1$2')
  // 범위: "900g - 1kg", "2~3개" → 물결(~)로 통일. 오른쪽에 '단위'가 있을 때만 바꿔
  // 연도·전화·날짜(2024-01-15, 010-1234 — 단위 없음)는 건드리지 않는다.
  s = s.replace(
    /\b(\d{1,3}[a-zA-Z가-힣]{0,3})\s*[-–—~]\s*(\d{1,3}\s*(?:kg|g|ml|l|cc|cm|mm|개|알|쪽|봉|모|장|대|톨|컵|스푼|숟가락|줌|꼬집|줄|캔|팩|조각|인분|마리|공기|리터))/g,
    '$1~$2'
  )
  // 크기 표기: "2cm x 2cm", "20 * 30" → 곱셈기호(×)로
  s = s.replace(/(\d\s*(?:cm|mm|g|kg)?)\s*[xX*]\s*(\d)/g, '$1×$2')
  // 작은술 't' 오독: OCR이 t를 '+'로 자주 읽는다 (소금 2+ → 2t, 참치액 1/2+ → 1/2t).
  // 숫자에 '바로 붙은' +만 교정 — "2 + 설탕"처럼 띄어 쓴 더하기는 건드리지 않는다.
  s = s.replace(/(\d)\+(?!\d)/g, '$1t')
  return s
}
