...
export function normalizeNumerals(text) {
  ...
  // 작은술 오독: OCR이 t를 '+'로 자주 읽는다 (소금 2+ → 2작은술, 참치액 1/2+ → 1/2작은술).
  // 숫자에 '바로 붙은' +만 교정 — "2 + 설탕"처럼 띄어 쓴 더하기는 건드리지 않는다.
  // 🥄 [창업자 확정 2026-09-03] *"큰술로 통일하자. 스푼 다 빼고"* → **「t」가 아니라 「작은술」로 적는다.**
  //   ⛔ 전엔 「$1t」였다 — **앱이 «직접» t 를 적고 있었다.** 창업자가 콕 집은 자리다:
  //      *"우리 큰술 작은술 적는 거 있었잖아(레시피 붙여넣기로 정리할때) 거기도 바꿔줘"*
  //   ⭐⭐ 「나중에 `단위통일()` 이 고쳐준다」로는 **모자란다** — 이 함수는 세 곳에서 불리는데
  //      `EditorScreen.jsx:35` 와 `ocr.js:626·656` 은 파서를 «안» 거친다(＝단위통일도 안 거친다).
  //      그 두 문으로 들어온 글은 「2t」인 채로 편집칸에 그대로 앉는다. 그래서 여기서 적어야 맞다.
  s = s.replace(/(\d)\+(?!\d)/g, '$1작은술')
  return s
}
