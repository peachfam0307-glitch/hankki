...
// 🐧🧥🧥 **[2026-09-02 밤 · 창업자 = *"옛펭펭아직있어"*] 씬 풀에도 옛 펭펭이 살아 있었다.**
//
//   ⛔⛔ **세 번째다.** 낮에 «솔로» 9컷 → 저녁에 «콤비» 12컷 → 밤에 **«씬»**.
//      세 번 다 뿌리가 같다 — **잣대를 「이름」으로 세웠다.**
//      📌 저녁에 `_repro-옛펭펭내림-0902.mjs` 주석에 내 손으로 이렇게 적어뒀다:
//         *"잣대를 「그 파일 이름」이 아니라 «그 그림이 나오나»로 세웠어야 했다."*
//         **그래놓고 또 이름으로 막았다.** 씬은 `scene_` 이라 `/^(peng_|pn_|duo_)/` 에 안 걸리고,
//         게다가 **풀이 아예 다르다**(`scenepool` ↔ `sharepool`) — 잣대가 그 폴더를 본 적이 없다.
//
//   🔍 10장을 «다 열어» 눈으로 갈랐다(규칙 21) — 잣대는 정본 문서와 같다: **흰 트렌치 ＋ 벨트 버클 없음**
//      ⛔ 옛 펭펭 **여섯** = camp · cook_kitchen · market · movie · sandcastle · soup
//      ✅ 남는 넷 = night_market · picnic · pool · walk (여름 원피스·수영복 — 트렌치가 아니다)
//
//   ⚠️⚠️ **비용을 적어 둔다** — 이걸 빼면 **사철 씬이 6장 → `walk` 하나**가 된다.
//      폴라로이드는 65% 확률로 씬에서 뽑으므로 12~8월엔 **늘 같은 그림**이 나온다.
//      👉 창업자가 *"대체컷 필요하면 뽑아줄게"* 라고 했다 → 요청서 = `docs/stickers/씬-다시뽑을것-2026-09-02.md`
//   ⛔ **파일은 «안» 지운다** — 대체컷이 오면 그 자리에 그대로 갈아끼운다.
const 옛펭펭씬 = /^scene_(camp|cook_kitchen|market|movie|sandcastle|soup)\./
