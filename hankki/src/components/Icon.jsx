...
const PATHS = {
  ...
  // 🔒 일기 잠금 (창업자 2026-08-15) — 잠긴 것 / 열린 것 한 짝.
  //    ⭐ 몸통은 둘이 «똑같다» — 고리만 열린다. 그래야 같은 자물쇠가 열린 것으로 읽힌다.
  lock: (
    <>
  ...
  unlock: (
    <>
      <rect x="5" y="10.5" width="14" height="9.5" rx="2.2" />
      <path d="M8.2 10.5V8a3.8 3.8 0 0 1 7.6 0" />
    </>
  ),
  clock: (
    <>
  ...
  // 📱 이 폰 — 클라우드 시트의 「이 폰 ↔ 클라우드」 두 판에 쓴다 (2026-08-27)
  //   ⛔ 그 자리에 «유니코드 이모지»(📱☁️)가 박혀 있었다 — 창업자가 강력·반복 명시한 금지 항목이다.
  //      게다가 바로 윗줄엔 우리 `cloud` 아이콘이 있어 «한 화면에 구름이 두 종류»였다.
  //   ⭐ 폰 아이콘이 우리 세트에 «없어서»(48개 중 0) 새로 그렸다 — 네모는 격자·사진이 이미 쓴다.
  phone: (
    <>
  ...
  // 📏 글자 «크기» — 큰 A 옆에 작은 A. 「글씨(글씨체)」와 한 줄에 나란히 서므로
  //    같은 A 를 쓰되 «두 크기»로 그려서 뜻이 갈린다(창업자 2026-08-12 요청으로 신설).
  textSize: <path d="M3 19 7.6 6h1L13.2 19M4.6 15h5.8M14.5 19l3.1-8h.8l3.1 8M15.6 16.4h4.4" />,
  share: (
    <>
      <path d="M12 3.5v10.5" />
