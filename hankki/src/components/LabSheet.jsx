      ...
      <div className="sheet-mask" onClick={onClose}>
        ...
        >
          ...
          <div style={{ overflowY: 'auto', padding: '2px 16px 0' }}>
            ...
            {rows.map((r) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  ...
                  <div style={{ flex: 1, fontSize: 15.5, fontWeight: 700, color: 'var(--text)' }}>{r.title}</div>
                  {r.badge && <span className="badge badge-sorted">{r.badge}</span>}
                  <Icon name="chevron-right" size={17} color="var(--sand)" />
                </div>
                <div className="t-sub" style={{ fontSize: 14.6, lineHeight: 1.5, marginTop: 5, paddingLeft: 28 }}>{r.desc}</div>
                ...
                {r.chips && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 8, paddingLeft: 28 }}>
                    {r.chips.map((c) => (
                      <span
                        key={c}
                        style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-sub)', background: 'var(--cream)', border: '1px solid var(--line)', borderRadius: 999, padding: '3px 9px' }}
                      >
                        {c}
                      </span>
                    ))}
            ...
            <div className="h-section" style={{ fontSize: 15, margin: '18px 0 4px' }}>이렇게 고쳐가고 있어요</div>
            <div className="t-sub" style={{ fontSize: 14, lineHeight: 1.5, marginBottom: 9 }}>
            ...
            <div className="card" style={{ padding: '4px 14px', background: 'var(--cream)', border: 'none' }}>
              {FIXED.map((t, i) => (
                ...
                >
                  ...
                  <span className="t-sub" style={{ fontSize: 14.8, lineHeight: 1.5, flex: 1 }}>{t}</span>
                </div>
              ))}
            </div>
          </div>
