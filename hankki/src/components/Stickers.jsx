...
export const STICKER_GROUPS = [
  ...
  // 🎁🎁 **[창업자 확정 2026-09-02] 「한 번만 드리는 선물」** — ⛔「출시기념 선물」로 되돌리지 말 것
  //   📮 창업자 원문 = *"**가을의 정원은 한정선물이잖아. 오해할 듯해. 매달주는걸로**"*
  //   ⭐⭐ 옛 글자(「출시기념 선물」)도 뜻은 «한 번뿐»이었는데 **그렇게 안 읽힌다** —
  //      「출시기념」은 «왜 주나»를 말하지 «몇 번 주나»를 말하지 않는다.
  //      유저는 그 옆의 선물 컷 넉 장을 보고 **「매달 이런 게 오는구나」**로 읽는다.
  //      📌 그게 창업자가 오늘 내내 걱정한 그것이다 — *"이번에 많이주면 담에 어쩌지;;"*
  //   ⛔⛔ **「한정」이라는 낱말은 «안» 쓴다** — 우리 선물엔 `until` 이 없어서 **안 사라진다.**
  //      「한정」은 「곧 없어진다」로 읽혀 **거짓말이 되고**, 절대원칙(*"한 번 준 것은 빼앗지 않는다"*)과도 부딪친다.
  //      ✅ 창업자가 뜻한 것은 «기간»이 아니라 **「매달 오는 게 아니다」** — 그걸 그대로 적는다.
  { key: 'deco_gift_dish_autumn', tab: 'frame', gift: true, giftLabel: '한 번만 드리는 선물', bigCell: true, from: '2026-09-01', label: '가을의 정원 세트', hint: '직접 찍은 음식 사진을 접시에 담아보세요', items: ['pf_ad01', 'pf_ad04', 'pf_ad05', 'pf_ad08'] },
