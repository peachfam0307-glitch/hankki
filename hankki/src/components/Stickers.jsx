...
// 🎃 [2026-09-20] 할로윈 접시 — 창업자가 뽑은 시트 둘에서 8컷을 잘라 «6컷만» 올렸다
//    🔢 잰 비율(가로÷세로) = 우리 집 모양은 **1.3 근처**다(가을 pf_ad = 1.24~1.34)
//       ✅ 올린 여섯 = 1.258 ~ 1.450
//       ⛔ 뺀 둘 = pf_hw(1.579)·(1.596) — 창업자 *"우리 기본그릇이 동그래서 길쭉한 프레임은 별로네"* · *"가로로 긴건 빼자"*
...
//    🔢 잰 비율(가로÷세로) — PNG 실제 크기. w = 시트1(①냄비 ②둥근볼 ③얕은손잡이접시 ④고리손잡이접시) · x = 시트2(⑤고리손잡이볼 ⑥주름볼 ⑦넓은접시 ⑧한손잡이팬)
  ...
  // 🍽🍽 [창업자 2026-09-23 00:1x] **테두리 있는 기본 그릇 4컷** — 창업자가 새로 뽑아 줬다(진갈색 테 한 줄).
  //    📮 *"테두리있는 기본그릇뽑았어. 이거 넣고 유저한테는 안보이게 해줘."* → *"대체하자."*
  //    ⭐ 값은 전부 **파일을 열어 잰 것**이다(Pillow · 가로÷세로). ⛔짐작으로 적지 않았다.
  pb_n01: 1.2663, pb_n02: 1.1107, pb_n03: 1.1588, pb_n04: 1.5000,
...
//    ⭐ 비율은 «잰 값»이다(PIL 실측 = 가로 ÷ 세로). ⛔ PNG 를 갈아끼우면 다시 잰다.
  ...
  n3007: 1.2646,
  n3008: 1.2800,
  // 🚨🚨 [2026-09-14 00:10 · 창업자 실물] *"음식아이콘안올라갔어"* — 빈 접시가 넷 떴다
  //    (꼬치 없는 꼬치전 · 새우관자전 · LA갈비 · 알배추전 — 그날 열린 편들).
  //    ⛔⛔ 뿌리 = **이 표에만 안 적혀 있었다.** 아이콘 파일(photo/n30*.png)도 있고
  //       FoodIcon 의 이름표(FOOD_NAMES)에도 등록돼 있었는데, PHOTO_FAMILY 는 «이 표를 돌면서» 만들어진다
  //       → 여기 없으면 src 가 undefined 가 되어 «빈 접시»가 된다.
  //    📌 또 「같은 것을 여러 곳에 손으로 적는다」였다 — 파일·이름표·비율표 셋 중 하나를 빼먹었다.
  //    ⭐ 값은 전부 **파일을 열어 잰 것**이다(Pillow · 가로/세로). ⛔짐작으로 적지 않았다.
  n3009: 1.1993,
  n3010: 1.2022,
  n3011: 1.1830,
  ...
  n3021: 1.2688,
  n3022: 1.3022,
  // 🌶 [2026-09-14] 매운 소갈비찜 — 창업자가 뽑아 줬다(«매운소갈비도 고춧가루가없네»).
  //    옛 컷 fh_k11 은 «간장» 갈비찜이라 고춧가루 3큰술 ＋ 고추장 1.5큰술이 들어가는 편과 색부터 달랐다.
  n3023: 1.3495,
  // 🎨 [2026-09-14] 창업자가 뽑아 준 여섯 — 이름과 그림이 어긋난 자리를 메운다(검수판 1·2·3·4·12·13)
  n3024: 1.2895, n3025: 1.2929, n3026: 1.2042, n3027: 1.2938, n3028: 1.2774, n3029: 1.2794,
  // 🚨🚨 [2026-09-15] **또 같은 구멍이었다** — n3030~n3033 이 파일도 이름표도 있는데 «이 표에만» 없었다.
  //    그래서 「간단 우삼겹 갈비탕」·「충무김밥 오징어무침」 둘이 «이미 열린 채로» 빈 접시로 떠 있었다.
  //    ⛔ 2026-09-14 00:10 에 n3009~n3012 로 똑같이 났고, 그때 주석까지 적어 뒀는데 하루 만에 되풀이했다.
  //    🔒 그래서 이번엔 «검사»를 만들었다 = scripts/check-photoratio.mjs — photo/*.png 와 이 표가 어긋나면 배포가 죽는다.
  //    ⭐ 값은 전부 파일을 열어 잰 것이다(Pillow · 가로/세로). ⛔짐작으로 적지 않았다.
  n3030: 1.2830, n3031: 1.2954, n3032: 1.2098, n3033: 1.2008,
  // 🌯 [2026-09-15] 창업자가 뽑아 준 브리또 두 컷 (비프페퍼브리또 · 무스비맛브리또)
  n3034: 1.2366, n3035: 1.2320,
  // 📱 [2026-09-25] 창업자가 뽑아 준 SNS 레시피 컷 넷 (시오콘부 치킨 · 소고기무솥밥 · 니쿠자가 · 오이탕탕이)
  n3036: 1.3476, n3037: 1.2621, n3038: 1.2302, n3039: 1.2287, n3040: 1.2667, n3041: 1.2453, n3042: 1.2682, n3043: 1.2672,
}

// 🔒🔒 [2026-09-15] **「빈 접시」가 «날 수 없게» 구조를 바꿨다.**
//    ⛔⛔ 같은 사고가 이틀에 두 번 났다 —
//       · 2026-09-13 v13.35 = n3009~n3022 열넷이 이 표에 없어 빈 접시
//       · 2026-09-14 v13.42 = n3030~n3033 이 «또» 없어서, 이미 열린 두 편(간단 우삼겹 갈비탕 ·
//         충무김밥 오징어무침)이 **유저 폰에 빈 접시로 떠 있었다**(2026-09-15 창업자 지적으로 발견).
//    🌲 뿌리 = v13.35 때 «빠진 값을 채우기»만 하고 **구조도 검사도 안 만들었다**(그 커밋에 check-* 파일 0개).
//       컷 하나를 넣으려면 손으로 «세 곳»(파일 · FOOD_NAMES · 이 표)에 적어야 했고,
//       그중 이 표를 빼먹으면 **그림 주소가 통째로 undefined** 가 되어 기본 SVG(빈 접시)로 떨어졌다.
//    ⭐⭐ 그래서 «숫자를 채우는» 대신 **실패의 모양을 바꿨다**(절대원칙 34) —
//       목록의 주인을 «손으로 적는 표»에서 **«폴더에 있는 파일»**로 옮겼다.
//       이제 비율을 빼먹어도 **그림은 뜬다.** 최악이 「안 보임」에서 「비율이 조금 어긋남」으로 내려간다.
//    🔒 2차 그물 = scripts/check-photoratio.mjs (스모크) — 레시피가 쓰는 키가 이 표에 없으면 배포가 죽는다.
//    ⚠️ 기본값 1.25 = 지금 표에 적힌 사진 컷들의 흔한 가로세로비다(대부분 1.20~1.30).
//       ⛔ 세로로 긴 컷(예: 0.73)은 이 값으로 뜨면 눌려 보인다 — 그래서 «게이트로 잡아» 제대로 적게 한다.
const 기본비율 = 1.25
export const PHOTO_FAMILY = {}
for (const 경로 of Object.keys(PHOTO_URLS)) {
  const key = 경로.slice(경로.lastIndexOf('/') + 1, -4)
  PHOTO_FAMILY[key] = { src: PHOTO_URLS[경로], ratio: PHOTO_RATIO[key] || 기본비율 }
}
// 표에만 있고 파일이 없는 키는 그대로 둔다 — 옛 코드가 부르면 src 가 빈 값이라 지금과 같이 동작한다.
...
// 🍽🍽 [창업자 확정 2026-09-23 00:16] **테두리 그릇 4컷으로 «대체»한다.**
//    📮 창업자 = *"대체하자. 새시트 1번은 냄비(국찌개탕조림) 2.볼(면,덮밥등) 3.접시(반찬,구이) 4.손잡이그릇(소스,샐러드)"*
//       ＋ *"네가 판단해서 범용으로 쓸 수 있게 넣어줘. 그릇 4개면 음식 넣는 데 지장 없을 것 같아."*
//    ⭐ 그래서 **갈래 15개를 창업자가 준 네 바구니에 나눠 담았다** — 아래 주석에 «왜 그 바구니인지»를 한 줄씩 적는다.
//    ⛔ 옛 8컷(`pb_*`)은 «지우지 않는다» — 파일도 비율표도 그대로 둔다(옛 폰에 저장된 꾸미기가 그걸 부른다).
const 기본그릇표 = {
  // ① 🍲 냄비 — 국물이 있거나 «끓이고 졸이는» 것
  '국·탕·찌개': 'pb_n01',
  '볶음·조림·찜': 'pb_n01',        // 창업자가 「조림」을 냄비로 못 박았다. 볶음·찜도 같은 갈래라 함께 간다
  // ② 🥣 볼 — 한 그릇에 «담아 비비거나 말아» 먹는 것
  '면': 'pb_n02',
  '밥': 'pb_n02', '밥·면·빵': 'pb_n02',   // 창업자 「덮밥 등」
  '분식': 'pb_n02', '동남아': 'pb_n02', '중식': 'pb_n02',   // 면·덮밥이 많은 갈래다
  // 🥗 [창업자 2026-09-23 00:53 · 실물을 보고 바꿨다] *"포케볼은 오목한 볼에 넣어야겠네.. 저 그릇이 넘 작아서 소스랑 양념장만 하자"*
  //    ⛔ 처음엔 창업자가 「소스·샐러드」라고 해서 ④로 뒀는데, 판을 찍어 보니 손잡이 그릇이 포케볼엔 작았다.
  //    ⭐ 그래서 포케·샐러드는 ② 오목한 볼로 옮긴다. ④는 «소스·양념장»만 남는다.
  '포케·샐러드': 'pb_n02',
  // ④ 🍳 손잡이 그릇 — 소스·양념장 «만»
  '양념·소스': 'pb_n04', '양념·장': 'pb_n04',
  // ③ 🍽 접시 = 그 밖 전부(아래 기본값) — 반찬·구이·전·회·양식·일식·빵디저트·모름
}
export const 기본그릇키 = (갈래) => 기본그릇표[갈래] || 'pb_n03'   // ③ 넓은 접시 = 반찬·나물·김치 · 구이·튀김·전 · 회·수육 · 양식 · 일식 · 빵·디저트·음료 · 모름
...
export const STICKER_GROUPS = [
  ...
  // 🎃 [2026-09-20] 할로윈 접시 6컷 — ⛔**창업자 열쇠(`?할로윈=1`) 뒤에만 보인다.**
  //    📮 창업자 = *"아래꺼 내가 직접 담아보고 싶은데"* → 실물로 담아 보고 1.45 짜리 둘을 남길지 정한다.
  ...
  //    🌲 왜 = 흰 도자기는 «음식 갈래에 맞춰 저절로» 깔리는 기본값이다(`기본그릇키` · Thumb.jsx). 유저가 고를 물건이 아니다.
  //    🔢 실측(2026-09-22 14:3x · 버섯전 · 열쇠 없는 폰) = 서랍에서 고르면 «바뀌지 않고 겹친다» —
  //       자동 pb_x03 → pb_w02 고르니 [pb_x03, pb_w02] → pb_x01 고르니 [pb_x03, pb_x01, pb_w02]. 흰 테가 두세 겹으로 포개져 망가져 보인다.
  //    ⛔ 내가 2026-09-22 13:4x 에 Thumb 열쇠를 떼면서 «이 묶음까지» 같이 떼서 유저에게 나갔다. 그걸 되돌리는 것이다.
  //    ⭐ Thumb.jsx 의 자동 흰 도자기는 «열쇠 없이» 그대로 유저에게 나간다 — 그건 창업자가 「올려야지」 한 것이다.
  { key: 'deco_dish_basic', tab: 'frame', key열쇠: '그릇', bigCell: true, from: '2026-01-01', label: '기본 그릇', hint: '내 사진을 흰 그릇에 담아요', items: ['pb_n03', 'pb_n02', 'pb_n01', 'pb_n04', 'pb_x03', 'pb_w02', 'pb_w01', 'pb_x01', 'pb_w03', 'pb_x02', 'pb_w04', 'pb_x04'] },
