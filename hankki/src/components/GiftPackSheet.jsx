      ...
      <div className="sheet-mask" onClick={close}>
        <div className="sheet" onClick={(e) => e.stopPropagation()} style={{ paddingBottom: 'calc(20px + var(--safe-bottom))' }}>
          ...
          <div style={{ padding: '2px 16px 0' }}>
            <div className="t-sub" style={{ fontSize: 13.5, lineHeight: 1.6, margin: '2px 0 12px', textAlign: 'center' }}>
              한끼가 정식으로 나왔어요.<br />
              ...
              {GIFTS.map((g, i) => (
                <button
                  ...
                  <span style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ display: 'block', fontSize: 14.5, fontWeight: 800, color: 'var(--text)' }}>{g.title}</span>
                    <span className="t-sub" style={{ display: 'block', fontSize: 12.5, marginTop: 2 }}>{g.where}</span>
                  </span>
                  <span aria-hidden style={{ color: 'var(--text-sub)', fontSize: 18, flex: '0 0 auto' }}>›</span>
                </button>
              ))}
            </div>
