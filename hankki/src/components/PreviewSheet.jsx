...
// 📌 순서 = 「가까운 것부터」. 자산·코드가 얼마나 있느냐로 줄을 세운다.
//    레꾸상점(팩 5개 ＋ `PackBuySheet` 이 이미 있다) → PDF(폰 안에서 끝난다) → 커뮤니티(서버가 있어야 한다).
const UPCOMING = [
  // 🛒 [2026-08-31 창업자] *"레꾸상점 넣자"*
  //   ⭐ 이름은 창업자가 지은 것이다(2026-08-24) — 상품 = 「레꾸팩」 · 파는 화면 = 「레꾸상점」.
  //      ⛔ 「레시피열쇠」와 «다른 갈래»다 — 열쇠는 AI 이용권(쓰면 없어진다), 레꾸팩은 소장(영구).
  //         창업자가 콕 집었다 = *"열쇠 사서 → 다시 팩을 사야 해?"* 로 한 단계 더 생각하게 된다.
  //   ✅ 여기 적어도 «거짓말이 아니다» — 팩 5개(186컷)와 `PackBuySheet.jsx` 가 이미 있다.
  //      ⛔ 그래서 이 줄은 「도장·컨페티」(코드·자산 0인데 예고했던 것) 사고와 다르다.
  //   ⛔ 값·시점은 한 글자도 안 적는다 — `sellable` 5개는 전부 false 이고 결제는 9월 판정이다.
  //      태그가 「나중에」인 이유가 그것이다.
  //   🏷 아이콘 `tag`(가격표) — ⛔`cart` 는 장바구니가, `gift` 는 선물이, `palette` 는 꾸미기가
  //      이 시트에서 이미 쓴다. 같은 그림이 두 번 뜨면 둘 다 안 읽힌다(2026-08-29 창업자 지적).
  { icon: 'tag', title: '레꾸상점', desc: '마음에 드는 꾸미기 팩을 골라요.', tag: '나중에' },
  // 📕 [2026-08-31 창업자 확정] *"내 레시피북, PDF로 만들기 / 꾸민 표지 그대로 예쁜 책 한 권. - 이렇게 가자"*
  //   ⭐ 「소장」 → 「만들기」 — 유저가 «하는 일»로 바뀌었다. 소장은 결과고 만들기는 행동이다.
  { icon: 'book', title: '내 레시피북, PDF로 만들기', desc: '꾸민 표지 그대로 예쁜 책 한 권.', tag: '나중에' },
