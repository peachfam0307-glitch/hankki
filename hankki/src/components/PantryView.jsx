...
import { 앱안인가 } from '../nativeAuth'   // 🍎 아이폰엔 「갤러리」가 없다 — 「사진」(2026-09-13)
  ...
  const saveFound = () => {
    ...
    names.forEach((nm) => {
      if (!pantry.some((p) => p.name === nm)) {
        ...
        try { 냉장고담음() } catch { /* 📊[09-14] 넣었다 · 아래 ⛔주석 참조 */ }
  ...
  return (
    <div className="fade">
      ...
      >
        ...
        {앱안인가() ? '사진에서 영수증 고르기' : '갤러리에서 영수증 고르기'}
      </button>
...
function PantryForm({ item, onClose }) {
  ...
  const save = () => {
    ...
    if (editing) { updatePantry(item.id, data); nav.showToast('재료를 수정했어요') }
      ...
      try { 냉장고담음() } catch { /* 통계가 죽어도 냉장고는 된다 */ }
      열쇠받기(EARN.냉장고).then((받음) => {
        if (받음) nav.showToast(`냉장고를 처음 채웠어요 · ${KEY_NAME} 1${KEY_UNIT}를 더 받았어요`, 5200)
      })
    }
