...
const NONFOOD =
  /비닐|종량제|종량|봉투|쇼핑백|에코백|장바구니|물티슈|휴지|화장지|기저귀|생리대|세제|세탁|섬유유연|샴푸|린스|바디|비누|치약|칫솔|면도|건전지|배터리|수세미|호일|위생장갑|지퍼백|담배|라이터|양말|의약|밴드|마스크|다이슨|청소기|가전|드라이기|고데기|믹서기|블렌더|충전기|이어폰|건조기|cordfree|타월|프레스앤씰|크린랩|위생랩|랩\s*$/i
...
export function extractReceiptItems(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // C) ⭐ 가격이 «없어도» 바로 다음 줄이 「바코드·수량·금액」이면 이 줄이 품목 이름이다.
    //    (2026-08-15 창업자 영수증 — 17품목 중 하나만 걸리던 뿌리)
    if (isPriceOnlyLine(lines[i + 1])) takeFrom(line)
  }
  return found
}
