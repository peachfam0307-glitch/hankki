...
export default {
  async fetch(request, env) {
    ...
    if (new URL(request.url).searchParams.get('quota') === '1') {
      ...
      const [월, 일, 월창, 일창, 막힘, 기본, 기본창, 기본막힘, 기본고름, 기본실패] = await Promise.all([
    ...
    if (body.기본) {
      ...
      const 갈래 = ['막힘', '고름', '실패'].includes(String(body.기본)) ? String(body.기본) : '실패'
