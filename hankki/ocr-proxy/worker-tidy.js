...
const ALLOWED_ORIGINS = [
  ...
  // 🍎 [2026-09-13] 아이폰 앱(Capacitor 껍데기) 안의 주소 — 없으면 AI 다듬기가 아이폰에서 403 으로 «항상» 실패한다.
  //    ⛔ 이 줄은 «서버에만» 들어가 있었고 저장소엔 없었다(2026-09-13 대조로 찾음) → 갈리지 않게 여기도 맞춘다.
  'capacitor://localhost',
]
...
7. steps 는 모두 "~요"로 끝나는 부드러운 말투로 바꿔라.
   ...
   ⛔ 숫자나 명사로 끝나는 줄에 "요"만 «덜렁» 붙이지 마라 — 어울리는 말을 채워라.
   (예: "중불에 10분"→"중불에 10분 익혀요"  ⛔"중불에 10분 요" 아님
        "10분 뜸들이면 끝"→"10분 뜸들이면 돼요"  ⛔"10분 뜸들이면 요" 아님
        "비벼주면 완성"→"비벼주면 완성이에요"  ⛔"비벼주면 요" 아님)
  ...
  async fetch(request, env, ctx) {
    ...
    if (new URL(request.url).searchParams.get('quota') === '1') {
      ...
      return json({
        ...
        //   🤖 [2026-09-14] **어느 AI 가 한 것인지** — 📮창업자 = *"ai가 2개가 아니라 3개가 도는거야??"*
        //      · `사진까지본것` = **gemma-4-26b** 가 사진＋글자를 같이 본 것 (VISION_MODEL · 541줄)
        //      · `글자만`      = **llama-3.3-70b** 등이 글자만 본 것 (DEFAULT_MODELS · 78줄)
        //   ⭐ 둘을 «한 통»에 담는 까닭 = 같은 일(정리)을 하고 **같은 한도(하루 200)를 나눠 쓴다.**
        //      따로 세면 「합쳐서 한도에 얼마나 찼나」를 볼 때마다 더해야 한다.
        //   ⛔ 그래서 열쇠통을 셋으로 늘리지 않는다 — 이 한 줄에 셋이 다 보인다.
        //      (📷사진 읽기는 «다른 워커»다 = `worker.js` 의 구글 Vision)
    ...
    const 일하기 = async () => {
    // ⚡⚡ **[2026-09-13 · 창업자 「너무 불안정하니까」] 빠른 글자 모델을 «맨 앞»에 세운다.**
    //
    //   ⛔⛔ 그 전엔 사진이 있으면 «눈 모델을 먼저» 불렀다. 그래서 결과가 둘로 갈렸다 —
    //      🔢 실측(2026-09-13 밤 · 창업자 갤럭시) = 📷없음 → llama 글자 모델 → **8.5초 성공**(AI_OK 로그)
    //                                              📷실음 → 눈 모델 먼저 → **안옴** 넷(21:09~21:47)
    //      ⭐ 「편차가 크다」가 아니라 **「8.5초 아니면 빈손」**이었다. 중간이 없었다.
    //   🌲 뿌리 = 눈 모델은 답 길이가 8000 토큰까지 열려 있어 «생각»을 길게 시작하면
    //      앱이 기다리는 120초(tidy.js 5초 × 24번)를 구조적으로 넘긴다. 넘으면 앱은 떠나고
    //      워커는 그 뒤에도 돌아 **뉴런은 태우고 결과는 버린다.**
    //   ✅ 그래서 «차례»를 뒤집는다 — 빠른 길이 기본, 눈은 «빈손일 때만» 도는 보험.
    //      ⛔ 사진을 버리는 게 아니다. 글자 모델이 빈손일 때 눈이 그대로 받는다(아래 차례 그대로).
    //      ⛔ 사진이 없는 편은 한 글자도 안 달라진다.
    //   📌 절대원칙 34 — 숫자를 넓힌 게 아니라 «실패의 모양»을 바꿨다:
    //      「사진을 실으면 2분 넘겨 빈손」 → 「사진을 실어도 빠른 길을 먼저 탄다」
    const 차례 = [
...
// ⚡⚡ **[2026-09-13 · 8000 → 2000] 답 길이가 «기다리는 시간»을 정한다.**
//   ⛔⛔ 8000 으로 열어두면 모델이 생각을 길게 시작한 날 그 길이를 다 쓴다.
//      🔢 토큰을 다 쓰는 데 걸리는 시간이 앱 대기(120초 = tidy.js 5초 × 24번)를 구조적으로 넘긴다.
//      → 유저는 2분 기다리고 빈손, 우리는 뉴런을 태우고 버린다. 창업자 = *"너무 불안정하니까"*
//   🔢 **왜 2000 인가 (잰 값)** — 레시피 한 편의 JSON 은 제목·재료 15·걸음 6·메모까지
//      한글 기준 대략 600~900 토큰이다(창업자 실물 「가지덮밥」·「멸치볶음」으로 셈). **두 배 넘는 여유**다.
//      ⛔ 「되겠지」로 고른 숫자가 아니다(절대원칙 34).
//   ⛔ 2026-09-01 사고(생각하다 잘려 빈손)가 되살아날까 — 그때 넣은 「즉답 못박기」가
//      프롬프트 «맨 앞»에 그대로 있고, 게다가 이제 눈은 «글자 모델이 빈손일 때만» 도는 보험이라
//      눈이 빈손이어도 지금보다 나빠질 자리가 없다.
